...
## 7. 执行矩阵全面测试（2026-06-06 第三轮，在线设备）

> 在线设备仅 2 台：`120200087125BJB0002`(FAP/BSQ7040A452) / `1202000240194DP0026`(FAP/mBS31001/SC)。**离线设备场景已按用户决定排除**。
> 为覆盖「成功 + 失败 path」：取 1 个有效 path `Device.DeviceInfo.SoftwareVersion` + 1 个该设备不支持的 path `Device.DeviceInfo.AdditionalHardwareVersion`。
> 模式：**整体下发**=一条 RPC 含全部 path（GPV 全或无）；**逐 PATH**=每 path 一条 RPC（成败独立）。

### 7.1 执行矩阵（通道 × 模式 × 设备数）
| 编号 | 通道 | 模式 | 设备 | 预期 |
|---|---|---|---|---|
| TC-M1-1 | 指定参数(裸路径) | 整体下发 | 1 在线 | 1 个 device_task；含无效 path → 整行 9005 失败、读回列空 |
| TC-M1-2 | 指定参数 | 逐 PATH | 1 在线 | 2 个 device_task（每 path 一条、顺序）；有效 path=值、无效=「✗ 失败」、行状态失败 |
| TC-M1-3 | 命令参数(结构化) | 整体下发 | 1 在线 | 1 个 device_task；同 M1-1（友好名列） |
| TC-M1-4 | 命令参数 | 逐 PATH | 1 在线 | 2 个 device_task；同 M1-2（友好名列） |
| TC-M2-1 | 指定参数 | 整体下发 | 2 在线 | 2 个 device_task；逐设备结果行（混类型 per-product 翻译） |
| TC-M2-2 | 指定参数 | 逐 PATH | 2 在线 | 4 个 device_task（2 设备 × 2 path）；每设备合并一行 |
| TC-M2-3 | 命令参数 | 整体下发 | 2 在线 | 2 个 device_task；逐设备（友好名列） |
| TC-M2-4 | 命令参数 | 逐 PATH | 2 在线 | 4 个 device_task；每设备合并一行 |

### 7.2 命令记录
| 编号 | 用例 | 预期 |
|---|---|---|
| TC-HIS-ORD | 命令记录按执行时间倒序 | 最近执行在最上 |
| TC-HIS-SW | 点击命令记录条目切换 | 右侧执行结果切到对应命令（含结果行、命令名、汇总） |

### 7.3 结果正确性
| 编号 | 用例 | 预期 |
|---|---|---|
| TC-R-1 | 单设备结果 | 1 行，状态 + 读回值/故障正确 |
| TC-R-N | 多设备结果 | N 行（每设备一行），逐设备状态独立 |
| TC-R-MIX | 含成功 + 失败 | 同一结果集里成功(值)、失败(故障/✗)并存，汇总统计正确 |

### 7.4 下载
| 编号 | 用例 | 预期 |
|---|---|---|
| TC-DL-AGG | 全部下载（多设备汇总 CSV） | 行=设备，列=参数 path，含状态/故障码/故障信息逐设备区分 |
| TC-DL-DEV | 单设备结果下载 | 纵向：设备摘要 + 参数路径/读回值；成功/失败 path 清晰 |
| TC-DL-ANALYZE | CSV 内容分析 + 建议 | 评估是否「既能看单设备成败 path，也能看某设备详情」，给改进建议 |
