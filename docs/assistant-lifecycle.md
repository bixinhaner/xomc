...
发布版本保存确认时的 `scope_digest`，并核对它与最近成功试运行的范围一致。每次新任务继续使用该摘要，不能在定时触发时自动扩大范围。已有未绑定摘要的预发布版本必须编辑成新草稿、重新试运行和发布。

菜单及内置角色的页面授权进入基线 `MainReconcile` 区段，已有数据库无需重置即可补齐。取消后重新读取远端 ledger：若任务先完成或失败，保留其真实终态和结果，不改写为已取消。
