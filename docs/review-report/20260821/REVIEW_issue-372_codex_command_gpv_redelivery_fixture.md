...
- `TestResultConsumerRealNATSRedeliveryAfterCommitDoesNotDuplicateBusinessProjection` 覆盖 PG commit 后 ACK 失败再投递的真实组合，并断言 result、staging、device parameter、run、task 均只保留一份。
