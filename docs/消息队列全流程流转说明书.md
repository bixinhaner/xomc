...
flowchart LR
    subgraph Producers["生产者"]
        ACS[ACS handler.go<br/>Inform / TransferComplete]
        APPSVC[APP 业务服务<br/>alarm · provision · software · backup · report · mml]
        TASKSVC[task.TaskService<br/>MarkTaskCompleted/Failed]
    end

    subgraph Brokers["消息基础设施"]
        NATS[(NATS JetStream<br/>14 Streams / 72h 持久化)]
        REDIS[(Redis<br/>acs:taskq:* · reboot:abnormal:* · redisx.Keys)]
        HUB[(SSE MessageHub<br/>进程内 channel + 可选持久化)]
    end

    subgraph Consumers["消费者（Queue Groups）"]
        DEV[device-mgr-* · 设备管理]
        ALARM[alarm-receivers · alarm-reboot-monitor]
        PM[pm-workers · mr-workers]
        PROV[provisioning · provision-gpv · provision-gpn · provision-model-upload]
        TASK[task-completion-bridge · task-reboot-closer]
        OTHERS[backup-executors · report-generators · transfer-receivers]
        OSS[northbound/push Engine]
    end

    Producers -->|Publish| NATS
    Producers -->|CreateTask / ZADD| REDIS
    Producers -->|Publish userID| HUB

    NATS -->|QueueSubscribe Durable Ack| Consumers
    REDIS -->|PopTask| ACS
    HUB -->|SSE /events/stream| Browser[(浏览器)]
```
...
```mermaid
flowchart TB
    subgraph ACS_PROC["omcgo-acs 进程"]
        ACSH[handler.go · Inform/响应路由]
        DISP[rpc.Dispatcher · 12 Handler]
    end

    subgraph APP_PROC["omcgo-app 进程"]
        APISRV[Gin REST API :8080]
        ENGINES[provision · alarm · software · backup · report · mml]
        BRIDGE[CompletionEventBridge]
        HUBH[events/hub.go]
    end

    subgraph WORKER_PROC["omcgo-worker 进程"]
        COLLECT[pm · mr · transfer Collectors]
        RESTORE[RestorePendingQueues]
        REBOOT[RebootCloser · RebootMonitor]
    end

    NATS[(NATS JetStream)]
    REDIS[(Redis)]
    PG[(PostgreSQL<br/>device_tasks · mml_tasks ...)]
    MINIO[(MinIO)]

    ACS_PROC <-->|Pub/Sub task.* · device.* · command.*| NATS
    APP_PROC <-->|Pub/Sub| NATS
    WORKER_PROC <-->|Pub/Sub| NATS

    ACS_PROC <-->|TaskQ ZADD/POP| REDIS
    APP_PROC <-->|TaskQ CreateTask| REDIS
    WORKER_PROC <-->|RestorePending · StaleRecover| REDIS
...
```mermaid
flowchart LR
    CPE1[CPE Inform] --> ACS1[ACS handler.go]
    ACS1 -->|Publish device.*| STREAM_DEV[(DEVICE Stream)]
    STREAM_DEV --> DEVH[device-mgr-* · alarm-receivers · transfer-receivers]
    DEVH --> PG[(PostgreSQL)]
    DEVH -->|触发| APPSVC[APP 业务服务]

    APPSVC -->|CreateTask| TASKQ[(Redis TaskQ)]
    APPSVC -->|Publish| STREAM_BIZ[(PROVISION/SOFTWARE/BACKUP ...)]

    TASKQ --> ACS2[ACS Pop]
    ACS2 -->|SOAP RPC| CPE2[CPE]
    CPE2 -->|SOAP Response| ACS3[ACS handler.go]
    ACS3 -->|MarkTaskCompleted| TASKSVC[task.TaskService]
    TASKSVC -->|Publish task.completed| STREAM_TASK[(TASK Stream)]
    STREAM_TASK --> BRIDGE[task-completion-bridge]
    BRIDGE --> AGG[mml.ResultAggregator]
    AGG --> SSE[SSE Hub] --> Browser

    ACS3 -->|Publish command.*.response| STREAM_CMD[(COMMAND Stream)]
    STREAM_CMD --> APPSVC
```
...
flowchart TB
    ...
    ACSH -->|0 BOOTSTRAP| S1[device.inform.bootstrap]
    ACSH -->|2 PERIODIC| S2[device.inform.periodic]
    ACSH -->|4 VALUE_CHG| S3[device.inform.value_change]
    ACSH -->|1 BOOT / M Reboot| S4[device.inform.reboot_complete]
    ACSH -->|M AlarmInfo| S5[device.inform.alarm]
    ACSH -->|AutonomousTransferComplete| S6[device.inform.autonomous_transfer_complete]
    S1 --> QBOOT[device-mgr-bootstrap]
    S2 --> QPERIOD[device-mgr-periodic]
    S3 --> QVC[device-mgr-valuechange]
    S4 --> QREBOOT[device-mgr-reboot]
    S4 --> QCLOSER[task-reboot-closer]
    S5 --> QALM[alarm-receivers]
    S6 --> QTRANS[transfer-receivers]
```
...
flowchart TB
    ...
    ACS -->|Publish| SUB[device.inform.autonomous_transfer_complete]
    SUB --> TR[transfer-receivers · Worker]
    TR -->|FileType=2| PM[pm.file.received]
    TR -->|FileType=5| MR[mr.file.received]
    TR -->|FileType=11| DM[datamodel.file.received]
    PM --> PMW[pm-workers] -->|解析 XML → TimescaleDB| TSDB1[(timescaledb)]
    MR --> MRW[mr-workers] -->|解析 XML → TimescaleDB| TSDB2[(timescaledb)]
    DM --> PMU[provision-model-upload] -->|写 data_model_definitions| PG1[(postgres)]
```
...
```mermaid
flowchart LR
    A[device.registered] --> B[provisioning 队列组]
    B -->|选模板 + 序列化| C[TaskService.CreateTask × N]
    C --> D[(Redis TaskQ)]
    D --> E[ACS Pop → SOAP RPC → CPE]
    E --> F[CPE Response → ACS]
    F --> G[Publish command.get_parameters.response]
    G --> H[provision-gpv]
    H -->|驱动下一步| C
    H -->|全部完成| I[Publish oss.config.snapshot]
...
- 发布过滤：仅 `source=mml` 且带 `source_id` 才发 `task.*`，普通 API 任务不占广播。
...
flowchart TB
    ...
    ACS -->|Publish device.inform.reboot_complete| S1((Stream))
    S1 --> QA[device-mgr-reboot]
    S1 --> QB[task-reboot-closer]
    S1 --> QC[northbound/push.Engine]

    QA -->|UpdateFromInform + RecordBootFromInform| DEVS[device_service]
    DEVS -->|纯 1 BOOT| S2[Publish device.reboot.abnormal]
    QB -->|含 M Reboot| CLOSE[扫 PG 关闭 Reboot/FactoryReset]

    S2 --> MON[alarm-reboot-monitor]
    MON -->|ZADD + ZREMRANGEBYSCORE + ZCARD + EXPIRE| RDS[(reboot:abnormal:{sn})]
...
```mermaid
flowchart LR
    ADMIN[管理员: 改 RBAC] -->|写 PG casbin_rule| ENF[Enforcer.LoadPolicy 本地]
    ENF --> NOTIFY[CasbinAuthorizer.NotifyPolicyChange]
    NOTIFY --> WATCH[natsCasbinWatcher.Update]
    WATCH -->|bus.Publish sys.casbin.policy.reload| NATS[(SYS Stream)]
    NATS -->|Subscribe 广播 非QueueGroup| R1[APP 副本 1]
    NATS -->|Subscribe 广播| R2[APP 副本 2]
    NATS -.未来.-> R3[ACS / Worker]
    R1 --> L1[Enforcer.LoadPolicy 全量]
    R2 --> L2[Enforcer.LoadPolicy 全量]
```
...
```mermaid
flowchart LR
    PUB1[mml/service.go<br/>MML 执行状态] --> HUB
    PUB2[mml/result_aggregator.go<br/>任务完成统计] --> HUB
    PUB3[notification/service.go<br/>系统通知] --> HUB
    HUB[events/hub.go<br/>per-user channel] --> HDL[events/handler.go]
    HDL -->|GET /api/v1/events/stream| BR[Browser EventSource]
    HUB -.离线缓存.-> PEND[(Redis sse:pending:{user})]
```
