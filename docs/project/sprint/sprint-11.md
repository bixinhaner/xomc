...
| 8 | ~~T-0125（接力 T-0123）~~ ✅ done 2026-05-14 | F09 参数同步触发链 §3 — firmware.changed 事件 + Redis 串行锁 + 重新交集（commit `b4790172` 7 文件 +567/-12 含 7 单测；同晚 pull-forward 进 sprint-10 buffer 接力 T-0123） | S (~0.5-1d) | 2026-05-14 user 拍板按顺序继续；deps T-0123 ✅ | **中** — 升级后 discovered_param_mappings 自动重新交集避免 Translator 降级；reason 通道复用 T-0127 协议；接力 T-0124（WithReason "periodic"）+ T-0126（WithReason "manual"）基础设施完整 |
...
| 10 | ~~T-0126（收官）~~ ✅ done 2026-05-14 🎉 | F09 参数同步触发链 §4 — 手动同步端点 + 前端按钮（commit `08754f89` 15 文件 +594/-161 含 7 handler 测；同晚 pull-forward 进 sprint-10 buffer 收官 F09 触发链 5/5）| S (~0.5-1d) | 2026-05-14 user 拍板按顺序收官；deps 无（共享既有 Path B + ParamSyncStarter narrow interface 避免循环依赖） | **中** — 运维一键全量对账补 PullConfig 缺口；**🎉 F09 参数同步触发链 5/5 全收官**（T-0123 device.online + T-0124 periodic + T-0125 firmware.changed + **T-0126 manual** + T-0127 diff log）|
