...
| **B′ T-0102 umbrella MVP 残债清账**（与 P1 同日 ULTRATHINK 发现 sprint-11 §2 #6 "T-0102-a planned" 描述过时 → 实际已 done 2026-05-12 → 转向收尾 2 项 MVP-out-of-scope 残债）：① GetRPCMethods ACS handler 实现 + 注册 (3 文件 / +20 行 / 1 单测) — action=get_rpc_methods 真链路打通；② ops.ResultAggregator completion ACS→ops 桥 (1 新 + 1 新测 / +400 行 / 12 单测) — 闭合入队侧 publishDispatchEvent → publishCompletionEvent SSE 通路；③ sprint-11 §2 #6 状态校正 planned → done | sprint-11 §2 #6 状态修正 + T-0102 umbrella 真 5/5 闭环（含 MVP 残债）+ 13 新单测 race 全 PASS；R-O02 mitigation 进一步深化（completion 终态可见）|
...
| **6** | **T-0102-a** | **MML 占位项（实际 scope 已重排）**：本行 sprint-planning 期原描述"`/ops/commands/rpc` 实 RPC 派发到 acs（替换 stub 占位）"实际由 T-0102-c (commit `9a95e138` 2026-05-12) 完成；T-0102-a 实际交付的是 SSEHub + Stats observability (commit `1de0a16d` 2026-05-12)；T-0102-a..e 全 5 sub-task done。**MVP 残债 (GetRPCMethods ACS handler + completion ACS→ops 桥) 由 B′ 收尾 2026-05-14**。 | Claude | M (~2d) | F06 ops P-1 通路 / unlock GWT V1 | P1 | **done 2026-05-14（pull-forward sprint-10 buffer，T-0102 umbrella 5/5 全闭 + B′ MVP 残债清账；T-0102-a `1de0a16d` SSEHub + T-0102-b `dd786a22` 真入队 + T-0102-c `9a95e138` RPC 派发 + T-0102-d `3f802ef7` per-device frame + T-0102-e `429178fb` per-device 限流/超时/重试；B′ 残债 1 GetRPCMethods handler 注册 + 残债 2 ops.ResultAggregator completion 桥；详 `backlog/done/2026Q2.md`）** |
...
| 7 | ~~T-0128 + T-0127（合并，原 T-0123 因 MML 撞号已重命名）~~ ✅ done 2026-05-14 | F09 参数同步触发链 §1+§5 — device.online 事件 + firmware 二选一挡板 + Provision 订阅 + Redis token bucket + Path B 差异日志（commit `d1eea063` 14 文件 +1643/-11 含 17 单测；同晚 pull-forward 进 sprint-10 buffer 而非 sprint-11 主线）| S (~0.5-1d) | 2026-05-14 user 拍板 ad-hoc 加入 stretch；T-0125 deps 本任务（T-0128）✅（共享 UpdateFromInform 改造；本任务原编号 T-0123 因与 MML 老交互撞号已重命名 T-0128） | **中** — 已给 T-0125（firmware 重新交集 / 共享 oldVersion 比对挂回挡板处）+ T-0124（周期同步 / WithReason("periodic")）+ T-0126（手动端点 / WithReason("manual")）提供完整基础设施 |
| 8 | ~~T-0125（接力 T-0128，原 T-0123 重命名）~~ ✅ done 2026-05-14 | F09 参数同步触发链 §3 — firmware.changed 事件 + Redis 串行锁 + 重新交集（commit `b4790172` 7 文件 +567/-12 含 7 单测；同晚 pull-forward 进 sprint-10 buffer 接力 T-0123） | S (~0.5-1d) | 2026-05-14 user 拍板按顺序继续；deps T-0128 ✅ | **中** — 升级后 discovered_param_mappings 自动重新交集避免 Translator 降级；reason 通道复用 T-0127 协议；接力 T-0124（WithReason "periodic"）+ T-0126（WithReason "manual"）基础设施完整 |
| 9 | ~~T-0124（最大工作量）~~ ✅ done 2026-05-14 | F09 参数同步触发链 §2 — PeriodicSyncer + PG advisory lock leader + 回写口径统一（commit `9f071c33` 26 文件 +1136/-6 含 14 单测 + 12 mock 桩补齐；同晚 pull-forward 进 sprint-10 buffer 收官） | L (~1.5d) | 2026-05-14 user 拍板按顺序继续；T-G OfflineDetector 迁移 LeaderElector 抽象 post-RC | **中** — 兜底"配置漂移检测"（10 万级 batch=200 interval=24h 每天扫一轮）；F09 触发链 4 task done + T-0126 仅需新增端点共享既有基础设施 |
| 10 | ~~T-0126（收官）~~ ✅ done 2026-05-14 🎉 | F09 参数同步触发链 §4 — 手动同步端点 + 前端按钮（commit `08754f89` 15 文件 +594/-161 含 7 handler 测；同晚 pull-forward 进 sprint-10 buffer 收官 F09 触发链 5/5）| S (~0.5-1d) | 2026-05-14 user 拍板按顺序收官；deps 无（共享既有 Path B + ParamSyncStarter narrow interface 避免循环依赖） | **中** — 运维一键全量对账补 PullConfig 缺口；**🎉 F09 参数同步触发链 5/5 全收官**（T-0128 device.online + T-0124 periodic + T-0125 firmware.changed + **T-0126 manual** + T-0127 diff log）|
