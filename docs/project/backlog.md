...
|----|-------|-------------|---------|-------|
| T-0101 | F06 运维管理 — TaskExecutor 任务调度引擎 | dev (Claude) | 2026-05-10 | **背景**：PRD `docs/project/prd/F06-ops-management.md` §5.3 + §12 P0；当前 `ops_tasks.CreateTask` 写库后空转无执行。**目标**：实现 TaskExecutor 引擎，含步骤路由（rpc/mml dispatcher）+ 并发控制 + 节流 + 状态机集成 + 结果聚合（ops_task_executions）+ 失败策略 + 暂停/恢复/取消 + rollback + 断点续传。**子任务**：9 条 T-0101-a..i，见 `backlog/subtasks/T-0101-ops-management.md`。**Type**=feat / **Domain**=F06/ops / **Prio**=P0 / **Est**=L (~5 天) / **Deps**=T-0112 (Foundation), Q2 决议（调度引擎放哪）/ **Risk**=R-O01。**实施计划**：`docs/project/F06-ops-management-implementation-plan.md` §2.2 + §4 Sprint 2 全量。 |
| T-0102 | F06 运维管理 — 即时命令 SSE 通道 | dev (Claude) | 2026-05-10 | **背景**：PRD §5.2 + §12 P0；当前 CommandManagement 仅历史记录，无即时下发。**目标**：POST /api/v1/ops/commands/rpc → 入队 → SSE 流式回执（含 RPC reboot/factory_reset/GPV/SPV + MML 多帧 + 超时重试 + per-device 限流）。**子任务**：5 条 T-0102-a..e。**Type**=feat / **Domain**=F06/ops+acs+frontend / **Prio**=P0 / **Est**=M (~3 天) / **Deps**=T-0101 / **Risk**=R-O02（SSE 在 nginx/k8s ingress 下连接保持）。**实施计划**：§2.2 + Sprint 3。 |
| T-0103 | F06 运维管理 — 前端 W1 MVP 补齐 | dev (Claude) | 2026-05-10 | **背景**：commit 6e42a664 已完成 50%（菜单+权限+3 真+2 占位），剩余 Edit modal / 模板导入导出 / 风险等级 UI / Tasks 设备级流水 / SSE 实时进度 / CommandManagement 新建即时命令 UI。**子任务**：6 条 T-0103-a..f。**Type**=feat / **Domain**=frontend+frontend-core / **Prio**=P0 / **Est**=M (~2 天) / **Deps**=T-0101 + T-0102 / **Risk**=—。**实施计划**：§2.2 + Sprint 3。 |
| T-0104 | F06 运维管理 — 网络诊断子系统（TR-181 Diagnostics） | dev (Claude) | 2026-05-10 | **背景**：PRD §5.4 + §12 P1；当前 NetworkDiagnosis 仅路线图占位卡。**目标**：接入 TR-181 IPPing/TraceRoute/Download/Upload/UDPEcho 五对象 + OMC 侧诊断（reachability/同站/GPS/邻区/告警风暴）+ 厂商扩展 DiagnosticsAdapter + 时序图可视化。**子任务**：9 条 T-0104-a..i。**Type**=feat / **Domain**=F06/ops+acs / **Prio**=P1 / **Est**=L (~5 天) / **Deps**=T-0112, Q4 决议 / **Risk**=R-O04（厂商兼容性）。**实施计划**：§2.3 + Sprint 4-5。 |
| T-0105 | F06 运维管理 — 运维下载子系统（按需采集 + 诊断包） | dev (Claude) | 2026-05-10 | **背景**：PRD §5.5 + §12 P1；当前 Downloads 仅路线图占位卡。**目标**：7 类内容采集（config/log/pm/mr/诊断包/pcap/gps）+ 打 zip 包 + presigned URL + 过期清理 + 断点续传。**子任务**：7 条 T-0105-a..g。**Type**=feat / **Domain**=F06/ops+transfer / **Prio**=P1 / **Est**=L (~5 天) / **Deps**=T-0112 / **Risk**=—。**实施计划**：§2.3 + Sprint 5。 |
| T-0106 | F06 运维管理 — 4 眼审批流（risk_level + ApprovalService） | dev (Claude) | 2026-05-10 | **背景**：PRD §5.2 + §8.2 + §12 P1；高风险动作（reboot/factory_reset/批量>50台）必须双人审批；合规阻塞 GA。**目标**：risk_level 自动评估 + ApprovalService（approver ≠ creator）+ 通知集成 + 前端审批页 + Modal。**子任务**：5 条 T-0106-a..e。**Type**=feat / **Domain**=F06/ops+admin+frontend / **Prio**=P1 / **Est**=L (~4 天) / **Deps**=T-0112, Q3 决议（首版必须） / **Risk**=R-O05（状态机契约改动）。**实施计划**：§2.4 + Sprint 6。 |
| T-0107 | F06 运维管理 — 维护窗口（告警抑制 + 暂停开站） | dev (Claude) | 2026-05-10 | **背景**：PRD §6.2 + §12 P2；现场维护期间需抑制误报告警 + 暂停 provision + 允许 dangerous 旁路。**目标**：ops_maintenance_windows 表 + EventBus 事件（maintenance.window.started/ended）+ alarm 模块订阅抑制 + provision 模块订阅暂停 + 前端 UI。**子任务**：5 条 T-0107-a..e。**Type**=feat / **Domain**=F06/ops+alarm+provision+frontend / **Prio**=P2 / **Est**=M (~3 天) / **Deps**=T-0112 / **Risk**=R-O06（跨域订阅）。**实施计划**：§2.4 + Sprint 6。 |
| T-0108 | F06 运维管理 — 健康巡检（定时诊断 + 报告） | dev (Claude) | 2026-05-10 | **背景**：PRD §6.1 + §12 P2；每周自动跑诊断模板 → 汇总报告邮件给运维。**目标**：cron 调度 + 报告生成（PDF/CSV，存 MinIO）+ 邮件/SMS 通知 + 前端配置。**子任务**：4 条 T-0108-a..d。**Type**=feat / **Domain**=F06/ops+notification / **Prio**=P2 / **Est**=M (~3 天) / **Deps**=T-0104 / **Risk**=—。**实施计划**：§2.5 + Sprint 7。 |
| T-0109 | F06 运维管理 — 审计归档（ops_audit_logs + 12 月保留） | dev (Claude) | 2026-05-10 | **背景**：PRD §6.3 + §12 P2；等保 2.0 三级 8.1.4.7 要求重要操作日志保留 ≥ 6 个月，运维场景升至 12 个月。**目标**：独立 ops_audit_logs 表（区别于通用 audit_logs，高频写）+ 全量 hook（template_run/command_run/task_create/cancel/diagnostic/download/approval）+ 月度归档 cron + 前端查询页。**子任务**：4 条 T-0109-a..d。**Type**=feat / **Domain**=F06/ops+admin / **Prio**=P2 / **Est**=M (~2 天) / **Deps**=T-0112 / **Risk**=—（合规阻塞 GA）。**实施计划**：§2.4 + Sprint 6（建议先做）。 |
| T-0110 | F06 运维管理 — 知识库 Playbook（告警→模板推荐） | dev (Claude) | 2026-05-10 | **背景**：PRD §6.5 + §12 P3；把"告警 → 处置模板"映射沉淀，告警详情页一键应用推荐模板。**目标**：ops_playbooks 表 + alarm_pattern → recommended_templates 匹配引擎 + 告警详情页推荐区块 + 知识库管理 UI。**子任务**：4 条 T-0110-a..d。**Type**=feat / **Domain**=F06/ops+alarm+frontend / **Prio**=P3 / **Est**=M (~2 天) / **Deps**=T-0101, T-0112 / **Risk**=—。**实施计划**：§2.5 + Sprint 7。 |
| T-0111 | F06 运维管理 — 紧急响应 break-glass（临时高权限 + 录像） | dev (Claude) | 2026-05-10 | **背景**：PRD §6.6 + §12 P3；故障紧急情况绕过审批的临时通道（含工单号 + 30 分钟自动失效 + 全程录像 + 24h 复盘）。**目标**：临时权限激活流 + ops_audit_logs.break_glass=true 录像 + 强制告警通知 + 复盘评注阻塞下次激活。**子任务**：4 条 T-0111-a..d。**Type**=feat / **Domain**=F06/ops+admin+notification / **Prio**=P3 / **Est**=M (~3 天) / **Deps**=T-0109 / **Risk**=R-O07（滥用）。**实施计划**：§2.6 + Sprint 7-8。 |
| T-0112 | F06 运维管理 — Foundation 基础设施（schema + seed + Adapter） | dev (Claude) | 2026-05-10 | **背景**：所有 T-0101..T-0111 依赖的前置 — 6 张新表 + 2 张扩展列 + 8 个权限点 + Carrier.OpsAdapter 接口 + 6 个内置 OEM 模板（注：PRD §12 原 T-0112 为远程控制台 V2 暂搁，slot 回收用于 Foundation）。**子任务**：4 条 T-0112-a..d（schema 扩展 / 新表 / 权限点 seed / 内置模板 seed + Adapter 骨架）。**Type**=feat+migration / **Domain**=F06/ops+infra / **Prio**=P0（解锁全部）/ **Est**=L (~5 天) / **Deps**=Q1-Q6 决议 / **Risk**=—。**实施计划**：§2.1 + Sprint 1。 |
| T-0100-P5 | license 治理层 review 残项清理（umbrella T-0100 收尾 sub-task） | dev (Claude) | 2026-05-09 | **背景**：review 报告 `docs/review-report/20260509/REVIEW_922d87a4_chenbo01_license.md` 在 P4-B/C commit 上点出 1 CRITICAL + 5 WARNING + 5 INFO + 4 配套缺口；CRITICAL #1 + WARNING #3 已分别由 P4-B1（commit d0cc4a0b）+ P4-B2（commit 76a9e983）关闭；剩余项需打包清扫。**WARNING 残项**（4 项）：① archiver doc/code 一致性（P4-B1 已部分修，doc 块已重写但接口 archiveObjectIO.StatObject 与 fakeMinIO 旧 stub 残留可清；本期 P4-B2 已删，状态需复核） ② handler.go 9100-9109 错误码段未注册到 `global/errors.go` license 域 12000-12999 段（pre-existing tech debt，P4-C 沿袭）→ 迁移到 12100-12109 + 注册 ④ DI strict + 0 keys 启动静默：`modules.go` LoadKeysFromDir 失败仅 warn，建议 strict=true && KeyCount()==0 时 Fatal + RecordHealthCheck("license-signing", err) 让 /healthz 反映 ⑤ archiver ctx cancellation 不敏感：循环开头加 `if err := ctx.Err(); err != nil { return nil, err }` 早返。**INFO 残项**（5 项）：① canonicalize 不递归 — license JSON 加嵌套字段时 verify 随机失败的 future risk；可加 doc-level fail-safe 或递归实现 ② P3 stub `VerifySignature(nil)` fallback 可 inline + Deprecated 注释 ③ 大对象监控：metric `omc_license_archive_bytes_per_month` ④ Monitor.SetArchiver 顺序 guard：cron 已起后再 SetArchiver 应 panic / return error ⑤ 测试 PEM 文件 mode 0o600 可降为 0o644。**配套缺口**（4 项）：① 前端 LicenseOperations Tab 1 Import 接文件读取 → 提取 signature → POST signed_license_json（当前手填表单，PRD §5.3.1 要求"上传 license 文件 → 后端解析+验签+入库"）+ strict 拒绝 9109 专属错误展示 ② `omcgo/cmd/app/etc/config.{dev,prod}.yaml` 同步加 `license:` section（signing.public_key_dir + log_archive.retention_months 默认 6）③ `omcgo/scripts/e2e_verify.sh` 补 license 导出（PDF magic header + CSV BOM）+ signed import（用 fixture key 签 license）用例 ④ `docs/project/prd/F06-license.md` §5.3.1/§5.4.5 措辞从"待 P4 实现"改为"P4-B/C 已实现"+ 附实现链接。**Triage 建议**：Type=fix+docs+chore 混合；Prio=P2（非阻塞 GA-prep，P4-B/C review CRITICAL/HIGH 已修，残项偏 polish + 合规建账）；Domain=F06/license+admin+frontend+ops；Est=L（粗估 2-3 天，含拆分执行）；Deps=T-0100-P4-B2 ✅；Risks=R-NEW（错误码野生段 + DI 静默失败 GA 前发现的运维事故风险）。**优先建议**：S2 阶段拆 4 段 a/b/c/d（a=后端代码项 4 WARNING + 4 INFO；b=配套 yaml/PRD/e2e；c=前端文件上传消费 SignedLicenseJSON + 9109 错误展示；d=错误码迁移 12100-12109）便于并行 review；当前可直接 §A6 triage 升 §4 Triaged 或留 §5 等下次 sprint-planning。 |
