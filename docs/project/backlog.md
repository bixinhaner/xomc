...
|----|-------|--------|--------|----------|
| **T-0102** | 即时命令 SSE 通道 umbrella — **5/5 全闭** | F06/ops+acs+mml | 2026-05-12 | 5 sub-task 全 done 单晚 pull-forward 进 sprint-10 buffer：a `1de0a16d` SSEHub Stats + 5 单测 / b `dd786a22` ExecuteRPC 真入队 + 7 单测 / c `9a95e138` actionToRPCMethod 5 动作真派发 + 18 单测 / d `3f802ef7` MML per-device frame SSE + 7 单测 / e `429178fb` per-device 限流 + retry + 60s 超时 + 8 单测；R-O02 mitigation 完整闭环；40+ 反退化单测入测；POST /ops/commands/rpc 端到端真链路打通 + MML fan-out 实时 frame + 三件套防灾配套 |
