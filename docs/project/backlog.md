...
| 指标 | 当前 | 目标 | 备注 |
|------|------|------|------|
| Total tasks | 159 | — | 含 T-0098 umbrella + 36 sub-task；2026-05-10 新增 12 umbrella（T-0101..T-0112 F06 运维管理）+ 66 sub-task；2026-05-11 +T-0113 F06 ops 合规债清账 + T-0114/T-0115/T-0116 F10 Phase 3 三 task（fault-inject / 报告 export / 设备矩阵）；2026-05-13 test.md §5 续测登 T-0119/T-0120/T-0121/T-0122（§5.2/5.3/5.5/§5.6-unknown 环境不可达 Proposed）；2026-05-14 +T-0123 MML 老交互恢复 umbrella + 5 sub-task（P0..P4，PRD v2 APPROVED 自我 triage 用户授权）；变更明细见 changelog |
| `done` | 160 | — | 详细 Closing Evidence → `backlog/done/2026Q2.md`（含 T-0095/T-0098/T-0101..T-0112/T-0099/T-0097/T-0030 + T-0115/T-0114/T-0116 + T-0113 + T-0090 全系列 + T-0096 + T-0102 umbrella 5/5 + T-0101 umbrella + T-0093 + T-0117 + **+T-0118 运维 TLS 部署 — 自签证书 + nginx 443 ssl 模板 + 5 步启用文档 2026-05-12（T-0117 长期方案兑现：opt-in 不破坏 :8081 plain HTTP 入口）**）|
| `in_dev` | 0 | — | T-0098 + T-0090 均收官 |
| `in_design` | 0 | — | T-0090 umbrella 4/4 sub-task 全闭，State in_design → done |
| `planned` | 7 | — | T-0009/T-0014/T-0017/T-0020/T-0023 (wave + 短信凭据外部阻塞) + T-0102-a (sprint-11 主线) + **T-0123-P0** (sprint-11 主线，dev-pipeline pick 2026-05-14 S1 入选) |
| `triaged` | 7 | — | T-0091 / T-0035 (XL 待 S2 拆) + T-0123 umbrella + T-0123-P1..P4 4 子任务（T-0123-P0 已升 planned 进 sprint-11 by /dev-pipeline pick 2026-05-14）|
| `blocked` | 0 | ≤ 3 | — |
| `proposed` 积压天数 | 0 | ≤ 7 | — |
...
| ID | Title | Type | Domain | Prio | State | Owner | Est | Deps | Risk/PRD | Sprint | Updated |
|----|-------|------|--------|------|-------|-------|-----|------|----------|--------|---------|
| T-0123-P0 | 数据层：migration 000095 (mml_command_sub_fields 表 + mml_params/commands/groups 加 access_type / is_object / supports_add / supports_delete / change_applies / constraint_text_i18n / catalog_protected / logical_code / logical_name_i18n / source 等元数据列) + `omcctl mml import-standard-xml` 工具 + seed/000NNN_mml_standard_import.sql 一次性导入 standard-model.xml ~2001 条 + 启动期 mmlstandardloader 下线 + admin CRUD API 骨架 (groups/commands/sub-fields/params 13 端点) | feat | F06/mml+infra | P1 | planned | Claude | M (~3d) | T-0119 ✅ | PRD §4-5 / §6.2 / §8.1 / R-206 Open | sprint-11 | 2026-05-14 |
| T-0123-P1 | Console 后端：5 API 端点（GET `/mml/groups/tree` / GET `/mml/commands/:id/sub-fields` / POST `/mml/render` / POST `/mml/parse` / POST `/mml/execute`）+ Go 侧 MML renderer/parser (`internal/mml/mml_renderer.go`) + Execute 支持 statements 数组 (N 设备 × M statements fanout) + sub-field 触发器维护 mml_commands.target_paths 派生缓存 | feat | F06/mml | P1 | triaged | — | M (~3d) | T-0123-P0 | PRD §6.1 / §6.3 / §8.1 | sprint-11 候选 | 2026-05-14 |
| T-0123-P2 | Console 前端：三栏布局 + Step Bar 1-2-3-4 还原 + MmlEditor (顶部 textbox + DO + 防抖 300ms parse) + SubFieldChecklist (LST 勾选 + 默认全勾) + SubFieldInputList (MOD 输入 + constraintText + required + OnReboot badge) + 双向绑定（UI ⇄ mmlText）+ mmlConsoleStore Zustand + Control Panel ⇄ ParameterPath Command 双 tab 切换保留 + 删 ParamPathPanel/ParamFormRenderer + 业务层全进 frontend-core (API/Hook/Store/Types/i18n) | feat | frontend+F06/mml | P1 | triaged | — | L (~4d) | T-0123-P1 | PRD §7.1-7.4 / §7.5 多皮肤兼容 | sprint-11 候选 | 2026-05-14 |
| T-0123-P3 | admin Catalog 管理 UI：`/mml/admin/catalog` 独立路由 + 4 Tab（Groups 树形增删改+拖拽 / Commands 列表+sub-field 绑定面板 / Params 字典浏览+搜索+"被哪些命令引用"反向查 / XML 导入预览） + 元数据可视化（access_type / is_object / supports_add / supports_delete / change_applies + catalog_protected 锁定列） + 多语言行内编辑（zh-CN / en-US，"🟡 需翻译" badge）+ RBAC `mml.catalog.manage` 单一权限点 + 写表后 `parammodel:cache_version` 失效跨实例缓存 | feat | frontend+admin+F06/mml | P1 | triaged | — | L (~4d) | T-0123-P0, T-0123-P1 | PRD §7.6 / §12 决策 3 / §6.2 | sprint-12 候选 | 2026-05-14 |
| T-0123-P4 | 收尾：ADD/RMV 命令体（InstancePicker — 先 GPV 探实例再选 index 删）+ Customized PrivateTemplate/PublicTemplate 集成（既有 `mml_custom_commands` 渲染进树 + 双击加载到右栏 + adapter 把旧 parameters JSONB 翻新成 statements）+ 元数据驱动 UI 差异化全量铺开（READ_ONLY 灰 / OnReboot 二次确认 / supports_delete 🗑 实例行 / catalog_protected admin "删除"禁用）+ DoD 验收清单（PRD §11）+ E2E +14 断言（226→240）+ webcode-v2/v3 typecheck PASS | feat | F06/mml+frontend | P2 | triaged | — | L (~4d) | T-0123-P2, T-0123-P3 | PRD §11 DoD + §7.3 元数据差异化表 | sprint-12 候选 | 2026-05-14 |
