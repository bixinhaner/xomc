...
|----|-------|-------------|---------|-------|
| T-0103 | D1 path-b 真性 9005 反馈式 unsupported 标记 + 跳过下发 | Claude+user | 2026-05-13 | D1 path-b 去叶子截断（commit `692d2a44`）落地后，BAICELLS `BaiBLQ_5.0.16.1_1229` 仍有部分字典叶子不支持 → §5.4 整批 9005 reject。已知 sample：`Device.DeviceInfo.AmbrLimitSwitch`（一批 5 个 invalid）/ `RunningStatus`（1 个）/ `X_COM_SCTP_CONFIG_MTU`（1 个）。**机制（反馈式学习，不主动探测）**：① ACS 收到 9005 Fault 时从 SOAP `<faultdetail>` / `Invalid Parameter Names` 提取完整 invalid path list（不只看 fault_msg sample）；② 把这些 path 在 `discovered_param_mappings` 标 `is_supported=false`（或等价机制）；③ path-b sync `extractStorablePrefixes` 跳过 `is_supported=false` 条目，下次 sync 不再下发；④ 同 SN 多次 sync 后 prefixes 数自然收敛到 CPE 实际支持集。本次 D1 改动已 `prefixes 410 / 754 mappings / 2 batch completed / 240 行 device_parameters` 验证主体效果 |
