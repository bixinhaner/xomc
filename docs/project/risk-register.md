...
- **影响**：运维场景不可用
- **Owner**：前端专家
- **状态**：⚠️ Partially Closed (2026-04-29) — Software/Topology/Report 全闭环 + Backup 子模块 3/4 闭环（Tasks ✅ + FTP ✅ + Schedule ✅）；仅 Backup Policy/Restore 留 2 followup
- **关联 Task**：T-0019 ✅（Software Canary 消费）/ T-0022 ✅（Topology/Report）/ T-0016 ✅（Backup Tasks + FTP）/ T-0070 ✅（BackupSchedule UI 重设计）；followups T-0071 (Policy 后端+前端) / T-0072 (Restore 流程设计)
- **关闭依据（已闭环部分）**：T-0019 commit `4ebdc91c` + T-0022 commit `c5e134c0` + T-0016 commit `c939ff12` + T-0070 commit `9f7ce74e`
- **未闭环部分**：BackupPolicy（后端无 `/backup/policies` endpoint，需先后端建 API → T-0071） + RestoreData（restore 流程是配置同步还是备份解压恢复尚未明确，跨架构+电信+前端 design → T-0072）
- **下次复盘**：T-0071/0072 完成时（预计 sprint-06+）
