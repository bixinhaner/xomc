...
- ✅ **G8-Gap-3**（曾标 ⚠️ partial）— 2026-05-25 终结 commit `300ae4a1`：HeartbeatInterval 从 `const` 改 `var` + `loadAsyncJobThresholds` 启动期注入；按用户决策（设计文档 §7.1 Decision Note 2026-05-25）放弃 EventBus 热重载，DoD G8.5 同步弱化为"改后重启 worker 生效"
