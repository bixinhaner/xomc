...
| T-0102-b | command-execute endpoint（POST → 入队 → 返 task_id） — **done 2026-05-12**（commit `dd786a22`；ExecuteRPC 改真入队 + TaskCreator 消费者驱动接口 + 两维风险评估 (action × deviceCount) + L3 必经审批 / L1-L2 fire-and-forget executor.Run + SSE command.enqueued + 7 unit test 含批量11/51设备升级 + 错误 + SSE 反退化）| feat | F06/ops | P0 | done | Claude | S | T-0101-*, T-0102-a | sprint-10 (pull-forward) | PRD §7.2 §4.2.2 | 2026-05-12 | /api/v1/ops/commands/rpc；可批量发起；inline command envelope JSON 写 OpsTask.Message 供 T-0102-c 反读派发 |
| T-0102-c | RPC 命令支持 — reboot/factory_reset/get_param/set_param/get_rpc_methods — **done 2026-05-12**（commit `5edf7269`；RPCInlineKind+Envelope typed const + actionToRPCMethod 5 动作 + alias 容错；TaskExecutor +enqueuer/sseHub optional setter 注入；Run() 检测 inline envelope → dispatchInlineRPC fan-out per device → task.Enqueuer.CreateTask Source=ops + 每设备 execution 行 success/fail 隔离 + SSE command.dispatched 事件；handler 上游 actionToRPCMethod 校验 400；TaskSourceOps 枚举；18 unit test）| feat | F06/ops+acs | P0 | done | Claude | M | T-0102-b ✅ | sprint-10 (pull-forward) | PRD §4.2.1 | 2026-05-12 | 风险等级 L1/L2/L3 自动评估；GetRPCMethods ACS handler 未注册 (future 3 行)；completion 侧 ACS→ops 桥未做 (future) |
| T-0102-d | MML 流式输出（多帧）通过 SSE 推送 — **done 2026-05-12**（commit `3f802ef7`；ResultAggregator.OnTaskCompleted 加 publishDeviceFrame 每 device_task 终态推 mml_device_frame SSE 事件 + 7 unit test；Q1=B 双通道边界守住 MML 走 events.MessageHub per-user，ops 走 ops.SSEHub per-channel）| feat | F06/ops+mml | P0 | done | Claude | S | T-0102-c ✅ | sprint-10 (pull-forward) | PRD §4.2.3 | 2026-05-12 | 设备返多帧时实时推；多帧语义即多设备 fan-out 的逐帧到达；TR-069 单 RPC 多响应超 spec 留 future |
| T-0102-e | 超时 + 重试 + per-device 限流 — **done 2026-05-12**（commit `429178fb`；ConcurrencyLimiter +per-device rate.Limiter sync.Map (默认 5/sec) + WaitForDevice / TaskExecutor +limiter setter + dispatchInlineRPC 加 ExpiresIn=60s + enqueueWithRetry 3 attempts 指数退避 2s/4s/5s + ctx cancel 短路 + per-device 失败隔离；8 unit test）| feat | F06/ops | P0 | done | Claude | S | T-0102-c ✅ | sprint-10 (pull-forward) | PRD §4.2.3 | 2026-05-12 | 单命令默认 60s 超时；批量场景设备级独立超时；**T-0102 umbrella 5/5 全闭** |
