...
| ID | Title | Type | Domain | Prio | State | Owner | Est | Deps | Sprint | Risk/PRD | Updated | Notes |
|----|-------|------|--------|------|-------|-------|-----|------|--------|----------|---------|-------|
| T-0101-a | TaskExecutor 接口设计（small interface）+ DI 注入 | feat | F06/ops | P0 | proposed | — | S | T-0112-a, Q2 | — | PRD §5.3.2 / R-O01 | 2026-05-10 | Run / Pause / Resume / Cancel / Rollback 5 方法；接口在 device service 侧定义（消费者驱动）|
| T-0101-b | 步骤路由器 — step.type → rpc/mml/wait/loop/branch dispatcher | feat | F06/ops+acs+mml | P0 | proposed | — | S | T-0101-a, T-0112-d | — | PRD §5.3.2 | 2026-05-10 | 复用 internal/task 队列；rpc 走 acs/rpc/，mml 走 internal/mml |
| T-0101-c | 并发控制（max=20）+ 节流（与 internal/task 队列对接） | feat | F06/ops+task | P0 | proposed | — | S | T-0101-a | — | PRD §5.3.2 | 2026-05-10 | 防 ACS 雪崩；nat-limit 控每秒新增 RPC |
| T-0101-d | 状态机集成（pending → approved → running → success/failed/...） | feat | F06/ops | P0 | proposed | — | S | T-0112-a, Q3 | — | PRD §5.3.1 | 2026-05-10 | approval_state 字段联动；Q3=A 时引入 pending_approval 节点 |
| T-0101-e | 结果聚合 + ops_task_executions 写入（device × step 明细） | feat | F06/ops | P0 | proposed | — | S | T-0112-b | — | PRD §7.2 | 2026-05-10 | 每 RPC 调用一行；含 status/started_at/completed_at/duration_ms/request/response/error |
| T-0101-f | 失败策略实现（abort / continue / retry-N / rollback） | feat | F06/ops | P0 | proposed | — | M | T-0101-e | — | PRD §5.3.2 | 2026-05-10 | failure_policy 字段；retry 含指数退避 |
| T-0101-g | 暂停/恢复/取消 dispatcher 集成 | feat | F06/ops | P0 | proposed | — | S | T-0101-d | — | PRD §5.3.1 | 2026-05-10 | 不打断已发出的 RPC，只阻止后续设备；状态机原子转换 |
| T-0101-h | 回滚引擎（执行 rollback_steps 反向序列） | feat | F06/ops+alarm | P0 | proposed | — | S | T-0101-f | — | PRD §4.1.1 / §5.3.2 | 2026-05-10 | rollback 失败发告警通知人工介入 |
| T-0101-i | 断点续传（进程重启 / 节点切换从 ops_task_executions 恢复） | feat | F06/ops | P0 | proposed | — | S | T-0101-e | — | PRD §5.3.2 | 2026-05-10 | 启动期扫描 status=running 的 task → 恢复 dispatcher |
...
| ID | Title | Type | Domain | Prio | State | Owner | Est | Deps | Sprint | Risk/PRD | Updated | Notes |
|----|-------|------|--------|------|-------|-------|-----|------|--------|----------|---------|-------|
| T-0102-a | SSE Hub + 路由（每用户连接，按 task_id 订阅） | feat | F06/ops | P0 | proposed | — | S | — | — | PRD §5.2.3 / R-O02 | 2026-05-10 | nginx / k8s ingress 长连接专项测试（buffer / timeout）|
