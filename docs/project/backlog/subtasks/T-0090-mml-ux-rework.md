...
|----|-------|------|--------|------|-------|-------|-----|------|--------|----------|---------|-------|
| T-0090-a | MML Console FE-only: 操作类型差异化（MOD→修改值入口）+ 删参数配置/所属分类/适用产品类型 UI section + 命令编码 input→textarea + textarea 自定义限制（不允许选既有命令）| feat | frontend | P2 | triaged | — | S (1-2d) | — | sprint-11 | R-NEW-3（组件抽象不足导致 d 复制粘贴）/ backlog T-0090 ①②③④ | 2026-05-11 | 改 webcode/src/pages/mml/Console + components/ScriptTaskDrawer + 约 5 文件；可复用 Drawer/Form/CommandInput 抽到 components/ 为 d 准备；纯 FE 低风险，typecheck PASS 即可 |
| T-0090-b | MML 后端 drop product_types 列 — migration 删 mml_custom_command.product_types + GIN 索引；扫净 36 处 Go 引用（model/repo/handler/test）+ FE form/field/state 同步删；productTypes 历史数据评估（SELECT 非空行 backfill 决策）| ref | F06/mml+migration+frontend | P2 | triaged | — | M (2-3d) | — | sprint-11 | R-NEW-1（破坏性 migration）/ backlog T-0090 ② | 2026-05-11 | DDL migrations/0000NN_drop_mml_product_types.sql；Up=DROP COLUMN+DROP INDEX，Down 段需评估能否回滚（历史数据已丢则不可逆，文档说明 Down 仅恢复 schema 不恢复数据）；先在 dev 实测 SELECT 非空行数 → 决定 backfill 策略 |
