...
|----|-------|------|------|--------|--------|
| **L-1** | P0 阻塞 | **菜单 seed migration 之前漏写** | `/ops/message-trace` 路由有但用户菜单没注入，PrivateRoute 动态菜单守卫直接重定向 /403 | M1 范围只写了表 migration，菜单 seed 没纳入 | ✅ 已补 `omcgo/migrations/seed/000115_seed_trace_menu.sql`（菜单 + admin/operator/viewer 三角色绑定），待随本次 commit 合入 |
