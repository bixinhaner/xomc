...
|--------|------|------------|----------|
| 总数 | 默认 | ✓ | 清除所有筛选 |
| 严重 | #E53935 | ✓ | severity=['critical'] |
| 主要 | #FB8C00 | ✓ | severity=['major'] |
| 次要 | #FDD835 | ✓ | severity=['minor'] |
| 警告 | #42A5F5 | ✓ | severity=['warning'] |
| 未确认 | #E53935 | ✓ | dealState=['0'] |
| 未读 | #722ED1 | ✓ | unread='1' |

**统计项样式**：
- 显示标签（12px 灰色）和数值（20px 粗体）
- 点击时有指针光标
- 激活状态背景色：#E6F4FF
- 圆角：6px
- 内边距：8px 16px
...
| 按钮 | 功能 | 说明 |
|------|------|------|
| 自动刷新 | 下拉菜单 | 开启/关闭自动刷新，可选间隔：15秒/30秒/1分钟/5分钟 |
| 导出 | 直接点击 | 直接导出全部告警（无下拉菜单） |

#### 搜索卡片筛选字段

| 字段名称 | 类型 | 说明 |
|----------|------|------|
| 搜索 | 输入框 | 告警标识/可能原因/网元定位 |
| 告警时间 | 日期范围 | 事件时间范围 |
| 告警级别 | 多选 | critical/major/minor/warning |
| 事件类型 | 下拉 | 通信/服务质量/处理失败/设备/环境 |
| 网元类型 | 下拉 | eNB/gNB/GSM |
| 读状态 | 下拉 | 全部/已读/未读 |
| 告警状态 | 下拉 | 全部/未确认未清除/已确认未清除 |
| 快捷时间 | 下拉 | 今日/昨日/本周/本月/最近7天/最近30天 |
...
| 次要 | #FDD835 | ✓ | severity=['minor'] |
| 警告 | #42A5F5 | ✓ | severity=['warning'] |
| 已清除 | #67D972 | ✓ | dealState=['2','3'] |
| 已确认 | #67D972 | ✓ | dealState=['1','3'] |
...
```typescript
// 统计卡片
<Card size="small" bordered style={{ marginBottom: 12 }} styles={{ body: { padding: '12px 16px' } }}>

// 搜索卡片
<Card size="small" bordered style={{ marginBottom: 12 }} styles={{ body: { padding: '12px 16px 0' } }}>
...
- 使用 `noDefaultStyle` 属性移除默认的 margin 和 padding
- 使用 `collapsedRows={1}` 默认折叠一行
