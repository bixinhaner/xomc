...
- 普通升级仅将现网历史默认值 10,000 定向迁移到 30,000；运维自定义值保持不变，
  不要求也不建议使用 `--overwrite-etc`。
...
通过条件为普通升级能安全迁移历史默认值、配置实际加载 30,000、准入拒绝为 0、队列和
发布积压可恢复、无资源饱和或新增业务错误。若仍出现 503，必须按来源区分准入上限、
设备限流、PM 背压和其他 HTTP 错误。
