...
保存任务时一次性查询设备、产品、频段和组关系；运行时不查询这些关系。自建任务由用户保存生成新版本；内置任务由内置任务协调器检测成员变化并自动生成新版本。两种方式都只让新版本从下一个完整窗口生效，避免窗口中途成员漂移。

### 6.2 十二个内置任务

升级和新装都必须幂等保证以下十二个内置任务存在：

| 维度 | LTE | NR | GSM |
|---|---|---|---|
| 全网 `network` | 内置-全网-LTE | 内置-全网-NR | 内置-全网-GSM |
| 设备组 `device_group` | 内置-设备组-LTE | 内置-设备组-NR | 内置-设备组-GSM |
| 产品 `product` | 内置-产品-LTE | 内置-产品-NR | 内置-产品-GSM |
| 频段 `band` | 内置-频段-LTE | 内置-频段-NR | 内置-频段-GSM |

任务固定 ID、默认指标、制式、维度和 `is_builtin=true` 继续沿用原 baseline seed；默认粒度保持 `hourly`。内置任务不可删除，只允许编辑指标，编辑后生成不可变新版本。

新增独立的内置任务协调器：

- 内置模板是唯一初始化清单；升级时只补缺失定义，不覆盖用户已修改的内置指标。
- worker 启动时立即协调，并每 5 分钟重新检查一次设备、产品、设备组和频段成员。
- 当前没有匹配设备时仍保留 `pm_tasks` 定义，不创建空执行版本；首次出现匹配设备后自动创建版本。
- 为任务定义、指标规则和排序后的成员快照计算内容哈希。在事务内锁定逻辑任务并比较当前版本哈希；内容未变化时不得产生新版本。
- 内容变化时创建新版本，旧版本关闭到新版本生效边界；新版本从下一个完整 15 分钟边界开始匹配。
- 多 worker 并发协调依靠数据库锁和内容哈希保持幂等，不能产生重复版本。
- 协调失败只记录错误和指标，保留上一个可用版本，下一个周期重试。

迁移不得再次无条件清空内置定义。用户自建旧任务和历史聚合结果仍按正式切换约定不迁移。
