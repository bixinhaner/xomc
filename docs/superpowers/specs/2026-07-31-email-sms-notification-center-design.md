...
通知策略必须复用现有 `ops_maintenance_windows`，不能另建一套互不一致的维护窗口。
只有已审批且当前生效、`suppress_alarms=true`、并且范围命中当前告警设备的窗口才能影响
外部通知。维护窗口不删除、不改写告警事实，也不阻止历史投影和北向事实入库；通知中心
只将待发外部通知记录为带 `maintenance_window_id` 和原因的 `suppressed`，Critical 是否
绕过必须由显式规则决定。窗口结束后不默认补发已过时的首次通知，是否进入摘要由规则
明确配置。

所有事件、schedule 和 delivery 时间统一以 UTC 持久化。涉及安静时段、摘要窗口和周期
提醒的规则必须绑定明确的 IANA 时区；未配置时使用部署级业务时区，而不是进程本地时区。
夏令时跳过或重复的本地时间按同一 UTC 时间点只执行一次，并在规则预览中展示下一次
实际执行时间。
