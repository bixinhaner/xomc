...
### 9.2 问题单 #307：设备列表管控原因呈现

设备列表沿用老 OMC 的高频交互：在既有“激活状态”单元格内、状态文字右侧增加可点击的
OMC 处置标志。标志只负责提示“本系统存在当前处置事实”，不改变激活状态的取值和颜色；
两类数据在接口和代码中保持分离。无当前处置时不显示标志，避免为绝大多数正常设备重复显示
“未管控”。当前事实源只允许使用 `geofence_control_actions`，不得从普通告警、离线状态或
小区状态反推“因越界去激活”。

列表接口按当前页设备 ID 一次批量读取管控摘要，禁止逐行查询；标志颜色表达处理中、已去激活
和失败，悬浮提示阶段、围栏名称与触发时间。恢复动作核验为 `verified` 后不再显示标志，但历史
动作和证据永久保留。阶段映射固定为：去激活中、核验中、已因越界去激活、部分失败、
去激活失败、恢复中、恢复失败。

点击处置标志打开设备级详情抽屉，读取 `/api/v1/devices/:id/control-actions`。端点复用设备组
数据权限校验，并返回动作父子关系、围栏判定位置/观测版本/规则/距离，以及控制前值、请求值、
GPV 核验值和失败原因。列表抽屉使用 `compact` 模式，展示当前阶段、围栏、触发原因、时间、
最近一次执行状态和围栏判定证据，技术参数默认折叠，并提供进入设备详情“处置记录”页签的
深链接；设备详情使用 `full` 模式
展示完整动作历史和位置证据，原因码、版本及 TR-069 参数路径收进“技术详情”。来源和阶段
作为展开后的高级筛选；
设备导出包含当前管控摘要。控制动作新增
`trigger_evaluation_id`、`trigger_reason_code` 和 `trigger_observation_version`，使页面证据从状态
边沿追溯到不可变围栏判定，而不是依赖后续可能变化的设备状态。
