...
第三方位置只允许写入 `location_source_mode=external` 的设备；默认 `tr069` 设备逐项返回
未配置外部定位。`external` 设备仍可上报普通 TR069 参数，但其 TR069 GPS 坐标不会写入位置
事实或初始化 OMC 接受坐标，从而避免两种来源按到达顺序互相覆盖。

当前接口同步返回的 `success` 只表示位置数据已校验并持久化；围栏判定、告警和设备控制异步执行。
...
| IPSec/RF 去激活及回执 | 已实现动作持久化、幂等 SPV、关联 GPV 和精确值校验 | 展示请求值、回读值及失败原因 | 代码闭环，待真实设备验收 |
| 安全返回与恢复 | 只恢复已验证且由本系统去激活的参数 | 展示恢复动作和回读结果 | 代码闭环，待真实设备验收 |
...
- 设备位置权威来源已收敛为设备级 `tr069/external` 模式；默认 `tr069`，船载且不能自主上报
  GPS 的设备由设备详情明确切换为 `external`。位置观测仓储统一执行来源隔离和时间乱序保护；
  外部模式下忽略 TR069 GPS，但不阻断同一批次的其他设备参数同步。
...
4. **位置来源已定版**：设备按 `location_source_mode` 选择唯一权威来源；默认 `tr069`，明确
   配置为 `external` 后只接受第三方平台位置。两种来源不按最后到达时间竞争。
...
位置来源 Adapter 已按设备模式落地。控制模块继续复用现有 CWMP 任务链；只有确认存在独立
