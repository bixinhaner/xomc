...
PM 聚合任务只是已入库 Counter 的过滤/计算口径，不会让设备额外产生 PM 原始文件，因此不能
把同一设备按聚合任务重复计数。冷启动预测使用在线设备数和系统 PM 文件上报周期；最近一小时
实际新增速率是运行期主要依据，因此设备离线、PM 周期变化或 MR 量变化都会自动反映。
...
- `shadow`：只计算候选、速率和压力，不删除；
- `fallback`：精确删除开启，MinIO ILM 保留作为兜底；
- `exclusive`：精确删除开启，并只移除 OMC 自己创建的 `omc-raw-expire-*` 规则。

默认种子值为 `shadow`。`MINIO_SCANNER_SPEED` 已降为 `slowest`，但没有关闭 MinIO scanner。
