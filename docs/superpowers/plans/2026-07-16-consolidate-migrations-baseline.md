...
: "${DOCKER_BIP:?请先设置客户规划的 DOCKER_BIP}"
docker_network_plan
docker network create --driver bridge --subnet "$DOCKER_MIGRATION_SUBNET" --gateway "$DOCKER_MIGRATION_GATEWAY" omc-migration-baseline
