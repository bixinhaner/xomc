...
故障恢复验证，可以确认 Task 1–6 的功能基础成立。但里程碑 A **尚未正式放行**，生产配置
必须继续保持 `legacy`：现场峰值至少 2 倍的容量压测和生产回退演练尚未完成，因此本轮
不进入 Task 7，也不启用邮件或短信发送。通知 Shadow durable 属于后续 canonical 生产
切换门禁，不反向阻塞建立通知 Inbox 的 Task 7，避免形成实施顺序上的循环依赖。
...
里程碑 A 正式放行仍需：以现场估算峰值至少 2 倍压测，记录 P50/P95/P99、Outbox oldest
age、durable lag、最老消息、磁盘水位和提前淘汰风险，并完成 Shadow 切换与回退演练。
上述门禁通过前，Relay/canonical 只能用于隔离验证。后续创建通知 Inbox 后，还要验证第三条
通知 Shadow 消费链；三条 durable 全部通过前，canonical 仍不得进入生产。

## 下一步

1. 保持默认 `legacy`，不进入生产 canonical。
2. 获取现场告警基线或可代表现场的回放样本，完成容量压测、保留窗口计算和 Shadow 回退
   演练；通过后正式完成里程碑 A，再开始 Task 7。
3. 通知 Inbox 建立后创建只入 Inbox、不发送渠道消息的通知 Shadow durable，补齐第三消费
   链门禁；其创建 sequence 使用里程碑 A 记录的切换基准。
4. 三条 durable 全部通过后才允许评估生产 canonical；邮件试点仍需单独通过里程碑 B。
