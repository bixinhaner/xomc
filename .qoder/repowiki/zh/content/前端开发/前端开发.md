...
- [pages/alarm/CurrentAlarms/ExportModal.tsx](file://omcmb/webcode/src/pages/alarm/CurrentAlarms/ExportModal.tsx)
- [pages/alarm/HistoricalAlarms/index.tsx](file://omcmb/webcode/src/pages/alarm/HistoricalAlarms/index.tsx)
- [pages/alarm/AlarmDetail/index.tsx](file://omcmb/webcode/src/pages/alarm/AlarmDetail/index.tsx)
...
- [i18n/zh-CN/index.ts](file://omcmb/webcode/src/i18n/zh-CN/index.ts)
- [i18n/en-US/index.ts](file://omcmb/webcode/src/i18n/en-US/index.ts)
</cite>

## 更新摘要
**所做更改**
- 更新了告警管理页面的重大UI改进部分，重点介绍了CurrentAlarms和HistoricalAlarms页面重构
- 新增了ExportModal组件的详细说明和使用指南
- 扩展了国际化翻译系统的介绍，涵盖新增的告警相关翻译项
- 更新了告警详情页面的组件分析
- 增强了告警管理页面的交互逻辑和状态管理说明
...
**更新** 本次更新重点关注告警管理页面的重大UI改进，包括CurrentAlarms和HistoricalAlarms页面的重构、新增ExportModal组件以及国际化的全面扩展。

## 项目结构
前端代码位于 omcmb/webcode 目录，采用按功能域划分的组织方式：pages（页面）、components（通用组件）、hooks（自定义 Hook）、services（API 层）、store（状态管理）、providers（上下文提供者）、router（路由）、theme（主题）、styles（样式）、i18n（国际化）等。
...
#### 告警管理（CurrentAlarms、HistoricalAlarms、AlarmDetail）

**更新** 告警管理页面经过重大UI重构，实现了更直观的用户界面和更完善的交互体验。

##### 当前告警页面（CurrentAlarms）
- **页面重构**：采用新的布局设计，优化了告警列表的显示方式，增强了视觉层次感。
- **导出功能**：集成ExportModal组件，支持灵活的告警数据导出配置。
- **批量操作**：提供完整的批量处理功能，包括确认、清除、过滤、标记已读等操作。
- **实时状态**：未读告警优先显示，确保重要信息及时传达。

```mermaid
flowchart TD
CA["CurrentAlarms 页面"] --> EM["ExportModal 导出"]
CA --> BA["批量操作"]
CA --> DT["DataTable 列表"]
CA --> AD["AlarmDetail 详情"]
EM --> EP["导出参数配置"]
BA --> ACK["确认告警"]
BA --> CLR["清除告警"]
BA --> FIL["过滤告警"]
...
**图示来源**
- [pages/alarm/CurrentAlarms/index.tsx:59-589](file://omcmb/webcode/src/pages/alarm/CurrentAlarms/index.tsx#L59-L589)
- [pages/alarm/CurrentAlarms/ExportModal.tsx:57-164](file://omcmb/webcode/src/pages/alarm/CurrentAlarms/ExportModal.tsx#L57-L164)

##### 历史告警页面（HistoricalAlarms）
- **页面重构**：与当前告警页面保持一致的设计风格，提供统一的用户体验。
- **删除功能**：新增告警删除操作，支持批量清理历史数据。
- **状态管理**：专门的状态配置，仅显示已清除的告警记录。
- **导出功能**：同样集成了ExportModal组件，支持历史告警数据导出。

```mermaid
flowchart TD
HA["HistoricalAlarms 页面"] --> EM["ExportModal 导出"]
HA --> BA["批量操作"]
HA --> DT["DataTable 列表"]
HA --> AD["AlarmDetail 详情"]
EM --> EP["导出参数配置"]
BA --> ACK["确认告警"]
BA --> DEL["删除告警"]
```
...
**图示来源**
- [pages/alarm/HistoricalAlarms/index.tsx:59-543](file://omcmb/webcode/src/pages/alarm/HistoricalAlarms/index.tsx#L59-L543)
- [pages/alarm/CurrentAlarms/ExportModal.tsx:57-164](file://omcmb/webcode/src/pages/alarm/CurrentAlarms/ExportModal.tsx#L57-L164)

##### 告警详情页面（AlarmDetail）
- **抽屉式设计**：采用Ant Design Drawer组件，提供沉浸式的告警详情查看体验。
- **信息分组**：将告警信息分为基本信息、设备信息、状态与时间、处理信息四个主要部分。
- **动态展示**：根据告警状态动态显示确认和清除相关信息。
- **响应式布局**：支持不同屏幕尺寸下的最佳显示效果。
...
**更新** 国际化系统得到全面扩展，新增了大量告警管理相关的翻译项，支持中英文双语界面。

- **翻译覆盖**：涵盖了导航、页面标题、表头、状态标签、操作按钮、确认对话框等各个方面的文本。
- **告警专用翻译**：新增了告警级别、事件类型、处理状态、操作确认等专用术语的国际化支持。
- **动态语言切换**：通过useT Hook实现组件级别的语言切换，无需刷新页面即可改变显示语言。
- **翻译结构化**：采用层级化的翻译键命名，便于维护和查找特定的翻译内容。
...
## 结论
本前端工程以 React 19 + TypeScript + Vite 为基础，结合 Ant Design 5.x 与多主题体系，形成高可维护、可扩展的前端架构。通过 React Router 的路由懒加载、React Query 的缓存与并发控制、Zustand 的轻量状态管理以及丰富的通用组件库，能够高效支撑设备管理、告警管理、性能监控、MR 管理与拓扑可视化等核心业务场景。

**更新** 最近的告警管理页面重构显著提升了用户体验，新增的ExportModal组件提供了灵活的数据导出功能，全面的国际化支持确保了多语言环境下的良好体验。建议在后续迭代中持续完善类型体系、测试覆盖率与性能监控，以保障长期演进。
...
**更新** 详细介绍告警管理页面重构的关键改进：

- **界面布局优化**：采用更清晰的信息层次结构，突出重要告警信息
- **交互体验提升**：优化批量操作流程，提供更直观的操作反馈
- **导出功能集成**：ExportModal组件提供灵活的字段选择和格式配置
- **状态管理完善**：区分当前告警和历史告警的不同状态处理逻辑
- **国际化支持**：全面的中英文翻译，支持多语言环境下的统一体验
