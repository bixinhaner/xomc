...
func Test_AlarmSeverityMapping(t *testing.T) {
	...
	tests := []struct {
		...
		{"UNKNOWN_ALARM", model.AlarmSeverity(0)},
	}
...
func Test_GetInfoParamMapping_NR(t *testing.T) {
	...
	for _, want := range []struct {
		...
		got, ok := m[want.path]
		assert.Truef(t, ok, "CUCC NR mapping missing path %s", want.path)
		assert.Equalf(t, want.col, got, "CUCC NR mapping %s expected column %s", want.path, want.col)
	}
}
