...
const (
	...
	// SMTP 配置由 app/worker 的 notification.smtp 统一注入。
...
type AlarmFilterRule struct {
	...
	EffectiveStart   *time.Time  `json:"effective_start,omitempty" db:"effective_start"`
...
type UpdateAlarmFilterRuleRequest struct {
	...
	EffectiveStart       *time.Time  `json:"effective_start"`
	EffectiveEnd         *time.Time  `json:"effective_end"`
	ClearEffectiveWindow bool        `json:"clear_effective_window"`
	Priority             *int        `json:"priority"`
	Enabled              *bool       `json:"enabled"`
}
