...
func Test_BatchCreateTasks_DefaultExpiresIn_TableDriven(t *testing.T) {
	type tc struct {
		name              string
		serviceDefault    int
		reqExpiresIn      int
		wantExpiresAtNil  bool
		wantExpiresInSecs int
	}
	cases := []tc{
		{name: "未传_有默认_批量任务用默认", serviceDefault: 120, reqExpiresIn: 0, wantExpiresAtNil: false, wantExpiresInSecs: 120},
		{name: "显式传值_有默认_批量任务用调用方值", serviceDefault: 120, reqExpiresIn: 420, wantExpiresAtNil: false, wantExpiresInSecs: 420},
		{name: "未传_无默认_批量任务仍为永不超时", serviceDefault: 0, reqExpiresIn: 0, wantExpiresAtNil: true},
	}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			reqs := []*CreateTaskRequest{
				{DeviceSN: "SN_BATCH_EXP", Method: "GetParameterValues", ExpiresIn: c.reqExpiresIn},
			}
			tasks, err := ts.BatchCreateTasks(context.Background(), reqs)
			require.NoError(t, err)
			...
			got := persisted[0]
			if c.wantExpiresAtNil {
				assert.Nil(t, got.ExpiresAt, "ExpiresAt 应为 nil（永不超时）")
				return
			}

			require.NotNil(t, got.ExpiresAt, "ExpiresAt 不应为 nil")
			elapsed := got.ExpiresAt.Sub(got.CreatedAt)
			wantLo := time.Duration(c.wantExpiresInSecs)*time.Second - 100*time.Millisecond
			wantHi := time.Duration(c.wantExpiresInSecs)*time.Second + 100*time.Millisecond
			assert.GreaterOrEqual(t, elapsed, wantLo, "ExpiresAt - CreatedAt 偏小")
			assert.LessOrEqual(t, elapsed, wantHi, "ExpiresAt - CreatedAt 偏大")
