...
func TestBuildOrchestrationDecision_RaisedPlansGateAndBoundedRepeats(t *testing.T) {
	...
	require.Equal(t, input.Event.EventID, decision.Schedules[0].EventID)
	require.Equal(t, input.Rules[0].Channels[0].RaisedTemplateVersionID, decision.Schedules[0].TemplateVersionID)
	require.Equal(t, input.Rules[0].Recipients[0].Ciphertext, decision.Schedules[0].AddressCiphertext)
