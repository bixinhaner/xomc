...
// NULL ILIKE '%cpe%' → NULL → 走 ELSE（基站阈值），不会被误用 CPE 阈值。
//
...
// product_class 为 NULL 的设备，CASE WHEN cpeProductClassPredicate 不为 TRUE，
// 因此选用 ELSE（基站 eNB）阈值。用一条不依赖具体表数据的查询，直接对 PG 求值
// CASE 表达式：给 NULL 当作 product_class 时返回 ELSE 的哨兵值。
