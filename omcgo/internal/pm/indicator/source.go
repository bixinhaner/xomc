...
// BuiltinDirSubdir 是 XMLBaseDir 下唯一的 indicator XML 子目录名(不含末尾 /)。
// 三库 XML 导入重构(2026-06-04 D3/D5/D6):取消 builtin/custom 双目录,所有 XML
// (出厂 + 用户上传)同住此目录树(enb/gsm/gnb 三制式子目录 + GSM.xml / GNB.xml 根级单文件);
// 来源由同目录 sidecar(X.xml.custom)判定,见 CustomMarkerSuffix。
// Loader 扫描、Handler Upload 写入、deploy.sh 初始化目录均引用此常量。
const BuiltinDirSubdir = "indicator-library" // 唯一 indicator XML 目录(builtin + custom 同住)
