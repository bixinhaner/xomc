...
func TestBuildDeviceKeysetSQL_FirstBatch_NoCursor(t *testing.T) {
	...
	assert.NotContains(t, q, "pm_hourly_bucket_versions")
	assert.NotContains(t, q, "pm_hourly_anchors")
	assert.NotContains(t, q, "pm_hourly_values")
	assert.NotContains(t, q, "d.metric_id=ANY(s.metric_ids)")
