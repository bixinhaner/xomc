...
func ParseFormula(expr string) (*Formula, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("parse formula %q: %w", expr, err)
	}
	p.skipSpaces()
	if p.pos < len(p.input) {
		return nil, fmt.Errorf("parse formula %q: unexpected character at position %d", expr, p.pos)
	}
	return &Formula{Expression: expr, root: root}, nil
}
