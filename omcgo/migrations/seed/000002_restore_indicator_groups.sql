-- +goose Up
-- 恢复 indicator_group_{enb,gsm,gnb} 功能集树（22+8+15=45 节点）。
--
-- 背景：该分组树原由 b0d65afc「功能集树治理」灌入 seed（设计 §2.2 —— 分组树定义属稳定字典、
--       留 seed；只有"指标→组"关联走 XML）。2026-06-08 dd991a44「init_seed 拆分为纯部署
--       baseline」按"字典走 XML"的理由把这三张表的种子数据删了，但 XML 实际只携带 groupId
--       引用、不含组节点定义，dictloader 启动也只插 1 个 'default' 占位组 —— 导致 22/15/8 个
--       业务功能集分类丢失，前端「分组」列退化为裸 hash、「按分组筛选」下拉为空。
-- 本迁移按原样恢复（id 与 perf_indicators_*.group_id / XML groupId 一一对齐，重启后 JOIN 即得名）。
-- 数据来源：b0d65afc:seed/000062_seed_indicator_groups.sql（更早溯源 commit 4eb84dca^ 的 seed/000027）。
-- 与 dictloader.ensureDefaultGroups 协作：dictloader 仅插 'default'，二者 ON CONFLICT DO NOTHING 不冲突。
...
-- +goose Down
-- 不可回滚：被 perf_indicators_*.group_id 引用，DELETE 会破坏关联。
SELECT 1;
