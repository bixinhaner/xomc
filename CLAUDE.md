...
**Claude AI 的 git 操作边界**（与 `.claude/settings.json` 双重落地）:

| 操作 | Claude 行为 |
|------|-----------|
| `git status` / `git diff` / `git log` / `git show` / `git branch`（只读） | **自动执行**，无需确认 |
| `git add` / `git commit` | **自动执行**，但提交消息必须符合上文 Conventional Commits 中文规范，并附 `Co-Authored-By` 尾巴 |
| `git push` / `git push --force` / `git push --tags` / `git push origin ...` | **严禁自动执行**。必须由用户明确发出 "推送" / "push" / "上传" / "同步远端" / "发出去" 等指令才能执行。模糊指令（如"搞定这个 bug"、"提交一下"）**只提交不推送** |
| `git rebase -i` / `git reset --hard` / `git branch -D` / `git clean -f` | 需用户明确指令；默认保护 |
| `git pull` / `git fetch` | 通常需用户明确指令（拉取可能引入意外变更） |

**行为准则**：Claude 完成一个任务后默认**只 commit 不 push**，并在回复中一句话说明"本地已提交（commit hash xxx），未推送远端，如需推送请告知"。
