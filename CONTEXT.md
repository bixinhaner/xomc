...
| **KPI 路由（KPI Route）** | 根据设备所属产品装配件及其 KPI 平台确定该设备允许采集的 counter 与需要计算的 KPI 集合；它同时约束原始 counter 准入和 KPI 求值范围。 |
| **PM 结果值规范化** | 新产生的 PM counter / KPI 结果写入结果表前，按指标单位和统计类型统一取整、保留小数或百分比截断；它改变新增结果数据的存储值，不是前端、导出或接口响应层的展示格式化。旧数据不回填。 |
| **统计时长（StatisDuration）** | **全系统通用的时间基准计数器**。解析每个性能文件时，由解析层为每个小区合成补一条，取值 = 文件头声明的**真实采集周期**（当前 15 分钟 = 900 秒，随文件实际值变化、不写死常量）。它是**累加型**计数器（statisType=sum），跟随现有累加聚合一路滚动：单小区小时=3600、天=86400、全网=所有小区累加。**约定**：今后所有"按时间归一化"的派生指标，分母一律用统计时长，禁止再把采集周期写死成秒数常量。命名严谨强调"累计"语义，**不叫"周期"**——避免被误当成固定 900 常量、在多小区多粒度聚合后误用算错。 |
| **小区在服时长（CellServiceTime）** | 原始累加计数器，记录小区处于在服状态的累计秒数（满在服 = 一个采集周期，如 900 秒）。是"小区可用率"的分子，保持累加（sum）语义不变，不为凑算法去改它的聚合方式。 |
| **小区可用率（Cell Availability）** | 派生指标（statisType=pct），公式 = **在服时长 ÷ 统计时长 × 100**。因分子（在服时长）与分母（统计时长）同源累加，比率在任意粒度、任意维度（单小区→设备→全网）都自动成立，小区数与停报自动消解。主仪表板"可用性"面板取此指标。 |
