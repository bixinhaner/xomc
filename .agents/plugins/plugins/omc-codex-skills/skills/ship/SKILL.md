...
|---|---|
| 空 | 推断当前阶段，并自动推进到 MR。 |
| `status` | 只报告 GitLab Issue 看板分桶和当前分支阶段。 |
| `audit` | 只推断当前阶段、已过门、未过门和下一步动作。 |
| `#NN` | 交付指定 Issue；如果还不是 `ready-for-agent`，先分诊。 |
| `N` 或 `all` | 按优先级选择前 N 个或全部 open `ready-for-agent` Issue，每个 Issue 独立分支、独立 MR。 |
| `P1`..`P9` | 只跑指定阶段。别名：`align`、`spec`、`slice`、`triage`、`build`、`verify`、`review`、`commit`、`mr`、`close`。 |
| `--fast bugfix\|hotfix\|docs\|refactor` | 使用对应快速通道，但仍执行所有适用硬门，并且必须开 MR。 |
...
当需求已成形，可以发布为 GitLab Issue 时，使用 `$to-prd`。
...
推送 feature 分支并使用 `glab mr create` 创建 GitLab MR。交付 Issue 时，MR description 必须包含 `Closes #NN`。
...
5. 已有成形需求但没有 GitLab Issue -> P2；如果仍模糊则 P1。
6. 只有模糊想法 -> P1。
...
1. 查询 open `ready-for-agent` Issue：`glab issue list --label ready-for-agent --per-page 30`（默认列出 open issues）。
2. 按优先级排序：`critical`、`high`、`medium`、`low`，同优先级按 Issue 编号升序。
3. 串行处理每个 Issue。从 `main` 起步，创建专属 feature 分支，执行 P5-P9，开一个独立 MR，再处理下一个。
4. 不自动 `git pull`，除非用户要求。
5. 如果某个 Issue 合理修复尝试后仍阻塞，在 Issue 评论阻塞点，保留半成品分支，在汇总中标记 blocked，然后继续下一个。

结束时给出逐 Issue 汇总：MR URL、blocked 原因或 skipped 原因。

## OMC 项目规则

- 从仓库根目录 `xomc/` 执行 git 命令。
- `AGENTS.md`、`.codex/` 以及无关本机 AI 配置默认不进 MR，除非用户明确要求。本 skill 自身位于 `.agents/skills/ship`，是用户明确要求的团队共享源码。
