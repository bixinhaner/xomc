...
本次改动将 BLQ、MLQ 的 WAN 配置切换为与 MLN 一致的表格操作形式，同时由各站型 QuickSettings 配置决定实际行数和字段。BLQ、MLQ 使用 WAN 1–12；MLN、BLN 保持既有 WAN 1–4 和静态路由配置。BLQ 的 TR-069 接口绑定恢复为根据连接类型、启用 WAN 和 VLAN 动态生成选项。
