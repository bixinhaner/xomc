  ...
  const columns: DataTableColumn<MMLCustomCommand>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        width: 100,
        render: (val: unknown) => { const v = String(val ?? ''); return <Tag color={v === 'MOD' ? 'orange' : 'blue'}>{v}</Tag>; },
      },
      { key: 'description', title: t('common.description'), dataIndex: 'description', ellipsis: true, render: (v: unknown) => (v as string) || '-' },
      { key: 'creator', title: t('mml.creator'), dataIndex: 'creator', width: 100 },
      { key: 'updatedAt', title: t('mml.updateTime'), dataIndex: 'updatedAt', width: 160, render: (val: unknown) => formatTime(val as string) },
    ],
    [t, handleDelete],
  );
