...
const mockCommands: MMLCommand[] = [
  { id: '1', commandName: '查询小区信息', commandCode: 'LST CELL', category: '小区管理', description: '查询小区配置信息，支持按小区ID过滤查询', params: [{ name: 'CELLID', type: 'number', required: false, description: '小区ID' }], productTypes: ['eNB', 'gNB'] },
  { id: '2', commandName: '激活小区', commandCode: 'ACT CELL', category: '小区管理', description: '激活指定小区，使其进入服务状态', params: [{ name: 'CELLID', type: 'number', required: true, description: '小区ID' }], productTypes: ['eNB'] },
  { id: '3', commandName: '去激活小区', commandCode: 'DEA CELL', category: '小区管理', description: '去激活指定小区，退出服务状态', params: [{ name: 'CELLID', type: 'number', required: true, description: '小区ID' }], productTypes: ['eNB'] },
  { id: '4', commandName: '查询邻区', commandCode: 'LST NCELL', category: '邻区管理', description: '查询邻区配置关系列表', params: [{ name: 'LOCALCELLID', type: 'number', required: false, description: '本地小区ID' }], productTypes: ['eNB', 'gNB'] },
  { id: '5', commandName: '添加邻区', commandCode: 'ADD NCELL', category: '邻区管理', description: '添加邻区关系', params: [{ name: 'LOCALCELLID', type: 'number', required: true, description: '本地小区ID' }, { name: 'CELLID', type: 'number', required: true, description: '邻小区ID' }], productTypes: ['eNB'] },
  { id: '6', commandName: '查询基站状态', commandCode: 'LST BTSSTATE', category: '基站管理', description: '查询基站运行状态信息', params: [], productTypes: ['eNB', 'gNB'] },
  { id: '7', commandName: '查询活动告警', commandCode: 'LST ALMAF', category: '告警查询', description: '查询当前活动告警列表', params: [{ name: 'ALMFAULTID', type: 'number', required: false, description: '告警ID' }, { name: 'SEVERITY', type: 'enum', required: false, description: '告警级别', options: [{ label: '严重', value: 1 }, { label: '主要', value: 2 }] }], productTypes: ['eNB', 'gNB'] },
  { id: '8', commandName: '复位基站', commandCode: 'RST BTS', category: '基站管理', description: '对基站进行软复位操作', params: [{ name: 'RSTTYPE', type: 'enum', required: true, description: '复位类型', options: [{ label: '软复位', value: 0 }, { label: '硬复位', value: 1 }] }], productTypes: ['eNB', 'gNB'] },
];
...
export default function CommandTree() {
  ...
  const columns: DataTableColumn<CommandRow>[] = useMemo(() => [
    ...
    { key: 'category', title: t('perf.category'), dataIndex: 'category', width: 110 },
    ...
    <TreeListPageLayout tree={treePanel}>
      <div style={{ padding: '12px 16px', borderBottom: '1px solid #f0f0f0' }}>
        <Typography.Text strong style={{ fontSize: 14 }}>
          {selectedCategory ? `${selectedCategory}` : t('nav.mml.commands')}
