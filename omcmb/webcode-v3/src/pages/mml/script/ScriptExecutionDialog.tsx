    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="执行脚本" onClick={onClose}>
      <div className="glass-strong relative flex max-h-[90vh] w-[min(680px,calc(100vw-32px))] flex-col overflow-hidden border border-cyan-500/30" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-auto px-4 py-3">
          ...
          {submitting || mutation.isPending ? <div role="status" className="font-mono text-xs text-cyan-200">正在提交执行…</div> : null}
        ...
        <footer className="flex justify-end gap-2 border-t border-cyan-500/20 px-4 py-3"><NeonButton onClick={onClose}>取消</NeonButton><NeonButton onClick={submit} disabled={mutation.isPending || submitting} icon={mutation.isPending || submitting ? <Loader2 className="animate-spin" /> : <Play />}>{mutation.isPending || submitting ? '提交中…' : '执行'}</NeonButton></footer>
