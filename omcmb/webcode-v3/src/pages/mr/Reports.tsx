...
// ---------------------------------------------------------------------------
// MR 分析报表（对齐 v1 mr/Reports）。
// v1 该页用 useDownloadReport 下载报表记录；这里用完整的报表定义 + 生成记录两路真实数据
// （useReportDefinitions / useReportRecords / useGenerateReport / useDownloadReport），
// 全部 @core mock/real 自动切换。点报表定义进入 /mr/reports/:id 详情。
// ---------------------------------------------------------------------------
...
const REC_COLOR: Record<RecStatus, string> = {
  ready: '#00ff88',
  generating: '#ffaa00',
  failed: '#ff2d6f',
}
...
export default function Reports() {
  ...
  const defParams = useMemo(
    () => ({ page: 1, pageSize: 200, ...(typeFilter ? { reportType: typeFilter } : {}) }),
    [typeFilter],
  )
  const {
    data: defData,
  ...
  const recParams = useMemo(() => ({ page: recPage, pageSize: recPageSize }), [recPage])
  const {
    data: recData,
  ...
  const onGenerate = (definitionId: string) => {
    generateReport.mutate({ definitionId }, { onSuccess: () => void refetchRecs() })
  }
  ...
  return (
    <PageShell
      code="F05"
      title="MR REPORTS · 分析报表"
      subtitle="MR ANALYSIS REPORT DEFINITION · GENERATED ARCHIVE"
    ...
    >
      <div className="mb-3 grid grid-cols-4 gap-3">
        <BigStat label="DEFINITIONS · 报表定义" value={stats.total} color="#00f0ff" icon={<FileText className="size-4" />} />
        ...
        <BigStat label="READY RECORDS · 就绪记录" value={recError ? '—' : readyRecCount} color="#ffaa00" icon={<Layers className="size-4" />} />
      </div>

      <div className="grid grid-cols-12 gap-3">
        {/* 左：报表定义 */}
        <div className="col-span-12 lg:col-span-7">
          <GlassPanel title="REPORT DEFINITIONS · 报表定义" meta={`${defs.length} / ${stats.total}`}>
            <div className="max-h-[calc(100vh-360px)] min-h-[280px] overflow-auto p-3">
              {defLoading ? (
                <Centered>
                  <Loader2 className="size-4 animate-spin" />
                  <span className="font-mono text-xs uppercase tracking-[0.2em]">SYNCING DEFINITIONS…</span>
                </Centered>
              ) : defError ? (
                  ...
                  {defs.map((d) => {
                    ...
                    return (
                      <div
                        ...
                        role="button"
                        tabIndex={0}
                        onClick={() => navigate(`/mr/reports/${encodeURIComponent(d.id)}`)}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' || e.key === ' ') {
                            navigate(`/mr/reports/${encodeURIComponent(d.id)}`)
                          }
                        }}
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <span className="chip shrink-0" style={{ color: tColor }}>
        ...
        <div className="col-span-12 lg:col-span-5">
          <GlassPanel title="GENERATED ARCHIVE · 生成记录" meta={`TOTAL ${recTotal}`}>
            <div className="max-h-[calc(100vh-360px)] min-h-[240px] overflow-auto">
              {recLoading ? (
                ...
                records.map((r) => {
                  ...
                  const color = REC_COLOR[st] ?? '#6b86b6'
                  return (
                    <div
                      key={r.id}
...
/** 确定性伪随机趋势（同 seed 同序列，避免 render 中 Math.random） */
