...
<div class="lockbody" id="winLockScreen" style="display:none;">			
	...
	<form id="lockform" class="lockformbox">  
		...
		<div class="lock_user_div">
			...
			<input style="color:#96b3ed;width:80%;" readonly type="text" id="uid_lock" class="input_" name="userInfo.usercode"  value="${userInfo.usercode}">
		</div>
		<div class="lock_pwd_div">
			...
			<div v-if="isBrowserAutoRecordPass" style="display: inline-block;width: 80%;">
				...
				<input id="lockPassword" style="width: 100%;display:none;"  type="text" class="input_" name="userInfo.password" placeholder="<%=rb.getString("QingShuRuMiMa")%>">
			</div>
			<input v-if="!isBrowserAutoRecordPass" id="lockPassword" style="width: 80%;"  type="password" class="input_" name="userInfo.password" placeholder="<%=rb.getString("QingShuRuMiMa")%>">
			<span id="lockPassResult" style="color:red; font-size: 13px; display:block;text-align:left;margin-left:41px;margin-top:8px;"></span>
		</div>
		<div class="lock_btn_div" onclick="javascript:cancelLock_RSA()" style="position: initial;margin-top: 10%;">
    ...
    function viewMoreDetail(ele,desc,versionId,upgradeType){
  	...
  	function toSoftwareFile(versionId,upgradeType){
  		 if(upgradeType == "enodeb"){
    		 ...
    		 //goMenuPage("10042");
			try{
				eventAllBus.$emit("gomenupage","10042","","10042",false);
			}catch(e){}
    	  } 
  		 ...
  		 if(upgradeType == "cpe_odu"){
  			isJumpToPage = {code:'cpe_odu',vid:versionId}
   		 	//goMenuPage("70042");
			try{
				eventAllBus.$emit("gomenupage","70042","","70042",false);
			}catch(e){}
  		 }
