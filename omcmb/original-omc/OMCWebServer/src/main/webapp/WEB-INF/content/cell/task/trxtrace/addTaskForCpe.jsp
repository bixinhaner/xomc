	...
	<div id="step_1" class="easyui-layout" data-options="border:false,fit:true">
		...
		<div region="center" data-options="border:false" style="padding: 0 20px 20px;">
			<div class="easyui-layout" data-options="border:false,fit:true">
				<div region="west" data-options="width:370,border:true,collapsible:false" title="<%=rb.getString("JiZhanLieBiao")%>" style="padding-top:20px;">
					<table id="gridCell_addTask_trace_cell"></table>
				</div>
				...
				<div class="special2" region="east" data-options="width:370,border:true,collapsible:false,title:'<%=rb.getString("YiXuanZeJiZhan")%>'" style="padding-top:20px;">
					<%-- 已选择CPE --%>
			    	<table class="easyui-datagrid" id="selectedCell_addTraceTaskCell"
		...
		<div region="south" data-options="border:false,height:78" style="padding:20px;">
		...
		<div class="easyui-layout" data-options="border:false,fit:true">
			...
			<div region="south" data-options="border:false,height:78" style="padding: 20px">
				<div class="windowButtonGroup">
					<a class="linkbutton linkbutton_nowanna" href="javascript:void(0)" onclick="cancelCreateTraceTaskCpe()" style="float: right;margin-right:0px"><span><%=rb.getString("QuXiao")%></span></a>
					<a class="easyui-linkbutton linkbutton linkbutton_trend" href="javascript:void(0)" onclick="saveTraceTaskCpe()" style="float: right;margin-right: 20px;"><span><%=rb.getString("WanCheng")%></span></a>
					<a class="linkbutton linkbutton_nowanna" href="javascript:void(0)" onclick="stepUp_addTraceTaskCpe()" style="float: right;margin-right: 20px;"><span><%=rb.getString("ShangYiBu")%></span></a>
				</div>
			</div>
		</div>
	</div>
...
<%-- 工具栏-CPE列表 --%>
<div id="toolbar_GridCell_addTask_trace_cell" class="admin_query_head" style="background-color:white;">
	 <ul class="inputslist defaultQuery" style="margin:0;">
	 	<li class="select_reset">
			<select id = "traceDeviceGroupCell" name="type" class="border border-box" style="width:100px;margin-right:1px;height: 26px;"></select>
		</li>
		<li class="serial_number input_li">
			<%-- <input style="width:195px;margin-left:0px;" name="value" type="text" class="border border-box" placeholder="<%=rb.getString("XiaoZhanBianMaHUOMINGCHENG")%>"> --%>
			<input style="width:195px;margin-left:0px;" name="value" type="text" placeholder="<%=rb.getString("XiaoZhanBianMaHUOMINGCHENG")%>" class="searchInputStyle" onfocus="inputOnfocusStyle(this)" onblur="inputOnBlourStyle(this)" />
		</li>
		<li>
			<b class="searchResultImgChangeStyle" onclick="$('#gridCell_addTask_trace_cell').datagrid('reload');"></b>
		</li>
	</ul> 
</div>
...
$(function() {
	...
	$("#gridCell_addTask_trace_cell").datagrid({
		border: false,
		fitColumns: true,
		fit: true,
        rownumbers: true,
        ...
        onBeforeLoad: beforeLoad_gridCell_addTask_trace_cell,
        onLoadSuccess: loadSuccess_addTask_trace_cpe,
        onLoadError: datagridLoadError,
        toolbar: '#toolbar_GridCell_addTask_trace_cell',
        onCheckAll:checkAll_trxtrace,
        onBeforeCheck:beforeCheck_trxtrace,
        columns: [[
	...
	$("#gridCell_addTask_trace_cell").datagrid("getPager").pagination({
		layout:['prev','manual','next','refresh']
	});
	
	$("#traceDeviceGroupCell").combobox({
...
$("#traceDeviceGroupCell").combobox('setValues',['<%=rb.getString("SheBeiZu")%>','<%=rb.getString("SheBeiZu")%>']);
function checkAll_trxtrace(rows){
	$(this).datagrid("initRow").datagrid("getPanel").find(".datagrid-htable .datagrid-header-check>input").prop("checked",true);
}
function beforeCheck_trxtrace(index,row){
...
function loadSuccess_addTask_trace_cpe() {
	...
	//初始添加-行操作提示记录
    $("#selectedCell_addTraceTaskCell").datagrid({
    	url : "",
    	data: {
    		total: 1,
    		rows: [
    ...
    if (delCellCodeObjArr.length > 0) {
    	for(var cellCount = 0; cellCount < delCellCodeObjArr.length; cellCount++) {
    		var rowIndex = $("#gridCell_addTask_trace_cell").datagrid("getRowIndex", delCellCodeObjArr[cellCount]["small_cell_code"]);
    		if (rowIndex != -1) {
    			//$("#gridCell_addTask_trace_cell").datagrid("deleteRow",rowIndex);
    			$("#gridCell_addTask_trace_cell").datagrid("setRow",{small_cell_code:delCellCodeObjArr[cellCount]["small_cell_code"],usable:false});
    		}
    	}
    	//向右侧列表添加已经选中的CPE
    	for (var i = 0; i < delCellCodeObjArr.length; i++) {
    		...
    		$("#selectedCell_addTraceTaskCell").datagrid("appendRow", row);
    	}
    	 
    	var rows = $("#selectedCell_addTraceTaskCell").datagrid("getRows");
    	if (rows.length >= 2 && (rows[0]["value"].length == 0 || rows[0]["text"] == "<%=rb.getString("QingXuanZe")%>")) {
    		$("#selectedCell_addTraceTaskCell").datagrid("deleteRow", 0);
    	}
    }
}
<%-- 初始化CPE树 --%>
function initCellGrid_addTraceTask() {
	<%-- CPE搜索-回车事件 --%>
    $("#toolbar_GridCell_addTask_trace_cell input[name='value']").bind("keyup", function (e) {
        if (e.keyCode == 13) {
            $("#gridCell_addTask_trace_cell").datagrid("reload");
        }
    });
	...
	<%-- CPE搜索-类型-change事件 --%>
	$("#toolbar_GridCell_addTask_trace_cell select[name='type']").bind("change", function(e) {
		var type = e.target.value;
		$("#toolbar_GridCell_addTask_trace_cell .input_li").hide();
		$("#toolbar_GridCell_addTask_trace_cell ." + type).show();
...
function stepDown_addTraceTaskCpe() {
	...
	// 取得已选择的CPE
	var cellCodes = "";
	var selCells = $("#selectedCell_addTraceTaskCell").datalist("getRows");
	...
	$.ajax({
		type: "post",
		url: "${ctx}/task/trxtrace/validateCellIsTracing.action", 
		data: {"cellCodes": cellCodes},
		async: false,
		dataType: 'json',
	...
	if (validateStep1_addTraceTaskCpe()) {
		...
		// 改变窗口大小
		/* $("#winAddTrxTraceTaskCpe").window("resize", {
			height: 545,
			width: 860
		}).window("center"); */
		setDefaultWindow({
...
function validateStep1_addTraceTaskCpe() {
	...
	if ($("#taskName_addTraceTaskCpe").val().trim().length == 0) {
		...
		$.ajax({
			type: "post",
			url: "${ctx}/task/trxtrace/taskNameExistForCpe.action", 
...
function delSelectedCell_addTraceTaskCpe() {
	var dl = $("#selectedCell_addTraceTaskCell");
	...
	if (selCellLength > 0) {
        		...
        		for(var cellCount = 0; cellCount < delCellCodeObjArr.length; cellCount++) {
        			if (selCell[selCellCount].value == delCellCodeObjArr[cellCount]["small_cell_code"]) {
            			...
            			if(choosedGroupId == delCellCodeObjArr[cellCount]["groupId"]){//如果是当前组被选中的站，则可以在右侧添加，否则不添加
            				//$("#gridCell_addTask_trace_cell").datagrid("appendRow",row);
            				$("#gridCell_addTask_trace_cell").datagrid("setRow",{small_cell_code:row["small_cell_code"],usable:true});
            			}
    	...
    	if (needDelCellArr.length > 0) {
    		...
    		for(var cellCount = 0; cellCount < needDelCellArr.length; cellCount++) {
    			var rowIndex = $("#selectedCell_addTraceTaskCell").datagrid("getRowIndex", needDelCellArr[cellCount]);
    			$("#selectedCell_addTraceTaskCell").datagrid("deleteRow",rowIndex);
    		}
    	}
		...
		//将复选框取消选中
		$("#selectedCell_addTraceTaskCell").datagrid("uncheckAll");
	}
}

<%-- 选择小站-向右键头的点击事件 --%>
function addSelectedCell_addTraceTaskCpe() {
	var selCell = $("#gridCell_addTask_trace_cell").datagrid("getSelections");
	var selCellLength = selCell.length;
	if (selCellLength == 0) {
		return;
	}
	...
	if (newSelectdArr.length > 0) {
		...
		for(var cellCount = 0; cellCount < newSelectdArr.length; cellCount++) {
			var rowIndex = $("#gridCell_addTask_trace_cell").datagrid("getRowIndex", newSelectdArr[cellCount]["small_cell_code"]);
			//$("#gridCell_addTask_trace_cell").datagrid("deleteRow",rowIndex);
			$("#gridCell_addTask_trace_cell").datagrid("setRow",{small_cell_code:newSelectdArr[cellCount]["small_cell_code"],usable:false});
		}
	}
	
	$("#gridCell_addTask_trace_cell").datagrid("uncheckAll");
}
...
function saveTraceTaskCpe() {
	...
	$.post("${ctx}/task/trxtrace/addTaskForCpe.action", params, function(data) {
		/* $("#winAddTrxTraceTaskCpe").window("close"); */
		closeDefaultWindow();
		if (data["success"]) {
			$("#trxTraceTaskListCpe").datagrid("load");
		} else {
...
//加载前事件-CPE列表
function beforeLoad_gridCell_addTask_trace_cell(param) {
	param["CA_FLAG"] = '0'; //表示不是CA的站，CA站暂不支持
	//查找当前选中的
	if(choosedGroupId > 0){
		param["group_id"] = choosedGroupId;
	}
	var search_text = $("#toolbar_GridCell_addTask_trace_cell .inputslist li.serial_number>input").val();
	if(search_text != ""){
