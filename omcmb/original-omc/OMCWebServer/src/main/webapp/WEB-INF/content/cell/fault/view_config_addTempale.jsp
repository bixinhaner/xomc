...
#viewInfoPage .el-input__inner, #viewInfoPage .el-textarea__inner{
...
#viewInfoPage .el-input__prefix{
	top:-5px;
}

#viewInfoPage .el-select .el-input .el-select__caret{
	line-height:28px;
}
...
#viewInfoPage .el-icon-search{
	margin-left:0px !important;
} 
.boxBorderCon .el-icon-circle-close{
	line-height:20px;
}
#viewInfoPage .el-form-item{
	margin-left:34px;
	margin-bottom: 13px;
}
#viewInfoPage .concentBoxENB .el-form-item,#viewInfoPage .concentBoxUPS .el-form-item,#viewInfoPage .concentBoxCPE .el-form-item,#viewInfoPage .concentBoxGSM .el-form-item{
	margin-left:20px;
	margin-bottom: 10px;
}
#viewInfoPage .el-form{
	width:100%;
}
#viewInfoPage .el-form-pagrid-container .el-form{
	width: 425px;
}
#viewInfoPage .el-form-item__content{
	width:50%;
	min-width:500px;
	white-space: nowrap;
}
#viewInfoPage .el-form-table-container .el-form-item__content{
	width:100%;
}
#viewInfoPage .el-form-pagrid-container .el-form-item__content{
	width:98%;
}
#viewInfoPage .el-ctable-toolbar .el-input__inner{
	border:none; 
}
.queryInfo{
	display:inline-block;
...
#viewInfoPage .queryInfo .el-input__inner{
...
#viewInfoPage .templateNameStys div:first-child{
	overflow: hidden;
}
#viewInfoPage .templateNameStys .el-form-item__error{
	font-size: 12px;
    line-height: 1;
    padding-top: 0px;
    position: absolute;
...
#viewInfoPage .pairgrid-left  div:first-child .el-ctable{
...
.emailPromptOne .el-icon:before,.emailPromptTwo .el-icon:before{
	color: #BBBBBB;
	font-size: 12px;
}
#viewInfoPage .ContentClass{
	padding-bottom: 20px;
}
.alarmBottomLine{
...
#viewInfoPage .el-ctable .el-query .advanceQuery .el-input__inner{
	padding-left: 0px;
}
#viewInfoPage .allAlarmClass{
	position: absolute;
	left: 80px;
	top:2px;
}
#viewInfoPage .transition-box{
	z-index: 102!important;
}
#viewInfoPage .el-tabs__content {
...
<div id="viewInfoPage">
	<el-form :model="ruleForm" :rules="rules" ref = "ruleForm" label-position="left" label-width="164" :hide-required-asterisk='true'>
		<div class="ContentClass">
			...
			<el-form-item label="<%=rb.getString("MuBanMingCheng")%>" prop="templateName" class="templateNameStys">
				<el-input ref="" :disabled="viewFlag" v-model="ruleForm.templateName" style="width:400px;padding-top:5px;"></el-input>
				<div class="emailPromptOne"><span class="el-icon-circle-info el-icon" style="margin-right:10px;" ></span><span><%=rb.getString("MingChengChangDu")%>1~50</span></div>
			</el-form-item>
			<el-form-item label="<%=rb.getString("MiaoShu")%>" prop="desc"> 
				<el-input type="textarea" resize="none" :rows="3" ref="" :disabled="viewFlag" maxlength="500" v-model="ruleForm.desc" style="width:560px;"></el-input>
			</el-form-item>
		</div>
		...
		<div class="ContentClass">
			...
			<!-- 设备类型 -->
			<el-form-item prop='deviceTypeList' label='<%=rb.getString("XinGaoJingYuan")%>'>
			...
			<el-tabs v-model="alarmTabsValue" type="card" :closable="!viewFlag" @tab-remove="removeTab" style="margin:0px 34px 10px 34px;"  v-show="alarmTabsShow == true">
				<el-tab-pane label="ENB" name="ENB"  v-if="deviceTypeShowENB == true">
					<div class="concentBoxENB">
						...
						<el-form-item label-width="0" class="el-form-table-container" v-show="ruleForm.enbDeviceByDeviceGroup == '1'" style="height:300px;">
							<el-ctable key="enbt"  
								@selection-change='enbDeviceGroupSelect' 
								@load-success="tableLoadSuccess('enbDeviceGroup')" border=true 
								:readonly='viewFlag || ruleForm.enbAllAndCustomize == 0' ref="deviceGroupTable" 
								:default-checked="enbDeviceChecked"
								row-key="id" 
								:query-params="queryDeviceForm" :url="deviceGroupUrl" style="width:60%;height:100%;border:1px solid #E9E9E9;">
				...
				<el-tab-pane label="UPS" name="UPS"  v-if="deviceTypeShowUPS == true">
					<div class="concentBoxUPS">
						...
						<el-form-item label-width="0" class="el-form-table-container" v-show="ruleForm.upsDeviceByDeviceGroup == '1' " style="height:300px;">
							<el-ctable key="upst"
								@selection-change='upsDeviceGroupSelect' 
								@load-success="tableLoadSuccess('upsDeviceGroup')" border=true 
								:readonly='viewFlag || ruleForm.upsAllAndCustomize == 0' ref="upsDeviceGroupTable" 
								:default-checked="upsDeviceChecked"
								row-key="id" 
								:query-params="upsQueryDeviceForm" :url="upsDeviceGroupUrl" style="width:60%;height:100%;border:1px solid #E9E9E9;">
								<el-table-column type="selection" reserve-selection width="55" key="upss"></el-table-column>
								<el-table-column prop="group_name" label="<%=rb.getString("SheBeiZuMingCheng") %>"></el-table-column>
							</el-ctable>
						</el-form-item>
						...
						<el-form-item label-width="0" v-show="ruleForm.upsDeviceByDeviceGroup != '1'" class="el-form-pagrid-container"   key="upsTable">
							<el-pairgrid @selection-change='upsSelectChange' @right-load-success="tableLoadSuccess('upsDevice')" ref="cpairgridUPS" query-name="serial_number" :rownumber="true" :page-list="pageList" :right-url="upsRightUrl" :left-url="upsLeftUrl" :height="height" :readonly='viewFlag' row-key="serial_number" :query-params="queryFormUPS" :title="deviceTitle" :messages="{placeholder:'<%=rb.getString("DianYuanBianMa")%>'}">
								<template slot="prev">
									<el-ctable ref="upsGroupFiltor" style="width:250px;height:100%;" :show-pager="false" row-key="id" :url="upsDeviceGroupUrlAll" rownumber="true" :page-size="pageSize" pagination="false" :page-list="pageList" :query-params="upsQueryDeviceForm"
				...
				<el-tab-pane label="CPE" name="CPE"  v-if="deviceTypeShowCPE == true">
					<div class="concentBoxCPE">
						...
						<el-form-item label-width="0" class="el-form-table-container" v-show="ruleForm.cpeDeviceByDeviceGroup == '1'" style="height:300px;">
							<el-ctable key="cpet"
								@selection-change='cpeDeviceGroupSelect'
								@load-success="tableLoadSuccess('cpeDeviceGroup')" :border="true"
								:readonly='viewFlag || ruleForm.cpeAllAndCustomize == 0' ref="cpeDeviceGroupTable" 
								:default-checked="cpeDeviceChecked"
								row-key="id" 
								:query-params="cpeQueryDeviceForm" :url="cpeDeviceGroupUrl" style="width:60%;height:100%;border:1px solid #E9E9E9;">
								<el-table-column type="selection" reserve-selection width="55" key="cpes"></el-table-column>
								<el-table-column prop="group_name" label="<%=rb.getString("SheBeiZuMingCheng") %>"></el-table-column>
							</el-ctable>
						</el-form-item>
						...
						<el-form-item label-width="0" v-show="ruleForm.cpeDeviceByDeviceGroup != '1' " class="el-form-pagrid-container"    key="cpeTable">
							<el-pairgrid @selection-change='cpeSelectChange' @right-load-success="tableLoadSuccess('cpeDevice')" ref="cpairgridCPE" query-name="macaddress" :rownumber="true" :page-list="pageList" :right-url="cpeRightUrl" :left-url="cpeLeftUrl" :height="height" :readonly='viewFlag' row-key="cpe_code" :query-params="queryFormCPE" :title="deviceTitle" :messages="{placeholder:'<%=rb.getString("MACDiZhi")%>'}">
								<template slot="prev">
									<el-ctable ref="cpeGroupFiltor" style="width:250px;height:100%;" :show-pager="false" row-key="id" :url="cpeDeviceGroupUrlAll" rownumber="true" :page-size="pageSize" pagination="false" :page-list="pageList" :query-params="cpeQueryDeviceForm"
				...
				<el-tab-pane label="GNB" name="GNB"  v-if="deviceTypeShowGNB == true">
					<div class="concentBoxCPE">
						...
						<el-form-item label-width="0" class="el-form-table-container" v-show="ruleForm.gnbDeviceByDeviceGroup == '1'" style="height:300px;">
							<el-ctable key="gpet"
								@selection-change='gnbDeviceGroupSelect'
								@load-success="tableLoadSuccess('gnbDeviceGroup')" :border="true"
								:readonly='viewFlag || ruleForm.gnbAllAndCustomize == 0' ref="gnbDeviceGroupTable" 
								:default-checked="gnbDeviceChecked"
								row-key="id" 
								:query-params="gnbQueryDeviceForm" :url="gnbDeviceGroupUrl" style="width:60%;height:100%;border:1px solid #E9E9E9;">
								<el-table-column type="selection" reserve-selection width="55" key="gnbs"></el-table-column>
								<el-table-column prop="group_name" label="<%=rb.getString("SheBeiZuMingCheng") %>"></el-table-column>
							</el-ctable>
						</el-form-item>
						...
						<el-form-item label-width="0" v-show="ruleForm.gnbDeviceByDeviceGroup != '1' " class="el-form-pagrid-container"  key="gnbTable">
							<el-pairgrid @selection-change='gnbSelectChange' @right-load-success="tableLoadSuccess('gnbDevice')" ref="cpairgridGNB" query-name="serial_number" :rownumber="true" :page-list="pageList" :right-url="gnbRightUrl" :left-url="gnbLeftUrl" :height="height" :readonly='viewFlag' row-key="small_cell_code" :query-params="queryFormGNB" :title="deviceTitle" :messages="{placeholder:'<%=rb.getString("Title_SheBeiBianMa")%>'}">
								<template slot="prev">
									<el-ctable ref="gnbGroupFiltor" style="width:250px;height:100%;" :show-pager="false" row-key="id" :url="gnbDeviceGroupUrlAll" rownumber="true" :page-size="pageSize" pagination="false" :page-list="pageList" :query-params="gnbQueryDeviceForm"
					...
					<div class="concentBoxCPE">
						...
						<el-form-item label-width="0" class="el-form-table-container" v-show="ruleForm.egwDeviceByDeviceGroup == '1'" style="height:300px;">
							<el-ctable key="gpet"
								...
								row-key="id" 
								:query-params="egwQueryDeviceForm" :url="egwDeviceGroupUrl" style="width:60%;height:100%;border:1px solid #E9E9E9;">
						...
						<el-form-item label-width="0" v-show="ruleForm.egwDeviceByDeviceGroup != '1' " class="el-form-pagrid-container"  key="egwTable">
							<el-pairgrid @selection-change='egwSelectChange' @right-load-success="tableLoadSuccess('egwDevice')" ref="cpairgridEGW" query-name="serial_number" :rownumber="true" :page-list="pageList" :right-url="egwRightUrl" :left-url="egwLeftUrl" :height="height" :readonly='viewFlag' row-key="egw_code" :query-params="queryFormEGW" :title="deviceTitle" :messages="{placeholder:'<%=rb.getString("eGWBianMa")%>'}">
								<template slot="prev">
									<el-ctable ref="egwGroupFiltor" style="width:250px;height:100%;" :show-pager="false" row-key="id" :url="egwDeviceGroupUrlAll" rownumber="true" :page-size="pageSize" pagination="false" :page-list="pageList" :query-params="egwQueryDeviceForm"
				...
				<el-tab-pane label="GSM" name="GSM"  v-if="deviceTypeShowGSM == true">
					<div class="concentBoxGSM">
						...
						<el-form-item label-width="0" class="el-form-table-container" v-show="ruleForm.gsmDeviceByDeviceGroup == '1'" style="height:300px;">
							<el-ctable key="gsmt"  
								@selection-change='gsmDeviceGroupSelect' 
								@load-success="tableLoadSuccess('gsmDeviceGroup')" border=true 
								:readonly='viewFlag || ruleForm.gsmAllAndCustomize == 0' ref="gsmDeviceGroupTable" 
								:default-checked="gsmDeviceChecked"
								row-key="id" 
								:query-params="gsmQueryDeviceForm" :url="gsmDeviceGroupUrl" style="width:60%;height:100%;border:1px solid #E9E9E9;">
								<el-table-column type="selection" reserve-selection width="55" key="gsms"></el-table-column>
								<el-table-column prop="group_name" label="<%=rb.getString("SheBeiZuMingCheng") %>"></el-table-column>
							</el-ctable>
						</el-form-item>
						<!-- 基站设备列表  -->
						<el-form-item label-width="0" v-show="ruleForm.gsmDeviceByDeviceGroup != '1' " class="el-form-pagrid-container"   key="gsmTable">
							<el-pairgrid @selection-change='gsmSelectChange' @right-load-success="tableLoadSuccess('gsmDevice')" ref="cpairgridGSM" query-name="serial_number" :rownumber="true" :page-list="pageList" :right-url="gsmRightUrl" :left-url="gsmLeftUrl" :height="height" :readonly='viewFlag' row-key="small_cell_code" :query-params="queryFormGSM" :title="deviceTitle" :messages="{placeholder:'<%=rb.getString("XiaoZhanBianMa")%>'}">
								<template slot="prev">
									<el-ctable ref="gsmGroupFiltor" style="width:250px;height:100%;" :show-pager="false" row-key="id" :url="gsmDeviceGroupUrlAll" rownumber="true" :page-size="pageSize" pagination="false" :page-list="pageList" :query-params="gsmQueryDeviceForm"
			...
			<!-- 告警列表  -->
			<el-form-item label-width="0" class="el-form-pagrid-container" style="margin-left:34px;margin-bottom:0px;position:relative;">
				...
				<div v-if="ruleForm.allAlarm == 1" style="font-size:14px;"><%=rb.getString("XuanZeGaoJing")%></div>
			...
			<el-form-item prop='alarmId' style="margin-left:45px;" label-width="0">
		...
		<div class="ContentClass">
			<!-- 邮件通知 -->
			<div class="headContent">
				<img src="${ctx}/css/images/global/settingBetter.png"><%=rb.getString("XinJianGaoJingTongZhi")%>
				<el-switch 
					v-model="ruleForm.emailEnable" 
					:active-value="1" 
					:inactive-value="0" 
					:disabled='viewFlag'  
				...
				></el-switch>
				<div class="emailPromptTwo" style="display:inline-block;margin-left:10px;">
					<span v-show="emailEnableTiShi == '2' || emailEnableTiShi == '3'" class="el-icon-circle-info el-icon" style="margin-right:10px;" ></span>
			...
			<el-form-item  prop="emailPeriod" label="<%=rb.getString("XinJianGaoJingShiJianJianGe")%>"  class="intervalStyle">
				<el-radio-group v-model="ruleForm.emailPeriod" style="padding-top:12px;" :disabled="viewFlag">
					<el-radio :label="5">Real Time</el-radio>
					<el-radio :label="10">10 Minute</el-radio>
					<el-radio :label="30">30 Minute</el-radio>
					<el-radio :label="60">60 Minute</el-radio>
				</el-radio-group>
			</el-form-item>
			<el-form-item  prop="remainTime" label="<%=rb.getString("RongRenShiChang")%>"  class="intervalStyle">
				<el-radio-group v-model="ruleForm.remainTime" style="padding-top:12px;" :disabled="viewFlag">
					<el-radio :label="5">Real Time</el-radio>
					<el-radio :label="10">10 Minute</el-radio>
					<el-radio :label="30">30 Minute</el-radio>
					<el-radio :label="60">60 Minute</el-radio>
			...
			<el-form-item  prop="emailAddress"  label="<%=rb.getString("YouXiang")%>">
				<el-input type="textarea" resize="none" :rows="3" ref="" :disabled="viewFlag" maxlength="500" v-model="ruleForm.emailAddress" style="width: 560px;"></el-input>
			</el-form-item>
			<el-form-item   label="" style="margin-bottom:0px;">
				<div class="emailPromptTwo"><span class="el-icon-circle-info el-icon" style="margin-right:10px;" ></span><%=rb.getString("YouXiangDiZhiTiShi")%></div>
			</el-form-item>
			<el-form-item  prop="" label="">
				<el-checkbox v-model="ruleForm.globalEmailEnable" true-label="1" false-label="0" :disabled='viewFlag'><%=rb.getString("TongZhiYouJianZu")%></el-checkbox>
			</el-form-item>
		</div>
	</el-form>
</div>
...
if(window.alarmTempalePage) {
	try {
		window.alarmTempalePage.$destroy();
	}catch(e){}
}
window.alarmTempalePage = new Vue({
	el:'#viewInfoPage',
	data(){
		var validatorTemplateName = (rule,value,callback) => { //  监测模板名称输入框 
			...
			if(value == ""){
				...
				//判断任务名称是否已经存在
				axios.post("${ctx}/fault/viewConfig/templateNameExist.action",stringify({
					templateName : this.ruleForm.templateName.trim(),
					oldTemplateName : this.oldTemplateName
				})).then((response) => {
		...
		return{
			...
			rules:{
				templateName:[
					{validator:validatorTemplateName,trigger:'blur'}
				],
				deviceTypeList:[
	...
	watch:{
		...
		"ruleForm.deviceType":function(newValue,oldValue){
			...
			if(this.actionType == 'edit' ){
				this.alarmRightUrl = "${ctx}/fault/viewConfig/queryViewIdentifierById.action?templateId="+this.templateId+"&deviceType="+newValue;
			}
		},
	...
	methods:{
		...
		*/
		...
		/**
		* 右上角关闭按钮
		* @param flag{boolean}  关闭新建页面
		*/
		closeAddViewTemple(){ 
			//发送数据
			var vm = this;
			eventBus.$emit("close-add")
		},
		/**
		* enb设备组单行点击事件
		...
		*/
		...
		submit(){  
			var vm = this,urls;
            if(alarmViewVue.viewAddSubmitLoading)return
			vm.goSubmit();
		},
		// 新增提交保存
		goSubmit(){
			...
			vm.$refs["ruleForm"].validate( valid => {
				if(valid){
					...
					if(vm.actionType == 'add'){
						urls = "${ctx}/fault/viewConfig/addViewConfig.action";
					}else if(vm.actionType == 'edit'){
						urls = "${ctx}/fault/viewConfig/updateViewConfig.action";
                    ...
                    alarmViewVue.viewAddSubmitLoading = true;
					axios.post(urls,stringify(params)).then(function(response){
						if(response.data.success){
                            ...
                            vm.closeAddViewTemple();
						}else{
							vm.$message({
								message: response.data.message,
								type:'error',
							});
                            alarmViewVue.viewAddSubmitLoading = false;
						}
		 			})
				}
			}) 
		...
		*/
		initTask(curTempId,timeZone,type){  
			...
			axios.post("${ctx}/fault/viewConfig/queryViewInfoById.action",stringify({templateId:curTempId,timeZone:timeZone})).then(function(response){
				var data = response.data;
				vm.oldTemplateName = data.templateName;
				Object.assign(vm.ruleForm,response.data);
				
				if(vm.ruleForm.emailPeriod == null){
					vm.ruleForm.emailPeriod = 10;
				}
				if(vm.ruleForm.remainTime == null){
					vm.ruleForm.remainTime = 10;
				}
				if(data.enbGroupId){
				...
				vm.alarmRightUrl = "${ctx}/fault/viewConfig/queryViewIdentifierById.action?templateId="+curTempId+"&deviceType="+data.deviceType
				if( type == "edit"){
					initForm(vm.$refs.ruleForm);
				}
	...
	mounted(){
		...
		eventBus.$off('alarm-add-cancel').$on('alarm-add-cancel',this.cancel);
		eventBus.$off('action-init').$on('action-init',this.initAlarmType);
	},
})
