...
<style>
#addPatchTask .el-form-item__label{
	line-height:26px;
	width:140px;
	text-align:left;
}
#addPatchTask .modeItem .el-radio{
	display:block;
	margin-left:0px;
	margin-bottom:20px;
}
#addPatchTask .modeItem{
	margin-top:20px;
}
#addPatchTask .el-select .el-input.is-disabled .el-input__inner{
	min-height:26px;
	max-height:26px;
}
#addPatchTask .el-form-item{
	margin-left:55px;
}
</style>
...
new Vue({
	...
	watch:{
		...
		"ruleForm.product":function(newVal){
			...
			if(vm.showPairGrid){
				vm.fileUrl = '${ctx}/cell/version/queryfileInfosList.action?file_type=1'
				vm.$refs.ctable.refresh();
			}else{
				vm.fileUrl = ''
				axios.post('${ctx}/cell/version/queryfileInfosList.action?file_type=1',stringify({
					timeZone:timeZone,
