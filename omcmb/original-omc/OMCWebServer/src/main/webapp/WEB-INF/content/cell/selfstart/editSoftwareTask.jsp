	...
	<div id="step_1" class="easyui-layout" data-options="border:false,fit:true">
		<div region="center" data-options="border:false" style="padding: 20px;">
			<div class="easyui-layout" data-options="border:false,fit:true">
				...
				<div region="south" id="editAutoSoftUpgradeNorth" data-options="height:110,border:false,collapsible:false" style="padding-top: 10px;">
					<div class="easyui-panel" data-options="fit:true,border:true,title:'<%=rb.getString("XuanZeZhiXingFangShi")%>'" style="padding: 15px 25px;">
						<div>
							...
							<select id="editUpgradeRawModeFlag" class="border border-box"
								style="height:26px;margin-left:15px;width: 180px">
...
$(function() {
	...
	var rawMode = "";
 	...
 	if (detail.originalVerList) {
		...
		$("#editUpgradeRawModeFlag").val(rawMode);
	} 
	...
	$("#gridOriSoftwareVer").datagrid({
		...
		onCheckAll:checkAll_oriSofteare,
        onBeforeCheck:beforeCheck_oriSoftware,
	...
	$("#gridFiles_editSoftwareAutoTask").datagrid({
		...
		queryParams:{timeZone:timeZone,"isAll":true},
		fitColumns: false,
		fit: true,
		idField: 'id',
		border: true,
...
function loadSucc_oriSoftwareVerDatagrid(){
					...
					if (oriVerList.length != 0) {
						for (var i = 0; i < oriVerList.length; i++) {
							...
							if (rowIndex >= 0) {
								...
								$(this).datagrid("setRow",{ver_name:oriVerList[i],usable:false});
							}
						}
					}
				}
	...
	if (!currVerData || currVerData.length == 0) {
		...
		for (var i = 0; i < currVerData.length; i++) {
			...
			if (rowIndex >= 0) {
				//$(this).datagrid("deleteRow", rowIndex);
				$(this).datagrid("setRow",{ver_name:currVerData[i].value,usable:false});
			...
			if (selVer[selVerCount]["value"]) {
        		...
        		for(var count = 0; count < delVerObjArr.length; count++) {
        			if (selVer[selVerCount].value == delVerObjArr[count]["ver_name"]) {
            			...
            			$("#gridOriSoftwareVer").datagrid("setRow",{ver_name:selVer[selVerCount].value,usable:true});
...
function addSelectedOriSoftwareVer() {
	...
	if (newSelectdArr.length > 0) {
		...
		for(var count = 0; count < newSelectdArr.length; count++) {
			...
			$("#gridOriSoftwareVer").datagrid("setRow",{ver_name:newSelectdArr[count]["ver_name"],usable:false});
		}
	}
...
function saveSoftwareAutoTask() {
	...
	// 是否保留配置
	params["rawMode"] = $("#editUpgradeRawModeFlag").val();
