...
<style>
	...
	.freq-item-suffix .index-item {
		 display: inline-block;
		 height: 24px;
		 width: 20px;
		 border-right: 1px solid #A8C9FA;
		 text-align: center;
		 line-height: 24px;
	 }
	 .grouAndIdContainer{
	...
	.prover-content span{
		...
		border-style:solid;
		border-color:transparent transparent #4D84FF;
		top:-22px;
		left:48%
	}
	.prover-content em{
		...
		border-width:0 10px 10px;
		border-style:solid;
		border-color:transparent transparent #F5F8FF;
		top:1px;
		margin-left:-10px
	}
	.prover-content p{
	...
	.preferredFrequencyInputCls .el-input{
		width: 80px;
	}
	.pr{
		position:relative
	...
	.parameterBox .antennaConfigurationBoxCls .el-form-item__label{
		width:180px
	}
	.modelInstalInput{
		display:inline-block;
	...
	.onlyFollowSelectCls .el-form-item__content .el-checkbox{
		margin-top:7px;
	}
	.onlyFollowSelectCls .el-form-item__label{
		line-height:28px;
		margin-top:6px;
	}
	.preferredCls{
	...
	<div class="sasSettingMainBox">
		<div v-show="!showWindowInfo">
			<div class="container">
				...
				<div class="headerCls">
					...
					<div class="selectBox">
						<div v-if="selectBoxShow" :class="{btnSelectCls:carrierType == 'pcell'}" @click="selectTabClick('pcell')">Cell 1</div>
						<div v-if="selectBoxShow || isTC" :class="{btnSelectCls:carrierType == 'scell'}" @click="selectTabClick('scell')">Cell 2</div>
						<div v-if="isTC" :class="{btnSelectCls:carrierType == 'tcell'}" @click="selectTabClick('tcell')">Cell 3</div>
					</div>
						...
						<el-form :model="ruleForm" :rules="rules" ref="ruleForm" :disabled="disableForm" label-position='left'>
							<div class="mainBox">
								<div class="preferredSettingCls boxBorderCls">
									...
									<div>
										<el-form-item  label="Frequency Selection Logic" prop="frequencySelectionLogic" required  label-width='180px'>
											<el-select v-model="ruleForm.frequencySelectionLogic">
												<el-option v-for="(item,index) in selectLoginArr" :key="item.value" :value="item.value" :label="item.label"></el-option>
											</el-select>
										</el-form-item>
										<el-form-item v-if="showWidebandMode" label="Wideband Mode" prop="widebandModeEnable" label-width='180px'>
											<el-select v-model="ruleForm.widebandModeEnable" @change="widebandModeEnableChange">
												<el-option label="Enable" value="1"></el-option>
												<el-option label="Disable" value="0"></el-option>
											</el-select>
										</el-form-item>
										<el-form-item v-if="!is5GType"  label="Legacy Mode" prop="legacyMode" required  label-width='180px'>
											<el-select v-model="ruleForm.legacyMode" class="legacy">
												<el-option label="Enable" value="true"></el-option>
												<el-option label="Disable" value="false"></el-option>
											</el-select>
										</el-form-item>
										<el-form-item label="Preferred Bandwidth" prop="preferredBandwidth" required label-width='180px'>
											<el-select v-model="ruleForm.preferredBandwidth">
												<el-option label="100M" value="n500" v-if="showHighBandwidth"></el-option>
												<el-option label="90M" value="n450" v-if="showHighBandwidth"></el-option>
												<el-option label="80M" value="n400" v-if="showHighBandwidth"></el-option>
												<!-- <el-option label="70M" value="n350" v-if="showHighBandwidth"></el-option> -->
												<el-option label="60M" value="n300" v-if="showHighBandwidth"></el-option>
												<el-option label="50M" value="n250" v-if="showHighBandwidth"></el-option>
												<el-option label="40M" value="n200" v-if='is5GType'></el-option>
												<el-option label="30M" value="n150" v-if='is5GType'></el-option>
												<el-option label="20M" value="n100"></el-option>
												<el-option label="15M" value="n75" v-if='!is15Disabled && !is5GType'></el-option>
												<el-option label="10M" value="n50" v-if='!is5and10Disabled'></el-option>
												<el-option label="5M" value="n25" v-if='!is5and10Disabled && !is5GType'></el-option>
											</el-select>
											<el-checkbox v-if="isQCA" v-model="ruleForm.isSymmetrical" true-label="true" false-label="false">Symmetrical</el-checkbox>
										</el-form-item>
										<el-form-item v-if="ruleForm.isSymmetrical == 'false'" label="Preferred Bandwidth(Scell)" prop="preferredBandwidthNoP" required label-width='180px'>
											<el-select v-model="ruleForm.preferredBandwidthNoP">
												<el-option label="20M" value="n100"></el-option>
												<el-option label="15M" value="n75" ></el-option>
												<el-option label="10M" value="n50"></el-option>
												<el-option label="5M" value="n25"></el-option>
											</el-select>
										</el-form-item>
										<el-form-item label="Preferred Power" required prop="preferredPower" label-width='180px' :class="ruleForm.isIntelCR == 'true' ? 'preferredPowerItemCls' : ''">
											<div style="display:inline-block;" v-show="ruleForm.isIntelCR == 'true'">
												<el-select v-model="ruleForm.antennaPortsCount" >
											...
											<el-select v-model="ruleForm.preferredPower" class="legacy">
												<el-option v-for="(item,index) in preferredPowerList" :key="item.value" :value="item.value" :label="item.text"></el-option>
											</el-select>
										</el-form-item>
										<el-form-item label-width='180px' label="Channel Reuse" prop="channelReuse"  v-show='isDC == true'>
											<el-select v-model="ruleForm.channelReuse">
												<el-option label="Enable" value="true"></el-option>
												<el-option label="Disable" value="false"></el-option>
											</el-select>
										</el-form-item>
										<el-form-item label="Contiguous" label-width='180px' v-show="isQCA === true">
											<el-checkbox
												v-model="ruleForm.contiguous" 
												true-label="true" 
												false-label="false"
												@change="contiguousChange">
											</el-checkbox>
										</el-form-item>
										<el-form-item label="Preferred Frequency" label-width='180px' class="preferredFrequencyInputCls">
											<div style="display: flex;">
												<span class="prefix-label" v-show="isMultiArea || isTC">
													Cell 1
												</span>
												<el-select :class="freqSelectCls" v-model="freqLowSelect" @change="freqSelectChange">
													<el-option v-for="item in freqRange" :label="item.label" :value="item.value"></el-option>
												</el-select> 
												<span v-show="isMultiArea || isTC" class="prefix-label" style="margin-left: 10px;">
													Cell 2
												</span>
												<el-select v-show="isMultiArea || isTC" v-model="freqHighSelect" :class="freqSelectCls" @change="freqSelectChange">
													<el-option v-for="item in freqRangeCA" :label="item.label" :value="item.value"></el-option>
												</el-select>
												<span v-show="isTC" class="prefix-label" style="margin-left: 10px;">
													Cell 3
												</span>
												<el-select v-show="isTC" v-model="freqTcValSelect" :class="freqSelectCls" @change="freqSelectChange">
													<el-option v-for="item in freqRange" :label="item.label" :value="item.value"></el-option>
												</el-select> 
												<i v-if="!disableForm" class="el-icon el-icon-plus" style="margin-left: 15px; margin-top: 5px;" @click="addFreq"></i>
												<span style="padding: 0 5px;"> (Limit of 15) </span>

											</div>

											<div class="freq-item">
												<span v-for="(item,index) in preferredFreqList" :key="index" 
													draggable="true"
													@dragstart="preferredFreqDrag($event,index)" 
													@drop="preferredFreqDrop($event,index)"
													@dragover="allowDrop($event)" 
													class="freq-item-suffix"
												>
													<div style="height: 26px;">
													<span class="index-item">{{index+1}}</span>
													<span class="num-item">
														<span>{{item.low?item.low:'Auto'}}</span>
														<span v-show="isMultiArea || isTC">: {{item.high?item.high:'Auto'}}</span>
														<span v-show="isTC">: {{item.tcVal?item.tcVal:'Auto'}}</span>
													</span>
													<i v-if="!disableForm" class="el-icon el-icon-operation-delete" @click="deleteFreq(index)"></i>
													</div>
												</span>
											</div>

											<el-form-item ref="freqHide" label="" prop="preferredFrequencyText" class="preferredCls">
												<el-input v-show="false" v-model="ruleForm.preferredFrequencyText" ></el-input>
												<div v-show="false" class="proverBox">
													<el-popover trigger="click" placement="bottom">
														<div class="prover-content" >
															<span><em></em></span>
															<p>The format of Preferred Frequency should be(xxxx,xxxx)or</p>
															<p>(xxxx:xxxx,xxxx:xxxx)</p>
															<p>Explanation:</p>
															<p>if Legacy Mode is False,frequency should be in Band48 3550-3700(MHz).</p>
															<p>if Legacy Mode is True,frequency should be in Band42 3550-3600(MHz) or</p>
															<p>Band43 3600-3700(MHz).</p>
															<p>Multi Preferred frequency are separated by(,),PCell&SCell are separated</p>
															<p>by(:)and in same Band.</p>
															<p>Example:</p>
															<p>eNB(SC):3550,3570</p>
															<p>eNB(CA):3560:3570</p>
															<p>eNB(DC):3560:3570,3570:3580</p>
														</div>
														<span slot="reference" class="el-icon el-icon-circle-info"></span>
													</el-popover>
												</div>
											</el-form-item>
										</el-form-item>
										<el-form-item label="<%=rb.getString("ZhiNengShiYongPreferFrequencyZhongDePinLv")%>" label-width='180px' prop="onlyFollowSelectFreq" class="onlyFollowSelectCls">
											<el-checkbox v-model="ruleForm.onlyFollowSelectFreq" :true-label="'true'" :false-label="'false'"></el-checkbox>						
										</el-form-item>
									</div>
								</div>
								<div class="basicInfoAndAirAndGroupCls">
									<div class="basicInfoCls boxBorderCls">
										...
										<el-form-item label="FCC ID" prop="fccId" required label-width='130px'>
											<el-input ref="fccid" placeholder="length：0 - 19" :disabled="fccIdIsNoMod"  v-model="ruleForm.fccId" @change="fccIdChange"  class="" maxlength='19'></el-input>
										</el-form-item>
										...
										<el-form-item label="Cell Name" prop="cellName" label-width='130px'>
											<el-input ref="cellname" placeholder="<%=rb.getString("ChangDuJiaoYan")%>0-48" v-model="ruleForm.cellName"></el-input>
										</el-form-item>
										...
										<el-form-item label="Antenna Gain2" prop="antennaGain2" label-width='130px' v-if="(isDC && ruleForm.antennaConfiguration == '2') || isTC">
											<el-input ref="antennaGain2" v-model="ruleForm.antennaGain2" :disabled="hasCode"></el-input>
										</el-form-item>
										<el-form-item label="Antenna Gain3" prop="antennaGain3" label-width='130px' v-if="isTC">
											<el-input ref="antennaGain3" v-model="ruleForm.antennaGain3" :disabled="hasCode"></el-input>
										</el-form-item>
									</div>
									<div class="airAndGroupCls">
										...
										<div class="groupCls boxBorderCls">
											...
											<el-form-item  label="Group Type " prop="groupType" label-width='120px'>
											...
											<el-form-item  label="Group ID" prop="groupId" label-width='120px'>
												...
												<div class="noUserIdErrBoxCls" v-if="noUserIdErrShow">
													...
													<!--<a href="#"  @click.prevent="goSetting"><%=rb.getString("QuSheZhi")%></a>-->
												</div>
											</el-form-item>
							...
							<div class="installationParameterCls boxBorderCls">
								<div class="group-title not-extend" style="margin:20px 0px;padding-left:20px;">
									...
									<!--<div class="prompt_one">
										<span class="el-icon el-icon-circle-info" style="margin-left:20px;"></span>
										<span>When the installation parameters are filled in and a CPI Signature is generated below, Single-Step registration will be used. If no CPI signature is generated, then Multi-Step registration will be used instead.</span>
									</div>-->
								</div>
		...
		<div v-show="showWindowInfo">
			<el-form :model="ruleFormParameter" :rules="rulesParameter" ref="ruleFormParameter" label-position='top' style="padding-bottom:50px;" class="dilogBox parameterBox">
				...
				<div class="ml30 antennaConfigurationBoxCls" v-if="isDC == true">
					<el-form-item label-width='180px' style="margin:10px 0px;" label="Antenna Configuration" prop="antennaConfiguration" >
						<el-select v-model="ruleFormParameter.antennaConfiguration" @change="antennaConfigurationChange">
							<el-option label="An antenna" value="1"></el-option>
							<el-option label="Two antenna" value="2"></el-option>
						</el-select>
					</el-form-item>
				</div>
...
var sasSettingsVue = new Vue({
	...
	data(){
		...
		var isValidFormatValue = function(val) {
				var list = (val||'').split(','),
					numReg =  /^(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)$/,
					// 格式 eg: 3550:3600,3650,3675:3700:3700
					formatReg = /^(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)(:(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)){0,2}(,(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)(:(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)){0,2})*$/,
					bool = true;

				if(formatReg.test(val) && list.length<=15) {
					list.map(function(item){
						if(item) {
							var arr = item.split(':');
							arr.map(function(value){
								if(numReg.test(value) && value%2.5 == 0) {
			...
			validTC = function(legacymode,value,callback) {
				var 
					// 格式 3550:3650 [,3650:3660]* -- /^reg(,reg)*$/
					// reg([3550-3700]:[]3550-3700]) + (,reg([3550-3700]:[]3550-3700]))*
					numReg = /^((35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)(:(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)){1,2})(,(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)(:(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)){1,2})*$/,
					reg35to36 = /^((35[5-9]\d(\.5)?|3600):(35[5-9]\d(\.5)?|3600):(35[5-9]\d(\.5)?|3600))$/,
					reg36to37 = /^((36\d{2}(\.5)?|3700):(36\d{2}(\.5)?|3700):(36\d{2}(\.5)?|3700))$/,
					bandwidth = vm.ruleForm.preferredBandwidth,
					valid = true;
				// 范围值合法输入 eg：3550:3655
				if(numReg.test(value)) {
					...
					list.map(function(item){
						...
						if(legacymode) {
							// 匹配同频段
							if(reg35to36.test(item) || reg36to37.test(item)) {
							}else {
								valid = false;
							}
							// legacymode: true -- band42(prefer freq + prefer bandwidth) <= 3600) ; badd43(prefer freq - prefer bandwidth) >= 3600)
							vals.map(function(val){
								if(val - 3600 <=0 && val*1 + bandwidth*1 > 3600) {
									valid = false;
									callback('Prefer freq + Prefer bandwidth must no more than 3600');
								}
								if(val - 3600 >= 0 && val - bandwidth < 3600) {
									valid = false;
									callback('Prefer freq - Prefer bandwidth must greater than 3600');
								}
							});
						}
					})
				...
				valid && callback();
				!valid && legacymode && callback('Range:3550-3600 or 3600-3700(MHz), eg:3550:3560:3590 or 3600:3680:3690');
				!valid && !legacymode && callback('Range:3550-3700(MHz), eg:3550:3680:3700');
			},
			validDC = function(legacymode,value,callback) {
				var 
					// 格式 3550:3650 [,3650:3660]* -- /^reg(,reg)*$/
					// reg([3550-3700]:[]3550-3700]) + (,reg([3550-3700]:[]3550-3700]))*
					numReg = /^((35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700):(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700))(,(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700):(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700))*$/,
					reg35to36 = /^((35[5-9]\d(\.5)?|3600):(35[5-9]\d(\.5)?|3600))$/,
					reg36to37 = /^((36\d{2}(\.5)?|3700):(36\d{2}(\.5)?|3700))$/,
					bandwidth = vm.ruleForm.preferredBandwidth,
					valid = true;
				// 范围值合法输入 eg：3550:3655
				if(numReg.test(value)) {
					...
					list.map(function(item){
						...
						// 起始值对比
						//if(vals[0] - vals[1] > 0) valid = false;
					})
				}else {
					valid = false;
				}
				...
				valid && callback();
				!valid && legacymode && callback('Range:3550-3600 or 3600-3700(MHz), eg:3550:3560 or 3600:3680');
				!valid && !legacymode && callback('Range:3550-3700(MHz), eg:3550:3680');
			},
			validCA = function(legacymode,value,callback) {
				var contiguous = [true, 'true'].includes(vm.ruleForm.contiguous),
					// 格式 3550:3650 [,3650:3660]* -- /^reg(,reg)*$/
					// reg([3550-3700]:[]3550-3700]) + (,reg([3550-3700]:[]3550-3700]))*
					singlenumReg =  /^((35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)(,(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700))*)$/,
					numReg = /^((35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700):(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700))(,(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700):(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700))*$/,
					reg35to36 = /^((35[5-9]\d(\.5)?|3600):(35[5-9]\d(\.5)?|3600))$/,
					reg36to37 = /^((36\d{2}(\.5)?|3700):(36\d{2}(\.5)?|3700))$/,
					bandwidth = vm.ruleForm.preferredBandwidth,
					bandwidth2 = vm.ruleForm.preferredBandwidthNoP,
					isSymmetrical = vm.ruleForm.isSymmetrical,
					bandwidthCodes={
						n25: 5,
						n50: 10,
						n75: 15,
					...
					valid = true;
				if(contiguous) {// 主小区
					if(singlenumReg.test(value)) {
						var list = value.split(',');
						list.map(function(item) {
							// legacymode为true
							if(legacymode) {
								if(['n25', 'n50'].includes(bandwidth) && isSymmetrical == 'true') {
									valid = false;
									callback('BandWidth must be 15M or 20M.');
								}else {
									// 15M 3600-3700校验
									if( ['n75'].includes(bandwidth) && (item - 3600 < 0)) {
										valid = false;
										callback('Range:3600-3700(MHz), eg:3600:3680');
									}

									// legacymode: true -- band42(prefer freq + prefer bandwidth) <= 3600) ; badd43(prefer freq - prefer bandwidth) >= 3600)
									if(item - 3600 <= 0 && item*1 + bandwidth*1 > 3600) {
										valid = false;
										callback('Prefer freq + Prefer bandwidth must no more than 3600');
									}
									if(item - 3600 >= 0 && item - bandwidth<3600) {
										valid = false;
										callback('Prefer freq - Prefer bandwidth must greater than 3600');
									}
								}
							}else {
								if(['n25', 'n50', 'n75'].includes(bandwidth) && isSymmetrical == 'true') {
									valid = false;
									callback('BandWidth must be 20M.');
								}
							}
						...
						valid = false;
						callback('Range:3550-3700(MHz), eg:3550,3680');
					}
				}else {
					if(numReg.test(value)) {
						var list = value.split(',');
						list.map(function(item){
							...
							if(legacymode) {
								// 15M 不能选
								if( ['n75'].includes(bandwidth) && !reg36to37.test(item) && isSymmetrical == 'true') {
									valid = false;
									callback('BandWidth must be: 5M、10M or 20M.');
								}
								// 5M、10M、20M  3550-3600校验
								if( ['n25','n50','n100'].includes(bandwidth) && !reg35to36.test(item)) {
									valid = false;
									callback('Range:3550-3600(MHz), eg:3550:3560');
								}
								// 5M、15M步长必须为 2.5
								if(['n25','n75'].includes(bandwidth) && !(vals[0].indexOf('.5')>=0 && vals[1].indexOf('.5')>=0) && isSymmetrical == 'true') {
									valid = false;
									callback('Step must be 2.5');
								}
								// 10M、20M步长必须为 5
								if(['n50','n100'].includes(bandwidth) && !(vals[0]%5==0 && vals[1]%5==0) && isSymmetrical == 'true') {
									valid = false;
									callback('Step must be 5');
								}

								// legacymode: true -- band42(prefer freq + prefer bandwidth) <= 3600) ; badd43(prefer freq - prefer bandwidth) >= 3600)
								vals.map(function(val,index){
									var theBandwidth = index == 1 ? bandwidth2 : bandwidth;
									if(val - 3600 <=0 && val*1 + theBandwidth*1 > 3600) {
										valid = false;
										callback('Prefer freq + Prefer bandwidth must no more than 3600');
									}
									if(val - 3600 >= 0 && val - theBandwidth < 3600) {
										valid = false;
										callback('Prefer freq - Prefer bandwidth must greater than 3600');
									}
								});
				...
				if(isSymmetrical == 'false'){
					var bandwidthStr = bandwidthCodes[bandwidth]+'+'+bandwidthCodes[bandwidth2];
					if(!['20+15','20+10','15+20','10+20'].includes(bandwidthStr)){
						valid = false;
						callback('BandWidth combination supports 20+15 20+10 15+20 10+20.');
					}
				}
				valid && callback();
			},
			validOther = function(legacymode,value,callback) {
				var numReg =  /^((35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700)(,(35[5-9]\d(\.5)?|36\d{2}(\.5)?|3700))*)$/,
					bandwidth = vm.ruleForm.preferredBandwidth,
					valid = true;

				if(!numReg.test(value)) valid = false;
				...
				valid && callback();
				!valid && callback('Range:3550-3700(MHz), eg:3550,3680');
		};
		var validateFrequency = function(rule,value,callback) {
			// isDC=true -- DC , isQCA =true -- CA
			var type = '';

			vm.isDC == true && (type = 'DC');
			vm.isQCA == true && (type = 'CA');
			vm.isTC == true && (type = 'TC');
			var validators = {
					DC: validDC,
					CA: validCA,
					TC: validTC
			...
			if(value){ 
				var legacymode = [true, 'true'].includes(vm.ruleForm.legacyMode);
				
				if(isValidFormatValue(value)) {// 合法输入
					validators[type]? validators[type](legacymode,value,callback) : validOther(legacymode,value,callback);
				}else {// 非法输入
					callback('<%=rb.getString("SASPinLvCuoWuTiShi")%>');
				}
			}else{
				if(type == 'CA'){
					var bandwidth = vm.ruleForm.preferredBandwidth,
						bandwidth2 = vm.ruleForm.preferredBandwidthNoP,
						isSymmetrical = vm.ruleForm.isSymmetrical,
						bandwidthCodes={
							n25: 5,
		...
		var basicGainValidator2 = (rule,value,callback) => {
			//var regNum = /^\d+$/; // 验证是否是数字
			var regNum = /^-?\d+$/; //正整数+负整数+0，不可有小数点
			var index = 1;
		...
		var basicGainValidator3 = (rule,value,callback) => {
			//var regNum = /^\d+$/; // 验证是否是数字
			var regNum = /^-?\d+$/; //正整数+负整数+0，不可有小数点
			var index = 2;
		...
		return {
			freqMap: {}, // 记录历史freq值，以legecymode、bandwidth、contiguous为联合键
			is5GType:false,
			stepsAction: 0,
			numPopover:'',
			stepsActive:true,
			...
			selectLoginArr:[
				{value:'Frequency,Bandwidth,Power',label:'Frequency,Bandwidth,Power'},
				{value:'Frequency,Power,Bandwidth',label:'Frequency,Power,Bandwidth'},
				{value:'Bandwidth,Frequency,Power',label:'Bandwidth,Frequency,Power'},
				{value:'Bandwidth,Power,Frequency',label:'Bandwidth,Power,Frequency'},
				{value:'Power,Bandwidth,Frequency',label:'Power,Bandwidth,Frequency'},
				{value:'Power,Frequency,Bandwidth',label:'Power,Frequency,Bandwidth'},
			],
			ruleForm:{
				contiguous: 'true',
				legacyMode:'false',
				...
				isSymmetrical:'true',
				preferredBandwidthNoP:'n100',
				widebandModeEnable:'0',
			},
			isQCA:false,
			isDC:false, //是否是两套参数
			isTC:false, //是否是三套参数
			...
			rules:{
				...
				cellName:[
					{min:0,max:48,message:'<%=rb.getString("ChangDuJiaoYan")%>0-48'}
				],
				fccId:[
				...
				antennaGain2:[
					{validator:basicGainValidator2,trigger:'change'}
				],
				antennaGain3:[
					{validator:basicGainValidator3,trigger:'change'}
				],
			},
			installTitleOne:'Antenna',
			...
			showWidebandMode:false,
		}
	},
	computed: {
		freqStatus() {
			var vm = this,
				contiguous = vm.ruleForm.contiguous,
				legacyMode = vm.ruleForm.legacyMode,
				bandwidth = vm.ruleForm.preferredBandwidth;

			return legacyMode + '_' + bandwidth + '_' + contiguous;
		},
		is15Disabled() {
			var vm = this,
				isLegacyMode = [true, 'true'].includes(vm.ruleForm.legacyMode),
				isContiguous = [true, 'true'].includes(vm.ruleForm.contiguous);

			return ((!isLegacyMode && isContiguous ) || (isLegacyMode && !isContiguous) || (isLegacyMode && isContiguous) ) && vm.isQCA === true && vm.ruleForm.isSymmetrical == 'true';
		},
		is5and10Disabled() {
			var vm = this,
				isLegacyMode = [true, 'true'].includes(vm.ruleForm.legacyMode),
			...
			return ( ( isLegacyMode && isContiguous ) || (!isLegacyMode && isContiguous) ) && vm.isQCA === true && vm.ruleForm.isSymmetrical == 'true';
		},
		isMultiArea() {
			var vm = this;

			return (vm.isDC == true || (vm.isQCA == true && vm.ruleForm.contiguous == 'false')) && !(vm.isDC == true && vm.ruleForm.channelReuse == 'true')
		},
		freqRange() {
			var vm = this,
				BWKey = vm.ruleForm.preferredBandwidth,
				BWMap = {
					n25: 5,
					n50: 10,
					n75: 15,
				...
				bwVal = BWMap[BWKey],
				list = [{label: 'Auto', value: ''}],
				min = 3550,
				max = 3700-bwVal/2;

			for(var i = min; i < max;) {
				list.push({label: i + bwVal/2, value: i + bwVal/2+''});
				i += 5;
			}
		...
		freqRangeCA() {
			var vm = this,
				BWKey = vm.ruleForm.isSymmetrical == 'true'? vm.ruleForm.preferredBandwidth : vm.ruleForm.preferredBandwidthNoP,
				BWMap = {
					n25: 5,
					n50: 10,
					n75: 15,
		...
		freqSelectCls() {
			return {
				'freq-select': true,
				'mult-select': this.isMultiArea || this.TC
			}
		},
		...
		showHighBandwidth(){
			...
			return vm.showWidebandMode && vm.ruleForm.widebandModeEnable == '1';
		},
	},
	watch: {
		freqStatus(key) {
			...
			if(list) {
				vm.preferredFreqList = list;
			}else {
				vm.preferredFreqList = [];
			}
		},
		preferredFreqList(list) {
			var vm = this;
			vm.ruleForm.preferredFrequency = list.map(function(item){
				var res = item.low;
				
				if (res == "") return;
				
				if(vm.isMultiArea || vm.isTC == true) {
					res += ':' + item.high;
				}else if(vm.isDC == true && vm.ruleForm.channelReuse == 'true'){// DC and Channel Reuse is enabled
					res += ':' + item.low;
				}
				if(vm.isTC == true){
					res += ':' + item.tcVal;
			...
			// 更新freqMap
			vm.freqMap[vm.freqStatus] = list.map(function(item){ 
		...
		freqRange: {
			handler: function(list) {
				var vm = this,
					length = vm.preferredFreqList.length,
				...
				for(var i = length-1; i>=0; i--) {
					var row = vm.preferredFreqList[i],
						low = row.low,
						high = row.high,
						tcVal = '';
					

					if(!keys.includes(low) || (high && !keys.includes(high)) ||  (tcVal && !keys.includes(tcVal))) idxList.push(i);
				}
				// 删除不合法数据
				idxList.map(function(idx){
					vm.preferredFreqList.splice(idx,1);
				})
			},
			deep: true
		},
	...
	methods:{
		...
		init(isAdd,type,row,sn,iscbsd){
			...
			if(isAdd === 'edit'){
				...
				axios.post("${ctx}/cell/SAS/getCbsdMode.action",stringify(objQuery)).then(function(response){
					var data = response.data;
					var cbsdMode = data.cbsdMode;
					if(cbsdMode == 'CA'){
						vm.isQCA = true
					}
					if(cbsdMode == 'DC'){
						vm.isDC = true;
						vm.carrierType = 'pcell';
						vm.selectBoxShow = true;
					}
					if(cbsdMode == 'BAIBNQ'){
						vm.is5GType = true;
						vm.ruleForm.preferredBandwidth = 'n200';
					}
					if(cbsdMode == 'TC'){
						vm.selectBoxShow = true;
						vm.carrierType = 'pcell';
		...
		getInstallParams(obj,objQuery) {
			...
			axios.post("${ctx}/cell/SAS/getInstallParams.action",stringify(obj)).then(function(response){
				...
				if(data.success){
					...
					vm.legacymode = data.installParam.legacyMode == "true"||data.installParam.legacyMode == true;
					// 解析contiguous值
					installParamsData.contiguous = installParamsData.contiguous == 'false'?'false':'true';
					if(installParamsData.preferredBandwidthNoP == '' || installParamsData.preferredBandwidthNoP == null || installParamsData.preferredBandwidthNoP == 'null' || installParamsData.preferredBandwidthNoP == undefined ){
						installParamsData.preferredBandwidthNoP = 'n100';
						installParamsData.isSymmetrical = 'true';
					}else{
						installParamsData.isSymmetrical = 'false';
					}
					if(installParamsData.cellName == null || installParamsData.cellName == undefined){
						installParamsData.cellName = '';
					}
					if(installParamsData.supportedN48N78 && installParamsData.supportedN48N78 == '1'){
						vm.showWidebandMode = true;
					}
					if(installParamsData.enb_install_params && Object.keys(installParamsData.enb_install_params).length !== 0){
						var enb_install_params = installParamsData.enb_install_params;
						installParamsData.indoor = enb_install_params.indoor;
						installParamsData.cpiId = enb_install_params.cpiId;
						installParamsData.cpiName = enb_install_params.cpiName;
						if(enb_install_params.cellSpecificParams){
							installParamsData.cellSpecificParams = enb_install_params.cellSpecificParams;
							installParamsData.cellSpecificParams.map((item,indexs)=>{
								...
								}else if(item.cellSequenceId == '2'){
									installParamsData.antennaGain2 = item.antennaGain
								}else if(item.cellSequenceId == '3'){
									installParamsData.antennaGain3 = item.antennaGain
								}
								item.heightUnit = item.heightUnit ? item.heightUnit : 'meter'
							})
						}	
						delete installParamsData.enb_install_params
					}else{
						var cellData = [
								{
									cellSequenceId:'1',
						...
						if(vm.isTC){
							installParamsData.cellSpecificParams = cellData;
						}
						if(vm.isDC){
							if(!installParamsData.antennaConfiguration){
								installParamsData.cellSpecificParams = cellData.splice(0,2);
							}else if(installParamsData.antennaConfiguration && installParamsData.antennaConfiguration == '2'){
								installParamsData.cellSpecificParams = cellData.splice(0,2);
							}
						}
						delete installParamsData.enb_install_params
					...
					vm.$nextTick(function(){
						...
						vm.initPreferredFrequency(vm.ruleForm.preferredFrequency);
					})
					...
					var preferredPowerRange = installParamsData.supportedPowerRange,
						preferredPowerList = [],
						preferredPowerRangeStart,
						preferredPowerRangeEnd;

					if(preferredPowerRange){
						preferredPowerRangeStart = parseInt(preferredPowerRange.split(',')[0],10);
						preferredPowerRangeEnd = parseInt(preferredPowerRange.split(',')[1],10);
					}else{
						preferredPowerRangeStart = 0;
						preferredPowerRangeEnd = 20;
					}
					for(var i=preferredPowerRangeStart;i<= preferredPowerRangeEnd;i++){
						var preferredPower = {value:'',text:''};
						preferredPower.value = i+'';
						preferredPower.text = i+'dbm';
						preferredPowerList.push(preferredPower);
					}
					vm.preferredPowerList = preferredPowerList.reverse();
				}
				vm.loading = false;
			})
		},
		initPreferredFrequency(frequency) {
			var vm = this,
				list = (frequency||'').split(','),
				contiguous = vm.ruleForm.contiguous,
				legacyMode = vm.ruleForm.legacyMode,
				bandwidth = vm.ruleForm.preferredBandwidth,
				key = legacyMode + '_' + bandwidth + '_' + contiguous;
			...
			list.map(function(item){
				...
				vm.preferredFreqList.push(obj);
			});

			// 更新freqMap
			vm.freqMap[key] = vm.preferredFreqList.map(function(item){ 
				return Object.assign({}, item);
			});
		},
		...
		//天线配置修改  
		antennaConfigurationChange(val){
			var vm = this;
			if(val == '1'){
				vm.ruleFormParameter.cellSpecificParams.pop();
			}else{
				vm.ruleFormParameter.cellSpecificParams.push({
					cellSequenceId:'2',
					antennaHeightType:'AGL',
					antennaHeight:'',
					longitude:'',
		...
		updateSettings(){// 提交设置函数
			...
			vm.$refs["ruleForm"].validate(valid => {
				if(valid ){
					...
					params.cellSpecificParams = JSON.stringify(params.cellSpecificParams);
					...
					if(vm.ruleForm.isIntelCR == 'false'){
						delete params.antennaPortsCount;
					}
					if(vm.ruleForm.scheduleTime == ''){
						delete params.scheduleTime
					}
					if(params.isSymmetrical == 'true'){
						params.preferredBandwidthNoP = '';
					}
					delete params.isSymmetrical
					delete params.executeType
					params.multiInstall = vm.antennaTwoBox; // 是否是 两套参数
					params.groupIdAndGroupTypeArr = groupIdAndGroupTypeArr;
					params.deviceType = vm.listType;
					delete params.groupIdAndGroupType;
					delete params.isIntelCR;
					...
					if((vm.ruleForm.cbsdCategory == 'A' && vm.oldRegistrationMethod == 'Single-Step' && vm.singleOrMultiTip_Category_A == 'Multi-Step') || (vm.ruleForm.cbsdCategory == 'B' && vm.oldRegistrationMethod == 'Single-Step' && vm.singleOrMultiTip_Category_B == 'Multi-Step')){
						vm.$confirm(regisMethodStrConfirm,'<%=rb.getString("QueRen")%>',{
							...
							vm.loading = true;
							axios.post('${ctx}/cell/SAS/saveEnbCbsdParams.action',stringify(params)).then(function(response){
					    		var data = response.data;
					    		vm.methodType = "";
					    		if(data["success"]){
					    			vm.$message({
		...
		parameterFrom(){ // 打开设置页面
			...
			// vm.$refs["ruleForm"].validateField(['userId','fccId','serialNumber'],function(errorMesage){
			// 	errCount ++;
			// 	if (errorMesage && flag) {
			// 		errIndex = errCount;
			// 		flag = false
			...
			// if (errIndex >0){
			// 	var refs = ["userid","fccid","serialnum"];
			// 	vm.$refs[refs[errIndex-1]].focus();
			// 	return;
			// }
		...
		saveInstallParamsModify(){
			...
			params.cellName = vm.ruleForm.cellName;
			params.cbsdCategory = vm.ruleForm.cbsdCategory;
			params.antennaConfiguration = vm.ruleForm.antennaConfiguration;
			params.cellSpecificParams = JSON.stringify(vm.ruleFormParameter.cellSpecificParams);
		...
		modifyInstallParamsSubmit(params){
			...
			axios.post('${ctx}/cell/SAS/generateCpiCode.action',stringify(params)).then(function(response){
				...
				if(data["success"]){
					...
					vm.showParams();
					var cpiInstallList = data.cpiMsg;
					cpiInstallList.map((item,index)=>{
		...
		showParams(){
			...
			vm.ruleForm.cellSpecificParams.map((item)=>{
				if(item.cellSequenceId == '1'){
					...
					vm.ruleForm.antennaGain2 = item.antennaGain
				}else if(item.cellSequenceId == '3'){
					vm.ruleForm.antennaGain3 = item.antennaGain
				}
			})
		},
		// 取消
		...
		freqSelectChange() {
			this.ruleForm.preferredFrequencyText = '';
			this.$refs.ruleForm.clearValidate('preferredFrequencyText');
		},
		addFreq() {
			var vm = this,
				freqVal = vm.freqLowSelect,
				valueList = vm.preferredFreqList.map(function(item){
								var low = item.low;
			...
			if(vm.isMultiArea || vm.isTC == true) {
				freqVal += ':'+vm.freqHighSelect;
			}else if(vm.isDC == true && vm.ruleForm.channelReuse == 'true'){// DC and Channel Reuse is enabled
				freqVal += ':'+vm.freqLowSelect;
				vm.freqHighSelect = vm.freqLowSelect
			}
			if(vm.isTC == true){// TC
				freqVal += ':'+vm.freqTcValSelect;
			}
			// 个数 15 限制
			if(vm.preferredFreqList.length>=15) {
				vm.$refs.freqHide.validateMessage = 'Limit of 15';
				vm.$refs.freqHide.validateState = 'error';

				return;
			}
			// 重复校验
			if(valueList.includes(freqVal)) {
				vm.$refs.freqHide.validateMessage = '<%=rb.getString("YiCunZai")%>';
				vm.$refs.freqHide.validateState = 'error';

				return;
			}
			var autoText = 'The Configured Preferred Frequency list will be cleared and changed to "Auto"'
			// auto添加逻辑
			if(freqVal == '' || freqVal == ':' || freqVal == '::') {
				vm.$confirm(autoText,'<%=rb.getString("QueRen")%>').then(function(){
					var obj = {
							low: vm.freqLowSelect,
							high: vm.freqHighSelect,
						},
						autoList = [];
					if(vm.isTC == true){
						obj.tcVal = vm.freqTcValSelect
					}
					autoList.push(obj);
					vm.preferredFreqList = autoList;
					...
					vm.ruleForm.preferredFrequencyText = '';
				}).catch(function(){});

				return;
			}

			vm.ruleForm.preferredFrequencyText = freqVal;

			vm.$refs["ruleForm"].validateField('preferredFrequencyText',msg => {
				if(msg) {
					
				}else {
					var index = -1;
					vm.preferredFreqList.map(function(item, idx){
						if(item.low=='') index = idx;
					})

					if(index>=0) {
						vm.preferredFreqList.splice(index,1);
					}
					var obj = {
						low: vm.freqLowSelect,
						high: vm.freqHighSelect,
		...
		deleteFreq(index) {
			this.preferredFreqList.splice(index,1);
		},
		// Fcc Id 改变 增加提示信息
		fccIdChange(val){
			var vm = this;
		...
		*/
			...
			vm.$refs.sasImportCertForm.validate((valid) => {
				if (valid){
					...
					setTimeout(()=>{
						...
						sasMonitorPage.sasCertStatus = true;
		...
		//拖拽开始事件
		preferredFreqDrag(event,index){
			var vm = this;
			event.dataTransfer.setData('index',index);
		},
		//拖拽结束事件
		preferredFreqDrop(event, index){
			var vm = this;
			event.preventDefault();
			let startIndex = parseInt(event.dataTransfer.getData('index'));
			let currentIndex = parseInt(index);
			if(startIndex - currentIndex > 0){
				vm.preferredFreqList.splice(currentIndex,0,vm.preferredFreqList[startIndex]);
				vm.preferredFreqList.splice(startIndex + 1 ,1)
			}else if(startIndex - currentIndex < 0){
				vm.preferredFreqList.splice(currentIndex + 1,0,vm.preferredFreqList[startIndex]);
				vm.preferredFreqList.splice(startIndex,1)
			}else{
		...
		// 高带宽模式切换
		widebandModeEnableChange(val){
			var vm = this;
			if(val && val == '0'){
				['n250','n300','n350','n400','n450','n500'].includes(vm.ruleForm.preferredBandwidth) ? vm.ruleForm.preferredBandwidth = 'n200' : '';
