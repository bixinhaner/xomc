...
<style>
#addViewTracePage_gnb .el-icon-time{
	line-height:1;
}

#addViewTracePage_gnb .timeItem .el-input__inner{
	width:220px;
}
#addViewTracePage_gnb .ml45{
	margin-left: 66px
}
#addViewTracePage_gnb .mt16{
	margin-top: 16px
}
#addViewTracePage_gnb .taskInput{
	width:300px;
	height:30px;
	line-height:30px;	
}
#addViewTracePage_gnb .radioBox{
	border:1px solid #D1ECF5 !important;
	width:660px;
	padding:15px 0px 15px 0px;
}
#addViewTracePage_gnb .el-form-item__label{
	line-height:26px;
	width:140px;
	text-align:left;
}

#addViewTracePage_gnb .alarmBottomLine{
	background-color:#E9E9E9;
	width: 100%;
	height: 1px;
	margin-bottom: 30px; 
}
#addViewTracePage_gnb .titleStyML{
	margin-left: 40px;
}
#addViewTracePage_gnb .el-pairgrid-title{
	top:-6px;
}
#addViewTracePage_gnb .deviceItem{
	display:inline-block;
	margin-right:60px;
}
#addViewTracePage_gnb .pairgrid-right .el-ctable-toolbar{
	padding: 10px!important;
}
#addViewTracePage_gnb .commonBorder { border: 1px solid #E9E9E9; border-radius: 10px;}
#addViewTracePage_gnb .commonToolBarBox .el-query .advanceQuery .el-icon { font-size: 14px; }
#addViewTracePage_gnb .commonFontSize12 { font-size: 12px; }
#addViewTracePage_gnb .commonColor, 
#addViewTracePage_gnb .commonToolBarBox .el-query .advanceQuery .el-icon:before { color: #7A7992; }
#addViewTracePage_gnb .commonColor2 {  color: #999999; }
#addViewTracePage_gnb .commonToolBarBox { height: 30px; }
#addViewTracePage_gnb .marginLeft5 { margin-left: 5px; }
#addViewTracePage_gnb .el-form-item { margin-bottom: 22px; }
#addViewTracePage_gnb .el-form-item__label { color: #666666; } 
#addViewTracePage_gnb .el-radio.is-bordered { max-width: 200px; height: 30px; padding: 7px 12px; }
#addViewTracePage_gnb .el-checkbox.is-bordered { max-width: 200px; height: 30px; padding: 6px 12px; }
#addViewTracePage_gnb .el-radio-group .el-radio__label, 
#addViewTracePage_gnb .el-checkbox-group .el-checkbox__label { font-size: 12px; line-height: unset;}
#addViewTracePage_gnb .el-input__inner { height: 30px; }
#addViewTracePage_gnb .selected-status:before { color: #67D972 !important; }
#addViewTracePage_gnb .textareStyle .el-textarea__inner { resize: none; border-radius: 4px; }
#addViewTracePage_gnb .el-input__inner { border-radius: 4px; }
</style>

<div id="addViewTracePage_gnb" style="margin-top:20px;">
	<el-form :model='ruleForm' :rules="rules" ref="ruleForm" label-position="top">
		...
		<el-ctable id="select_device_list_gnb" :row-key="'serial_number'" ref="deviceCtable" :url='deviceListUrl'
...
var addViewTraceVueGnb = new Vue({
	el:'#addViewTracePage_gnb',
	data(){
		var vm = this,
	...
	methods:{ 
		...
		init(type, row){
			...
			var prodUrl = '${ctx}/cell/cpeinfos/getEnbMonitorProductList.action?isGnb=1'
		...
		submit(){ // 确定按钮 
            ...
            // 防止多次提交
            if(traceVueGnb.slideSubmitLoading)return
            
	    	vm.$refs.ruleForm.validate((valid) => {
	    		if(valid){
                    traceVueGnb.slideSubmitLoading = true;
                    axios.post(url,stringify(params)).then(function(response){
                        ...
                        if(data["success"]){
                            ...
                            eventBus.$emit('hander-cancel-gnb');
                        }else{
                            vm.$message.error(data["message"]);
                            traceVueGnb.slideSubmitLoading = false;
                        }
                    })
	    		}
	    	})
	...
	mounted(){
		eventBus.$off('init-config-gnb').$on('init-config-gnb', this.init); 
		eventBus.$off('hander-ok-gnb').$on('hander-ok-gnb',this.submit);
		eventBus.$off('close-slide-gnb').$on('close-slide-gnb',this.closeSlider);
	}
})
</script>
