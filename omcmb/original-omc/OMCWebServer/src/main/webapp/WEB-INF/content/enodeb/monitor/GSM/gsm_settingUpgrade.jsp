...
#gsmSettingUpgradePage .form-info {
	margin:10px 20px;
}
#gsmSettingUpgradePage .form-info .info-title {
	color:#7A7992;
	display:inline-block;
	width:120px;
}
#gsmSettingUpgradePage .form-info .el-radio-group .el-radio {
	display:block;
	margin-bottom:20px;
	margin-left:0 !important;
}
#gsmSettingUpgradePage .item-title {
	margin:20px 0;
	font-weight:bold;
}
#gsmSettingUpgradePage .form-info .el-form-item__label {
	font-size:12px;
	text-align:left;
	line-height:unset;
}
#gsmSettingUpgradePage .linkStyle {
	color:#4d84ff;
	font-size:14px;
...
<div id="gsmSettingUpgradePage" class="borderPage">
		...
		<div class="form-info">
			...
			<el-form-item prop="type">
				<el-radio-group v-model="upgradeform.type">
					...
					<el-radio label="4" class="CODE_ENB_UPGRADE_PATCH hidden"><%=rb.getString("CAZhengShuShengJi")%></el-radio>
					<el-radio label="6" class="CODE_ENB_UPGRADE_FPGA hidden"><%=rb.getString("FPGAShengJi")%></el-radio>
					<el-radio label="7"><%=rb.getString("APShengJi")%></el-radio>
				</el-radio-group>
			</el-form-item>
		</div>
		<div class="form-info">
...
<script>
var gsmSettingUpgradePageVue = new Vue({
	el: '#gsmSettingUpgradePage',
	data() {
	...
	methods: {
		init(row){
            ...
            reg = new RegExp('\\\\',"g"),
            product =  row.product;
			vm.currentVersion = row.software_version;
			vm.cellCode = row.small_cell_code;
			...
			axios.post('${ctx}/task/upgrade/getProductType.action').then(function(response){
				var productTypeList = response.data ?  response.data : [];
