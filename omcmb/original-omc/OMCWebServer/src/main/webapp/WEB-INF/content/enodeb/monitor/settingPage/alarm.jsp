...
<style>
#enbDetailPages .alarmMinorIcon::before,
#enbAlarmPage .alarmMinorIcon::before{
	color: #FFDA41;
	font-size: 18px;
}
#enbDetailPages .alarmMajorIcon::before,
#enbAlarmPage .alarmMajorIcon::before{
	color: #FF973E;
	font-size: 18px;
}
#enbDetailPages .alarmCriticalIcon::before,
#enbAlarmPage .alarmCriticalIcon::before{
	color: #FC5959;
	font-size: 18px;
}
#enbDetailPages .alarmWarningIcon::before,
#enbAlarmPage .alarmWarningIcon::before{
	color: #60BEFC;
	font-size: 18px;
}
#enbAlarmPage .disabledClass {
	cursor: not-allowed !important;
	opacity: 0.4;
	font-size: 16px;
}
#enbAlarmPage .defaultClass {
	cursor: pointer;
	font-size: 16px;
	color: rgba(0, 0, 0, 0.8);
}
#enbAlarmPage .el-icon-operation-more-circle:hover, .el-icon-operation-more-circle:active {
	color: var(--main-color) !important;
}
#enbAlarmPage .alarmItemTableBoxCls {
    height: 50%;
}
#enbDetailPages label{
	color: #7A7992;
	margin-right:5px;
	text-align:left;
...
#enbAlarmPage .el-ctable-toolbar {
    padding: 0 !important;
}
#enbAlarmPage .el-icon-arrow-right:before{
...
<div id="enbAlarmPage" class="commonWarp" style='background: #FFFFFF;width: 100%; height: 100%;'>
    <div class="alarmItemTableBoxCls">
        <el-ctable ref="alarmTable" :rownumber="true" id="alarmTable" :url="alarmTableUrl" :time="6" :query-params="alarmQueryParams" height="100%" pagination="true">
            ...
            <el-table-column label='' width="40">
                <template slot-scope="scope">
                    <div class="el-icon el-icon-operation-more-circle" @click="optAlarmClick(scope.row,event)" v-clickoutside="handerAlarmClose" style="cursor: pointer;"></div>
                </template>
            </el-table-column>
    ...
    <div class="alarmItemTableBoxCls" style='border-top: 1px solid #D5DCEC;'>
        <el-ctable ref="historyAlarmTable" id="historyAlarmTable" :url="historyAlarmTableUrl" :query-params="historyQueryParams" pagination="true" :time="6" :rownumber=true :row-key="'serial_number'">
            <template slot="toolbar">
                <div class='toolbarHeadBtnBoxCls'>
            ...
            <el-table-column label='' width="40">
                <template slot-scope="scope">
                     <div class="el-icon el-icon-operation-more-circle" @click="optHistoryAlarmClick(scope.row,event)" v-clickoutside="handerAlarmClose"></div>
                 </template>
            </el-table-column>
            <el-table-column label='<%=rb.getString("XuHao")%>' prop="alarm_id" sortable width="120"></el-table-column>
            <el-table-column label='<%=rb.getString("GaoJingJiBie")%>' prop="alarm_serverity_value" sortable>
...
<script>
var enbAlarmVue = new Vue({
	el:'#enbAlarmPage',
	data(){
		return{
			small_cell_code:'',
			status:'',
			activeName:	"alarmView",
            enbSelectedRow:'',
			rowDataAlarm:'',
			alarmTableUrl:'',
			alarmQueryParams:{
	...
	methods:{
		...
		// 初始化
		init(row,code,status){
			var vm = this;
			vm.small_cell_code = code;
			vm.status = status;
			vm.enbSelectedRow = row;
			vm.alarmQueryParams.deviceCode = code;
			...
			axios.post(url,stringify({
				...
				if(data["success"]){
					...
					if(vm.activeHistoryFlag  == true){
                        vm.$refs.alarmTable.refresh();
                    }else{
                        vm.$refs.historyAlarmTable.refresh();
                    }
				}else{
					vm.confirmAlarmDialogClose();
