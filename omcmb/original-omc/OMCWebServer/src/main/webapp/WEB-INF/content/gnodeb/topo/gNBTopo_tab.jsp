...
<div class="overflow-cls">
	<div id="gnb_topo_container" style="height: 99.5%;width: 100%;min-width: 900px;overflow: hidden;display: flex;padding: 2px 0px 0px 5px;background: #fff;">
		<!-- Device group -->
		<div style="width: 240px;height: 100%;position: relative;" class="flex-ctn" v-show="groupShow">
			<div class="flex-ctn absolute-ctn">
				<div style="font-weight: bold;padding: 5px 0;font-size: 14px;border-bottom: 1px solid #E9E9E9;"><%=rb.getString("SheBeiZu")%></div>
				<div style="flex: auto;" @click="observSlider">
					<el-ctable ref="group" :pagination="false" :rownumber="false" :query-params="groupQueryParams" :url="groupURL" @selection-change="groupChange" row-key="id"
						...
						<template slot="toolbar">
							<el-query style="zoom: 0.8;" class="width-mini" type="normal" @query="queryGroup" placeholder="<%=rb.getString("SheBeiZuMingCheng")%>"></el-query>
						</template>
			...
			<div ref="deviceSlider" class="device-slider flex-ctn">
				...
				<div style="flex: auto;overflow: auto;">
					<el-tabs style="height: 100%;" class="is-bottom-active">
						<el-tab-pane label="<%=rb.getString("gNB")%>">
		...
		<div style="flex: auto; display: flex; flex-direction: column;position: relative;background: #fff;">
					...
					<el-form style="width: 500px;padding: 15px;" class="form-item-bottom-15">
						<el-form-item>
							<el-checkbox-group v-model="statusForm.deviceType" style="margin-top: 10px;" @change="setStatus">
								<el-checkbox disabled label="gnb"><%=rb.getString("gNB")%></el-checkbox>
							</el-checkbox-group>
						</el-form-item>
						<el-form-item label="">
							<div>
						...
						<el-form-item>
							<el-checkbox-group v-model="statusForm.ueStatus" style="margin-left: 25px;margin-top: 10px;" @change="ueStatusChange">
								<el-checkbox label="ue"><%=rb.getString("UEShu")%></el-checkbox>
							</el-checkbox-group>
						</el-form-item>
					</el-form>
				</el-popover>
			...
			<div class="topo-legend-cls" v-if="statusType=='deviceStatus'">
				...
				<div class="legend-title" style="margin-top: 25px;">
					<div style="font-weight: bold;text-align: left;padding-left: 15px;">
						<%=rb.getString("gNB")%> <%=rb.getString("SheBeiZongShu")%>：{{deviceInfos.enbTotal}}
					</div>
				</div>
				<div><i class="el-icon el-icon-topo-enb online"></i> <%=rb.getString("gNB")%> - <%=rb.getString("ZaiXian")%> ({{deviceInfos.enbOnlineCount}})</div>
				<div><i class="el-icon el-icon-topo-enb offline"></i> <%=rb.getString("gNB")%> - <%=rb.getString("LiXian")%> ({{deviceInfos.enbTotal - deviceInfos.enbOnlineCount}})</div>
			</div>
			<!-- 图例 -->
			<div class="topo-legend-cls" v-if="statusType=='activeStatus'">
				...
				<div><i class="el-icon el-icon-topo-enb online"></i> <%=rb.getString("gNB")%> - <%=rb.getString("JiHuo")%> ({{deviceInfos.enbActiveCount}})</div>
				<div><i class="el-icon el-icon-topo-enb offline"></i> <%=rb.getString("gNB")%> - <%=rb.getString("QuJiHuo")%> ({{deviceInfos.enbTotal - deviceInfos.enbActiveCount}})</div>
			</div>
			<!-- 图例 -->
			<div class="topo-legend-cls" v-if="statusType=='sasStatus'">
				...
				<div>
					...
					<div style="padding: 8px 0;border-bottom: 1px dashed #e3e3e3;">
						<i class="el-icon el-icon-topo-enb offline"></i> <%=rb.getString("gNB")%>({{deviceInfos.enbunregistered}})
					</div>
				</div>
				<div>
					Registered ({{deviceInfos.registered}})
					<div style="padding: 8px 0;border-bottom: 1px dashed #e3e3e3;">
						<i class="el-icon el-icon-topo-enb registed"></i> <%=rb.getString("gNB")%>({{deviceInfos.enbregistered}})
					</div>
				</div>
				<div>
					Granted ({{deviceInfos.granted}})
					<div style="padding: 8px 0;border-bottom: 1px dashed #e3e3e3;">
						<i class="el-icon el-icon-topo-enb granted"></i> <%=rb.getString("gNB")%>({{deviceInfos.enbgranted}})
					</div>
				</div>
				<div>
					Authorized ({{deviceInfos.authorized}})
					<div style="padding: 8px 0;">
						<i class="el-icon el-icon-topo-enb authed"></i> <%=rb.getString("gNB")%>({{deviceInfos.enbauthorized}})
					</div>
				</div>
			</div>
			<!-- topo 地图 -->
			<div style="flex: auto;overflow: auto;" :class="sasSwitchCls">
				<div id="map_gnb" style="width: 100%;min-height: 100%;background: #A1D4E0;"></div>
			</div>
			<!-- 状态数值 -->
			...
			<div :class="infoPanelCls">
				...
				<div v-show="infoShow && infoForm.type=='gnb'">
					<div class="info-panel-title">
                        <%=rb.getString("XinXi")%>
                        <span class="el-icon el-icon-operation-settings" @click="showGnbTopoSettingPanel"></span>
                    </div>
...
<script type="text/javascript">
		...
		gnbMeasureObj = {
			...
			measure: function() {
				...
				if(this.layer) {
					...
					document.querySelector('#map_gnb').click();
				}
	...
	function hideGnbRepeatedList() {
		$('#gnb_topo_container .repeated-list').fadeOut();
	}
	$('body').off('click',hideGnbRepeatedList).on('click',hideGnbRepeatedList);
	
	var globGnbMap,groupName = '';
	// 扩展字符串的数据映射能力
	...
	var topognbvm = new Vue({
		el: '#gnb_topo_container', 
		data() {
			var vm = this,
				/**
				* 纬度校验
			...
			deviceInfos() {
				var vm = this,
					...
					allEnbNodes = vm.allNodes.concat(vm.repeatedHideNodes).filter(function(item){
						return !['',null].includes(item.lat) && !['',null].includes(item.lon) && item.type == 'gnb';
					}),
					...
					enbOnlineList = allNodes.filter(function(item){
						return item.online == 'on' && item.type == 'gnb';
					}),
					...
					enbActiveList = allNodes.filter(function(item){
						return item.active == 'yes' && item.type == 'gnb';
					}),
					...
					enbUnregList = allNodes.filter(function(item){
						return item.sasState == 'Unregistered' && item.type == 'gnb';
					}),
					enbRegList = allNodes.filter(function(item){
						return item.sasState == 'Registered' && item.type == 'gnb';
					}),
					enbGrantedList = allNodes.filter(function(item){
						return item.sasState == 'Granted' && item.type == 'gnb';
					}),
					enbAuthList = allNodes.filter(function(item){
						return item.sasState == 'Authorized' && item.type == 'gnb';
					}),
		...
		methods: {
			...
			**/
			rowdblclick(row,evt) {
				...
				if(valid && row.latitude && row.longitude){
					globGnbMap.setView([row.latitude-0, row.longitude-0], 3);
					setTimeout(function(){
						highlightNode(code)
					},500)
				}
			...
			**/
			...
			modifyInfoGPS() {
				...
				vm.gpsDlg({
					lat: form.lat,
					lon: form.lon,
					cellCode: form.type == 'gnb'?form.cellCode:form.code,
					sn: form.type == 'gnb'?form.code:form.sn
				});
			},
			/**
			* 菜单点击事件
			...
			**/
			createEnbLayer(nodes,map,bufEnable) {
				...
				var enbOptions = getGnbLayerOptions();
	            // 渲染eNb和 CPE节点
			...
			**/
			createLineLayer(nodes,eNbsLookup,map) {
				var vm = this,
					...
					options = getGnbAllLayerOptions(eNbsLookup);
			...
			**/
			filterRender(nodes, map, bufEnable) {
				...
				map.eachLayer(function(layer){
					...
					if(layer.options && (layer.options.type == 'gnb'||layer.options.fromField == 'gnb')) {
						map.removeLayer(layer);
					}
				});
			...
			initMap() {
				...
				if(globGnbMap) globGnbMap.remove();
				...
				if(offlineMapEnable) {
					...
					// 设置中心位置和放大倍数
					map = L.map('map_gnb',{maxZoom: 12,minZoom: 4}).fitBounds(bounds); // .setMaxBounds([[-360,-360],[360,360]])
				}else {
					// 设置中心位置和放大倍数
					map = L.map('map_gnb',{maxZoom: 18,minZoom: 4}).fitBounds(bounds); // .setMaxBounds([[-360,-360],[360,360]])
				}
				globGnbMap = map;
				...
				// 过滤后生成节点图层
				vm.createEnbLayer(vm.filterNodesByStatus(resObj.enb), globGnbMap);
				// 节点挂着到全局，方便过滤的时候处理
				vm.allNodes = resObj.enb;

				// 事件绑定
				globGnbMap.on('zoomend',function(ev){ // 缩放结束后重新计算
					vm.filterRender(resObj.enb, globGnbMap);
				});// 事件绑定
				globGnbMap.on('moveend',function(ev){ // 移动结束后重新计算
					vm.filterRender(resObj.enb, globGnbMap,true);
				});
			},
			closeMesure() {
			...
			**/
			proccessLines(nodes) {
				var vm = this,
					lineList = [],
					enblist = nodes.filter(function(item){ return item.type=='gnb';}),
			...
			**/
			...
			// 全屏设置
			FullScreen() {
				var el = document.querySelector('#gnb_topo_container');
			...
			**/
            ...
            // 显示GNB拓扑设置面板
            showGnbTopoSettingPanel() { 
                var vm = this,
                    str = Math.random().toString(),
                    rowData = {},
                    params = {
                        timeZone: timeZone,
                        small_cell_code: vm.infoForm.cellCode,
                        isDual: false,
                        isGnb: 1,
                        page: 1,
                        rows: 50
                    };
				axios.post('${ctx}/cell/cpeinfos/queryCpeInfosList.action',stringify(params)).then(function(res){
                    // 检查接口返回数据是否有效
                    ...
                    // 打开设置页面滑动面板
                    // 检查 gnodeb_monitor.jsp 是否已打开 setting.jsp 页面，如果打开则先关闭以防止 ID 冲突
                    if(typeof gnbMonitor !== 'undefined') {
                        try {
                            // 直接尝试关闭 gnodeb_monitor 的 slide，即使它没有打开也不会报错
                    ...
                    vm.sliderHeight = '100%';
                    vm.sliderWidth = '72%';
                    vm.$refs.gnbTopoSettingSlide.showSlide(function(){
                        eventBus.$emit('action-settingPage', rowData, 'info', 'gNBTopo_tab');
                    });
                }).catch(function(error){});

                
            },
			hideGnbTopoSettingSlide() {
				var vm = this;
				vm.$refs.gnbTopoSettingSlide.hide();
			},
			moveTop(row, index) {
				var vm = this;
			...
			replaceTop(node, tar, n) {
				...
				vm.filterRender(vm.allNodes, globGnbMap);
			},
			removeTop(tar) {
				var vm = this,
	...
	**/
	...
	function showGnbRepeatedList(dom,event) {
		...
		// 根据控制状态过滤节点
		var filterNodes = topognbvm.filterNodesByStatus(topognbvm.repeatedHideNodes).map(function(item){
			return item.code
		});

		var repeatedNodes = topognbvm.repeatedMap[codeKey]||'';

        topognbvm.infoShow = false;
		topognbvm.repeatShow = true;
        topognbvm.hideGnbTopoSettingSlide();
		topognbvm.repeatData = topognbvm.repeatedHideNodes.filter(function(item){
			return repeatedNodes.includes(item.code) && filterNodes.includes(item.code);
		});

		var row = topognbvm.allNodes.filter(function(item){
					return item.code == code;
				  })[0];
		
		if(row) topognbvm.repeatData.unshift(row);

		topognbvm.repeatedQueryText = '';
	}
	// 生成enb节点的图层配置项
	function getGnbLayerOptions(){
		...
		var options = {
				...
				layerOptions: {
					...
					dragend: function(evt){
						...
						Object.assign(topognbvm.gpsmvoeForm, {
									...
									$.ajax({
										...
										success: function(data){
											if(data['success']){
												...
												topognbvm.$refs.enbDevice.refresh();
												topognbvm.$refs.cpeDevice.refresh();
											}else{
				...
				setIcon: function (record, options) {// 自定义节点图标
					...
					var latlonKey = record.lat+'-'+record.lon,
						repeatedNodes = topognbvm.repeatedMap[latlonKey]||'',
						listText = '';
					...
					if(repeatedNodes.length) {
						...
						listText += '<span class="repeated-count" key="'+latlonKey+'" code="'+record.code+'" onclick="showGnbRepeatedList(this,event)">+'+ (repeatedNodes.length+1) +'</span>';

						$html.prepend('<span class="repeated-nodes" onclick="event.stopPropagation()">'+ listText +'</span>');
					}

					// ue 
					if(topognbvm.statusForm.ueStatus.includes('ue') || topognbvm.statusForm.ueStatus.includes('mdt')) {
						var ueShow = record.ueShow && topognbvm.statusForm.ueStatus.includes('ue');
						//var hasMDT = topognbvm.statusForm.ueStatus.includes('mdt') && record.hasMDT == true;
						var ueHtml = [
										'<span class="ue-count" code="' + record.code + '" >',
											ueShow? record.ueCount:'',
											//(ueShow && hasMDT)? '<span class="mdt-split-line"></span>':'',
											//hasMDT? '<i class="el-icon el-icon-MDT mdt-flag" onclick="showUEcount(this,event)"></i>':'',
										'</span>'
									];

						$html.prepend(ueHtml.join(''));
					}
					...
					// sas 状态
					if(topognbvm.statusType == 'sasStatus' && record.sasState && ['Unregistered','Registered','Granted','Authorized'].includes(record.sasState)) {
						var codes = {
							Unregistered: 'offline',
							Registered: 'registed',
							Granted: 'grated',
				...
				onEachRecord: function (layer, record) {
					layer.off('click').on('click', function (evt) {
						...
						// 显示详情
						showGnbDeviceInfo(record, evt);
					});

					layer.off('mouseover').on('mouseover', function (evt) {
						if(isGnbMeasuring) return;
						layer.openPopup();
					});
					
					$(window).resize();
					// 点击节点弹出明细层
					layer.bindPopup($(showGnbNodeInfo(record)).wrap('<div/>').parent().html(),{autoPan: false, keepInView: true});
				}
			};
        return options;
    }
	function showGnbDeviceInfo(item, evt) {
		Object.assign(topognbvm.infoForm, item);
        topognbvm.infoShow = true;
        topognbvm.repeatShow = false;
	}
	/**
	* 跳转到告警菜单
	...
	**/
	function showGnbNodeInfo(node){
		var nodeInfo = $.extend({},node);
		var str = [
				'<div class="panelDefault" style="width: 325px;height: 170px;border: none;">',
					'<div class="el-icon el-icon-close node-info-close" onclick="cancelSet()"></div>',
					'<div class="tabsContentDiv">',
						'<div style="position: relative;border-bottom: 1px solid #eee;padding: 5px;font-weight: bold;">gNB Information</div>',
	...
	**/
	function getGnbAllLayerOptions(eNodebsLookup){
		...
		var options = {
			...
			getIndexKey: function (location, record) {
				return record.gnb + '_' + record.tar;
			},
	...
	**/
		...
		if(alarmObj) {
			...
			if(alarmCount) {
				str = '<span class="el-icon el-icon-menu-alarm margin-right-5 '+ type +'"></span>'
						+type+'<span style="color: blue;cursor: pointer;" onclick="jumpToAliveAlarmGnb(&quot;' + row.code + '&quot;)">('+(alarmCount*1)+')</span>';
			}
		}
	...
	function jumpToAliveAlarmGnb(sn) {
    	try{
            var params = {
                alarm_severity: '', 
                unread: '',
