...
<style>
	...
	#online_status_chart .no-data::before,
	#active_status_chart .no-data::before,
	#mme_connected_chart .no-data::before,
	#enb_product_type_chart .no-data::before,
	#device_time_chart .no-data::before {
...
<!-- 右上角导出按钮 -->
<div class="circleIcon placeholder-bt" style="top:15px;right: 210px;" placeholder="<%=rb.getString("DaoChu")%>" id="enb_export_op" @click="showExport">
...
<!-- 基站监控 -->
<div class="panelDefault" id="cellInfo" style="overflow:hidden;background: transparent;border: 1px solid #E9E9E9;" >
	<div class="tabsTitle active-top" id="eNb_tabs_area">
		<span tabtit="singleCarrier" onclick="turnTabs(this)" class="active"><%=rb.getString("LieBiao")%></span>
		<span tabtit="topo" onclick="turnTabs(this);loadEnbTopo();"><%=rb.getString("Topo")%></span>
	</div>
	<div class="tabsContentDiv" style="top: 30px;border: none;">
		<div class="singleCarrier" id="singleCarrier" style="display: block;">
			<%@ include file="enodeb_monitor.jsp" %> <!-- 嵌入基站监控列表页 -->
			...
			<div class="slide-arrow left mainColBg" onclick="toggleExpand()">
				<i class="el-icon el-icon-down"></i>
			</div>
			<div class="slide-arrow right mainColBg" onclick="minisize()">
				<i class="el-icon el-icon-down"></i>
			</div>
		</div>

		<div class="topo">
			<div id="enb_topo_ctner" class="nocontent-loading"></div>
		</div>
	</div>
</div>
<!-- 快速设置 功能 -->
...
<script>
	...
	var exVm = Vue.getInstance('#enb_export_op');
	exVm && exVm.remove();
	new Vue({
		el: '#enb_export_op',
		methods: {
	...
	function minisize() {
		...
		if(cls.contains('minisize')) {
			cls.remove('minisize');
			$('#enb_export_op').css({right: '210px'});
			$('#addDeviceBtn').css({right: '310px'});
			$('#importDeviceBtn').css({right: '260px'});
			$('#addDeviceCard').css({right: '230px'});
			$('#importDeviceCard').css({right: '230px'});
		}else {
			cls.add('minisize');
	...
	function initLineChart(isExpanded) {
		var charts = [],
			titles = {
		...
		["online_status_chart","active_status_chart","mme_connected_chart"].map(function(id){
			...
			if(chartDom) {
				...
				chart.on('timelinechanged',function(p){
					proccessEnb(id,p.currentIndex,isExpanded);
				});
				proccessEnb(id,6,isExpanded);
			}
		});
	...
	function proccessEnb(code,index,bool,suf) {
		...
		$.post("${ctx}/system/device/getDeviceStatisticsDataList.action", paramsENB, function(data){
			var params = {
				...
				chart = echarts.getInstanceByDom(document.querySelector('#'+code+(suf||'')));
...
<script type="text/javascript">
	$(function(){
		$('#eNb_tabs_area span').off('click').on('click',function(){ // 导出按钮控制
			var flag = $(this).attr('tabtit'),
				opTool = $('#enb_export_op');
			if(flag == 'singleCarrier') opTool.show();
			else opTool.hide();
		});
	});
	...
	function loadTopo(){
		$('#group_table_list').datagrid('selectRow',0);
	}
</script>
