...
<style>
	#ipsecRuleDiv .el-input{
		width:300px;
	}
	#ipsecRuleDiv .el-form-item{
		margin-right:100px;
	}
</style>
<div id="ipsecRuleDiv">
...
<script>
	...
	var ipsecRuleVue = new Vue({
		...
		methods:{
			init(){
				var vm = this;
				configRuleVue.paramList.map(function(item){
				...
				if(configRuleVue.operTypeIpsec == "view"){
			...
			saveIpsec(){
				...
				vm.$refs.ipsecForm.validate(function(valid){
					if(valid){
						if(configRuleVue.operTypeIpsec == "add"){
							params.IPSEC_INDEX = configRuleVue.settingForm.ipsecList.length + 1;
							for(var key in vm.ipsecForm){
								params[key] = vm.ipsecForm[key]
							}
							configRuleVue.settingForm.ipsecList.push(params);
							configRuleVue.$refs.slide.hide();
							configRuleVue.$refs.settingForm.validateField("ipsecLength");
						}
						if(configRuleVue.operTypeIpsec == "edit"){
							params.IPSEC_INDEX = configRuleVue.rowDataIpsec.IPSEC_INDEX;
							for(var key in vm.ipsecForm){
								params[key] = vm.ipsecForm[key]
							}
							var ipsecArr = configRuleVue.settingForm.ipsecList.map(function(item){
								return item.IPSEC_INDEX;
							})
							var index = ipsecArr.indexOf(params.IPSEC_INDEX);
							configRuleVue.settingForm.ipsecList.splice(index,1,params);
							configRuleVue.$refs.slide.hide();
						}
					}
				})
			...
			getInfo(){
				var vm = this;
				for(var key in vm.ipsecForm){
					vm.ipsecForm[key] = configRuleVue.rowDataIpsec[key]
				}
			}
		},
		mounted(){
