...
<div class="panelDefault" id="enb_modify_device_record">
	<el-ctable ref="recordList" :rownumber="true" :height="'99%'" time=6 :url="tbURL" :pagination="true" :query-params="queryParam">
		<!-- 查询域 -->
		<template slot="toolbar">
			<div class="queryGroup" style="margin-left: 15px;">
				<input placeholder="<%=rb.getString("JiZhanLieBiao")%>" v-model="form.searchText"/>
				<b style='cursor:pointer' class="el-icon el-icon-common-search" @click="query"></b>
			</div>
		</template>
		<!-- 表格列 -->
		<el-table-column label='<%=rb.getString("JiZhanLieBiao")%>' width="200"  prop="serialNumber"></el-table-column>
...
<script type="text/javascript">
	new Vue({
		el: '#enb_modify_device_record',
		data(){
			
			return {
				tbURL: '',
				exportURL: '${ctx}/cell/password/exportCellUpdatePwdDetailToCSV.action',
				queryParam: {
					searchText: '',
					taskId: '',
					timeZone: timeZone
		...
		methods: {
			init(id){
				...
				vm.tbURL = '${ctx}/cell/password/getCellUpdatePwdDetailPageList.action';
			},
			query(){
				var vm = this;
