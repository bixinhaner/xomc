	...
	var enbFileImportVue = new Vue({
		...
		data(){
			...
			var validateFilePath = (rule,value,callback) => {
				if(cauFileVue.file_type == 'upgrade' && vm.productLabel == 'DXDF'){
					var reg = vm.difFileObj[cauFileVue.file_type].fileFmtYD;
					var message = vm.difFileObj[cauFileVue.file_type].fileErrorMsgYD;
				}else if((vm.productLabel == 'CR-B4860/EU' || vm.productLabel == 'CR-B4860/RU') && cauFileVue.file_type == 'upgrade'){
					var reg = vm.difFileObj[cauFileVue.file_type].fileFmt4860;
					var message = vm.difFileObj[cauFileVue.file_type].fileErrorMsg4860;
				}else{
					var reg = vm.difFileObj[cauFileVue.file_type].fileFmt;
					var message = vm.difFileObj[cauFileVue.file_type].fileErrorMsg;
				}
				if(value == ""){
			...
			versionValidate = function(rule,value,callback) {
				if(value) {
					if(value.length>45) {
						...
						var fileType = vm.importForm.product.includes("CR-B4860") ? vm.importForm.product : cauFileVue.file_type;
		...
		methods:{
			init(){
				var vm = this;
				vm.fileTypeTip = vm.difFileObj[cauFileVue.file_type].fileTip;
				axios.post("${ctx}/cau/upgrade/getProductType.action").then(function(response){
					var data = response.data;
				...
				vm.$refs.enbFileImportForm.validate((valid) => {
					if(valid){
						...
						/* if(vm.importForm.product.includes("CR-B4860")){
							...
							$("#uploadForm_enbFileImport [name=fileType]").val(cauFileVue.file_type);
						} */
						$("#uploadForm_enbFileImport [name=fileType]").val("cau")
					    ...
					    uploadWithProgress({
					    	...
					    	success: function(data){
					    		...
					    		if (data["MD5"]) {
					            	...
					            	cauFileVue.$refs.slide.hide();
					            	cauFileVue.$refs.file_table.refresh();
					            } else {
			...
			getVersion(val){
				var vm = this;
				if(vm.productLabel == 'DXDF' && cauFileVue.file_type == 'upgrade'){
					var fileFmt = vm.difFileObj[cauFileVue.file_type].fileFmtYD;
				}else if((vm.productLabel == 'CR-B4860/EU' || vm.productLabel == 'CR-B4860/RU') && cauFileVue.file_type == 'upgrade'){
					var fileFmt = vm.difFileObj[cauFileVue.file_type].fileFmt4860;
				}else{
					var fileFmt = vm.difFileObj[cauFileVue.file_type].fileFmt;
				}
		...
		watch:{
			...
			productLabel:function(val){
				var vm = this;
				if(val == "DXDF" && cauFileVue.file_type == 'upgrade'){
					vm.fileTypeTip = vm.difFileObj[cauFileVue.file_type].fileTipYD
				}else if((val == "CR-B4860/EU" || val == "CR-B4860/RU") && cauFileVue.file_type == 'upgrade'){
					vm.fileTypeTip = vm.difFileObj[cauFileVue.file_type].fileTip4860;
				}else{
					vm.fileTypeTip = vm.difFileObj[cauFileVue.file_type].fileTip;
				}
