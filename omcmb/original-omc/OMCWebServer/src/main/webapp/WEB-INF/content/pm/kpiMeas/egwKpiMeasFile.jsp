...
<style>
	#egwKpiMeasFilePage .monitorContentBoxCls{
		height: 100%;
		position: relative;
		display: flex;
	}
	#egwKpiMeasFilePage .monitorContentBoxCls >div:first-child{
		flex: 1;
		position: relative;
	}
	#egwKpiMeasFilePage .el-ctable-toolbar{
		padding: 0px !important;
	}
	#egwKpiMeasFilePage .el-ctable th>.cell {
		max-height: 25px !important;
	}
    #egwKpiMeasFilePage .kpiMeasFileHeadCls{
        height: 40px;
        display: flex;
        align-items: center;
        position: relative;
    ...
    #egwKpiMeasFilePage .toolbarHeadBtnBoxCls{
        border-bottom: none;
    }
	#egwKpiMeasFilePage .el-date-editor .el-range__close-icon{
		line-height: 20px;
	}
    #egwKpiMeasFilePage .kpiMeasFileNameBoxCls{
		display: flex;
		align-items: center;
		flex-wrap: nowrap;
	}
	#egwKpiMeasFilePage .kpiMeasFileNameCls{
		overflow: hidden;
		white-space: nowrap;
...
<div class="overflow-cls">
    <div id="egwKpiMeasFilePage" class="panelDefault overHide" style="min-width: 1200px;border:none;">
        <div class="monitorContentBoxCls">
            <el-ctable id="egwKpiMeasFileTable" ref="egwKpiMeasFileTable" 
...
var refreshTable;
if(window.egwKpiMeasFilePageVue) {
	try {
		window.egwKpiMeasFilePageVue.$destroy();
	}catch(e){}
}
window.egwKpiMeasFilePageVue = new Vue({
	el:'#egwKpiMeasFilePage',
	data(){
		return{
            rowDeviceData:'',
			kpiMeasFileTableUrl:'',
	...
	methods:{
		init(row){
            ...
            vm.queryKpiMeasFileParams.smallCellCode = row.egwCode;
			vm.kpiMeasFileTableUrl = '${ctx}/egw/pm/customizemg/getPMFileList.action';
		},
		// 时间粒度查询
		dateChange(val) {
			var vm = this;
		...
		clearBulkSelected(){
			var vm = this;
			vm.$refs["egwKpiMeasFileTable"].clearSelection();
			
		},
		// 设备已选表格 单个删除事件
		delBulkSelected(rows){
			var vm = this,
				tabs = 'egwKpiMeasFileTable',
				rowKey = 'fileName';
		...
		*/
        ...
        batchKpiMeasFileDownload(){
            var vm = this, 
				...
				url='${ctx}/egw/pm/customizemg/doDownloadEnbKPIFileZip.action',
				params={
					timeZone: timeZone,
        ...
        batchKpiMeasFileDelete(){
            var vm = this, 
				fileList=JSON.stringify(vm.selectionData),
				url='${ctx}/egw/pm/customizemg/deleteFiles.action',
				params={
					timeZone: timeZone,
					serialNumber:vm.rowDeviceData.serialNumber,
					fileList:fileList
            ...
            vm.$confirm('<%=rb.getString("QueRenShanChu")%>','<%=rb.getString("QueRen")%>',{
				...
				axios.post(url,stringify(params)).then(function(res){
                    ...
                    if(data.success){
                        ...
                        vm.$refs.egwKpiMeasFileTable.refresh();
                        vm.clearBulkSelected();
                    }else{
                        vm.$message({
        ...
        closeKpiMeasFilePage(){
            var vm = this;
            egwKpiMeasPageVue.$refs.slideView.hide();
        }
	},
    mounted(){
        eventBus.$off("egwkpiMeas-file").$on("egwkpiMeas-file",this.init)
	},
})
</script>