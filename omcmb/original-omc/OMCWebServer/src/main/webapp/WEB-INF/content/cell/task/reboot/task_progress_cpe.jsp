...
<div id='reboot_progress'>
	<el-ctable id="rebootProgressTable" ref="ctable" :url="url" :query-params="params" :height="height" time="6">
		...
		<el-table-column label='<%=rb.getString("CPEBianMa")%>' width="220"  prop="SERIAL_NUMBER"></el-table-column>
		<el-table-column label='<%=rb.getString("CPEName")%>' width="220" prop="HOST_NAME"></el-table-column>
		...
		<el-table-column label='<%=rb.getString("JieGuo")%>' width="250" prop="PROGRESS_RESULT" :formatter='resultFmt'></el-table-column>
...
new Vue({
	...
	data(){
		return{
			url:'${ctx}/task/reboot/getRebootTaskProgressForCpe.action?task_id=${taskInfo.TASK_ID}',
			params:{
				searchText:'',
				timeZone:timeZone,
	...
	methods:{
	    ...
	    exportResult(task_id,type){
	    	var vm = this;
	    	exportByForm('${ctx}/task/reboot/exportRebootProgResultForCpe.action',{
	    		taskId: task_id,
	    		timeZone: timeZone,
