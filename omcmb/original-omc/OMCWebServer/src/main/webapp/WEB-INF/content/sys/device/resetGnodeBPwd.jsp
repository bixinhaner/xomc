...
<style>
	#gnbChangePassWordPage .changePassWordMainBox{
		height: 100%;
		flex: 1;
		display: flex;
		flex-direction: column;
	}
	#gnbChangePassWordPage .container .cmenu{
		z-index: 361!important;
	}
	#gnbChangePassWordPage .deviceTable{
		height: calc(50% - 5px);
	}
	#gnbChangePassWordPage .productTypeButtonBox .el-radio-button{
		margin-right:10px;
	}
	#gnbChangePassWordPage .productTypeButtonBox .el-radio-button__inner{
		border:1px solid #F6F7FB;
		background:none;
		padding:5px  15px;
	}
	#gnbChangePassWordPage .productTypeButtonBox .el-radio-button--mini .el-radio-button__inner,
	#gnbChangePassWordPage .productTypeButtonBox.el-radio-button:first-child .el-radio-button__inner,
	#gnbChangePassWordPage .productTypeButtonBox .el-radio-button:last-child .el-radio-button__inner{
		border-radius:13px;
		border-left:1px solid #F6F7FB;
	}
	#gnbChangePassWordPage .productTypeButtonBox .el-radio-button__orig-radio:checked+.el-radio-button__inner{
		color:#666;
		border-color:#4D84FF;
		background-color:#fff;
		-webkit-box-shadow:none;
	}
	#gnbChangePassWordPage .flex-form {
		display: flex;
		flex-wrap: wrap;
	}
	#gnbChangePassWordPage .flex-form .el-form-item {
		margin-right: 100px;
		margin-bottom: 10px;
	}
	#gnbChangePassWordPage .changePassWordTaskBox{
	...
	#gnbChangePassWordPage .taskHeadBox{
		position: absolute;
		top:15px;
		right: 0px;
		display: flex;
	...
	#gnbChangePassWordPage .taskHeadBox  .statisticsDiv{
		height: 16px;
		line-height: 16px;
		display: flex;
		overflow: hidden;
	...
	#gnbChangePassWordPage .statisticsDiv > div:first-child{
	...
	#gnbChangePassWordPage .statisticsDiv > div:last-child{
		padding: 0px 10px;
	}
	#gnbChangePassWordPage .statisticsDiv .el-icon, #gnbChangePassWordPage .statisticsSuccessDiv .el-icon, #gnbChangePassWordPage .statisticsFailDiv .el-icon{
		margin-right: 6px;
		font-size: 14px;
	}
	#gnbChangePassWordPage .resultHeadBox{
		position: absolute;
		top:17px;
		right: 60px;
		display: flex;
	...
	#gnbChangePassWordPage .resultHeadBox .statisticsSuccessDiv{
		height: 14px;
		line-height: 14px;
		display: flex;
		font-size: 14px;
	...
	#gnbChangePassWordPage .statisticsSuccessDiv .el-icon::before{
		color:#67D972;
	}
	#gnbChangePassWordPage .statisticsSuccessDiv > div:first-child{
		padding: 0px 10px;
		color: #67D972;
	}
	#gnbChangePassWordPage .statisticsSuccessDiv > div:last-child{
		padding: 0px 10px 0 0;
	}
	#gnbChangePassWordPage .resultHeadBox .statisticsFailDiv{
		height: 14px;
		line-height: 14px;
		display: flex;
		font-size: 14px;
	}
	#gnbChangePassWordPage .statisticsFailDiv .el-icon::before{
		color: #E88282;
		content:'\e6fb';
	}
	#gnbChangePassWordPage .statisticsFailDiv > div:first-child{
		padding: 0px 10px;
		color: #E88282;
	}
	
	#gnbChangePassWordPage .productTypeButtonBox{
		height:36px;
		width:100%;
		margin-top:10px;
		line-height:34px;
	...
	#gnbChangePassWordPage .el-query {
		display:inline-block;
	}
	#gnbChangePassWordPage .el-date-editor .el-range__close-icon {
		line-height:20px;
	}
</style>
<!-- eNB修改密码 -->
<div class="pageDefault" id='gnbChangePassWordPage' style='background:unset; border: 0;'>
	<div class="container">
		<div class="changePassWordMainBox">
			<!-- 操作按钮 -->
			<div class="operations">
				<div class="circleIcon CODE_GNB_CHANGE_PASSWORD hidden" placeholder="<%=rb.getString("XinJianRenWu")%>" @click="addTask"  style="right:20px;top:3px;">		
				...
				<el-ctable
					...
					<template slot="toolbar">
						<div class='commonQuery'>
							<span class="commonText14" style='padding: 4px 0 0 10px'><%=rb.getString("gNBSheBei")%></span>
			...
			<div class="changePassWordTaskBox commonWarp" >
				<el-tabs v-model="activeName" class=" newTabs" style='height: 100%;'>
					<el-tab-pane label="<%=rb.getString("RenWuLieBiao")%>" name="task">
						...
						<el-ctable id="changePasswordTaskTableGnb" time=6  @load-success="taskTableLoadSuccess" ref="changePasswordTaskListTable" :url="taskListTableUrl" :query-params="queryTaskParams" :page-size="20" :pagination=true height="100%" class='commonQueryToolbar'>
							<template slot="toolbar">
					...
					<el-tab-pane label="<%=rb.getString("SheBeiLieBiao")%>" name="device"> 
						...
						<el-ctable 
							id="changePasswordResultTableGnb"  class='commonQueryToolbar'
		...
		<!-- slide -->
		<el-slide ref="changePasswordSlide" id="changePasswordSlideGnb" :url='slideUrl' :title="slideTitle" :footer="slideFooter" :header="slideHeader" :position="slidePosition" class='commonBorderSlide'
...
var changePasswordGnbVue = new Vue({
	el:'#gnbChangePassWordPage',
	data(){
		return {
			product_type:'',
			productTypeList:[],
    ...
    computed:{
		...
		optBtnShow() {
			return writableMap['CODE_GNB_CHANGE_PASSWORD'] == true;
		},
    },
	watch:{
		'product_type':function(newValue,oldValue){

			this.queryDeviceParams.productValue = newValue;
	...
	methods:{
		...
		addTask(){
			var vm = this;
			vm.slideUrl = '${ctx}/cell/password/toGnbCellUpdatePwdTaskAdd.action';
			vm.slideFooter = true;
			...
			vm.$refs.changePasswordSlide.showSlide(()=>{
				setTimeout(function(){
					eventBus.$emit('action-init-gnb','','add',selectData.name);
				},100)
			})
		},
		// 设备表格选择事件
		...
		*/
		...
		taskOptClick(row,evt){
			...
			vm.taskMenus = [
                {label:'<%=rb.getString("KaiShi")%>',cls:"el-icon el-icon-operation-start CODE_GNB_CHANGE_PASSWORD hidden",code:'start'},
                {label:'<%=rb.getString("ZanTing")%>',cls:"el-icon el-icon-operation-awaiting CODE_GNB_CHANGE_PASSWORD hidden",code:'wait'},
                {label:'<%=rb.getString("ZhongZhi")%>',cls:"el-icon el-icon-operation-terminate CODE_GNB_CHANGE_PASSWORD hidden",code:'end'},
				{label:'<%=rb.getString("XinXi")%>',cls:"el-icon el-icon-operation-info",code:'info'},
				{label:'<%=rb.getString("XiuGai")%>',cls:"el-icon el-icon-operation-edit CODE_GNB_CHANGE_PASSWORD hidden",code:'mod'},
                {label:'<%=rb.getString("ShanChu")%>',cls:"el-icon el-icon-operation-delete CODE_GNB_CHANGE_PASSWORD hidden",code:'del'},
            ];
		...
		*/
		taskInfo(){
			...
			vm.$refs.changePasswordSlide.showSlide(()=>{
				eventBus.$emit('action-init-gnb',vm.taskRowData.taskId,'view');
			})
		},
		/**
		 * 修改任务
		...
		*/
		taskModify(id){
			...
			vm.$refs.changePasswordSlide.showSlide(()=>{
				eventBus.$emit('action-init-gnb',vm.taskRowData.taskId,'edit');
			})
		},
		/**
		 * 删除升级任务
		...
		*/
		...
		closeSlide(){
			var vm = this;
			eventBus.$emit('cancel-slide-gnb');
	...
	mounted(){
		...
		eventBus.$off('hide-changePassword-slide-gnb').$on('hide-changePassword-slide-gnb',this.hideSlide);
		this.init();
	}
