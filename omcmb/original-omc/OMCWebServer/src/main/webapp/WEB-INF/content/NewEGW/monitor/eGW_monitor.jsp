		...
		<el-ctable
			...
			<el-table-column label="" width="70" class-name="no-text-tips">
				<template slot-scope="scope"><!-- 将元素或组件表示为作用域插槽          -->
                    ...
                    <div v-show="isMonitorWritable" class="el-icon el-icon-operation-more-circle" @click="optClick(scope.row, event)" v-clickoutside="hideMenus"></div>
				</template>
			</el-table-column>
				...
				<template slot-scope="scope">
					...
					<div v-if="scope.row.deviceNameTip !== '0'">
						<el-popover trigger="click">
							<div slot="reference" class="abbreviationCls" v-show="isMonitorWritable">
						...
						<div class="abbreviationCls" v-show="!isMonitorWritable">{{scope.row.egwName}}</div>
					</div>
				</template>
			</el-table-column>
	...
	methods:{
		...
		exportEgwDevice(){
			var vm = this,
				params={
                    timeZone: timeZone,
                    search_text: vm.queryParams.search_text
                },
