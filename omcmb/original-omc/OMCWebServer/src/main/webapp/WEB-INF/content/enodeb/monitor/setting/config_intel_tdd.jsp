...
<!-- tab选项卡 -->
<div id="intelTdd_compactParamConfig" class="inputInfos">
				<!-- 选项 -->
		...
		<div class="omcTabsPage logsTabsMainPage" style="left:20px;top:45px;overflow-x:hidden;overflow-y:auto;height:60vh;">
			...
			<div class="wirelessSetting" style="padding: 20px 0 0 20px;display:block">
				<div class="itemDiv">
					...
					<input id="intelTdd_LTE_CELL_ECI" type="text" name="LTE_CELL_ECI" class="border border-box item" oldValue="${CELL_IDENTITY}" value="${CELL_IDENTITY}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="0" max_value="268435455" 
					...
					<div class="errorTitle" id="intelTdd_LTE_CELL_ECI_err">int, min value: 0, max value: 268435455</div>
				</div>
				<div class="itemDiv">
					<span title="<%=rb.getString("RSCellName")%>"><%=rb.getString("HostName")%></span>
					<input type="text" name="LTE_HOME_NODEB_NAME" id="intelTdd_LTE_HOME_NODEB_NAME" class="border border-box item" oldValue="${HOST_NAME}" value="${HOST_NAME}"
							onblur="intelTdd_validateHostName(event)" max_length="48"
							title="<%=rb.getString("SheBeiMingChengGuiZe")%>"/>
					<div class="errorTitle" id="intelTdd_LTE_HOME_NODEB_NAME_err"><%=rb.getString("SheBeiMingChengGuiZe")%></div>
				</div>
				<div class="itemDiv" style="float:none;clear:both">
					<span title="<%=rb.getString("RSMME")%>"><%=rb.getString("HeXinWang")%></span>
					<input type="text" name="LTE_SIGLINK_SERVER_LIST" id="intelTdd_LTE_SIGLINK_SERVER_LIST" class="border border-box item" 
						oldValue="" value="" onblur="intelTdd_validateIPAddress1(event)" 
						title="<%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%>"/>
					<a onclick="intelTdd_addMMEIpInputText(this)">
						<img src="${ctx}/skin/${manufacturer}/images/bi/setting_add.png"/>  
					</a>
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>
					<div class="errorTitle" id="intelTdd_LTE_SIGLINK_SERVER_LIST_err"><%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%></div>
				</div>
				<div class="itemDiv">
					<span title="<%=rb.getString("RSPLMN")%>"><%=rb.getString("PLMN")%></span>
					<input type="text" name="LTE_OAM_PLMNID" id="intelTdd_LTE_OAM_PLMNID" class="border border-box item" oldValue="${PLMNID}" value="${PLMNID}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="10000" max_value="999999" 
							title="<%=rb.getString("PLMNTitle")%>"/>
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>	
					<div class="errorTitle" id="intelTdd_LTE_OAM_PLMNID_err"><%=rb.getString("PLMNTitle")%></div>		
				</div>
				<div class="itemDiv pciisLock">
					<c:if test="${isElfCell != 1}">
						<span title="<%=rb.getString("RSPCI")%>"><%=rb.getString("PCI2")%></span>
						<input type="text" name="LTE_PHY_CELLID_LIST" id="intelTdd_LTE_PHY_CELLID_LIST" class="border border-box item" oldValue="${PHYCELLID}" value="${PHYCELLID}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="0" max_value="503"
							title="int, min value: 0, max value: 503"/>
							<img src="${ctx}/skin/${manufacturer}/images/bi/pci_lock.png"/>		
						<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>			
						<div class="errorTitle" id="intelTdd_LTE_PHY_CELLID_LIST_err">int, min value: 0, max value: 503</div>			
					</c:if>
					<c:if test="${isElfCell == 1}">
						<span title="<%=rb.getString("RSPCI")%>"><%=rb.getString("PCI2")%></span>
						<input type="text" name="LTE_PHY_CELLID_LIST" id="intelTdd_LTE_PHY_CELLID_LIST" class="border border-box item" oldValue="${PHYCELLID}" value="${PHYCELLID}"
							onblur="intelTdd_commonChangeTip(event)" 
				...
				<div class="itemDiv">
					...
					<input type="text" name="LTE_TAC"  id="intelTdd_LTE_TAC" class="border border-box item" oldValue="${TAC}" value="${TAC}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="0" max_value="65535"
							title="int, min value: 0, max value: 65535"/>
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>	
					<div class="errorTitle" id="intelTdd_LTE_TAC_err">int, min value: 0, max value: 65535</div>	
				</div>
				<div class="itemDiv" style="display: none">
					<span title="<%=rb.getString("RSFrequencyBand")%>"><%=rb.getString("ZhiChiPinDuan")%></span>
					<input type="text" name="LTE_BANDS_SUPPORTED" id="intelTdd_LTE_BANDS_SUPPORTED" class="border border-box item" 
					 		oldValue="${BANDS_SUPPORTED}" value="${BANDS_SUPPORTED}" 
					 		onblur="intelTdd_commonChangeTip(event)" title="stringList-[1:62]"/>
					<div class="errorTitle" id="intelTdd_LTE_BANDS_SUPPORTED_err">stringList-[1:62]</div>	
				</div>
				<div class="itemDiv">
					<span title="<%=rb.getString("RSBandIndicator")%>"><%=rb.getString("PinDuan")%></span>
					<input type="text" name="LTE_FREQ_BAND_INDICATOR" id="intelTdd_LTE_FREQ_BAND_INDICATOR" class="border border-box item" 
					 		oldValue="${FREQBAND_INDICATOR}" value="${FREQBAND_INDICATOR}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="1" max_value="65"
							title="int, min value: 1, max value: 65"/>
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>	
					<div class="errorTitle" id="intelTdd_LTE_FREQ_BAND_INDICATOR_err">int, min value: 1, max value: 65</div>
				</div>
				<div class="itemDiv pciisLock">
					<span title="<%=rb.getString("RSEarfcn")%>"><%=rb.getString("PinDian")%></span>
					<input type="text" name="LTE_UL_DL_EARFCN" id="intelTdd_LTE_UL_DL_EARFCN" class="border border-box item" oldValue="${EARFCNULINUSE}" value="${EARFCNULINUSE}"
							onblur="intelTdd_validateMaxAndMinVal1(event);translateToFre(this)" onfocus="translateToEarfen(this)" min_value="0" max_value="65535"
							title="int, min value: 0, max value: 65535"/>					
						<img src="${ctx}/skin/${manufacturer}/images/bi/pci_lock.png"/>
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>	
					<div class="errorTitle" id="intelTdd_LTE_UL_DL_EARFCN_err">int, min value: 0, max value: 65535</div>
				</div>
				<div class="itemDiv">
					<span title="PB"><%=rb.getString("WuXianDuanKouXinHaoGongLvBi")%></span>
					<select name="LTE_PB" id="intelTdd_LTE_PB" class="border border-box item" oldValue="${LTE_PB}" onchange="intelTddSelectChange(event)">
						<option value=""> </option>
					...
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>
					<input type="hidden" value="" id="intelTdd_OLD_LTE_PB">
				</div>
				<div class="itemDiv" id="intelTdd_intelTddPowerModifyDiv">
					<span title="<%=rb.getString("GongLvDengJi")%>"><%=rb.getString("GongLvDengJi")%></span>
					<input name="LTE_CELL_POWER_MODIFY" id="intelTdd_LTE_CELL_POWER_MODIFY" class=" border border-box item" value="${CELL_POWER_MODIFY}" oldValue="${CELL_POWER_MODIFY}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="15" max_value="40"/>
					<div class="errorTitle" id="intelTdd_LTE_CELL_POWER_MODIFY_err">int, min value: 15, max value: 40</div>
					<input type="hidden" value="" id="intelTdd_OLD_LTE_CELL_POWER_MODIFY">
				</div>
				<div class="itemDiv" style="display:none">
					<span title="<%=rb.getString("RSDLBandWidth")%>"><%=rb.getString("XiaXingDaiKuan")%></span>
					<select name="LTE_DL_BANDWIDTH"  id="intelTdd_LTE_DL_BANDWIDTH" class="border border-box item">
						<option value=""> </option>
					...
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>
					<input type="hidden" value="" id="intelTdd_OLD_LTE_DL_BANDWIDTH">
				</div>
				<div class="itemDiv">
					<span title="<%=rb.getString("RSBandwidth")%>"><%=rb.getString("DaiKuan")%></span>
					<select name="LTE_UL_BANDWIDTH" id="intelTdd_LTE_UL_BANDWIDTH" class="border border-box item" >
						<option value=""> </option>
						<!-- 
					...
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>
					<input type="hidden" value="" id="intelTdd_OLD_LTE_UL_BANDWIDTH">
				</div>
				<div class="itemDiv">
					<span title="<%=rb.getString("RSSfAssignment")%>"><%=rb.getString("ZiZhenPeiBi")%></span>
					<select name="LTE_TDD_SUBFRAME_ASSIGNMENT" id="intelTdd_LTE_TDD_SUBFRAME_ASSIGNMENT" class="border border-box item">
						<option value=""> </option>
					...
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>
					<input type="hidden" value="" id="intelTdd_OLD_LTE_TDD_SUBFRAME_ASSIGNMENT">
				</div>
				<div class="itemDiv">
					<span title="<%=rb.getString("RSSpecialSfPatterns")%>"><%=rb.getString("TeShuZiZhenPeiBi")%></span>
					<select name="LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS" id="intelTdd_LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS" class="border border-box item">
						<option value=""> </option>
					...
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>
					<input type="hidden" value="" id="intelTdd_OLD_LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS">
				</div>
				<div class="itemDiv" style="display:none">
					<span title="<%=rb.getString("RSRootSequenceIndex")%>"><%=rb.getString("GenXuLieSuoYin")%></span>
					<input type="text" name="LTE_SON_PRACH_ROOT_SEQUENCE_INDEX_LIST"  id="intelTdd_LTE_ROOT_SEQ_INDEX" class="border border-box item" 
						oldValue="${ROOT_SEQUENCE_INDEX}" value="${ROOT_SEQUENCE_INDEX}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="0" max_value="837"
							title="int, min value: 0, max value: 837"/>
					<div class="promptTitle"><span><%=rb.getString("SheZhiCellId")%></span><span class="unmodiValue"></span></div>	
					<div class="errorTitle" id="intelTdd_LTE_ROOT_SEQ_INDEX_err">int, min value: 0, max value: 837</div>
				</div>
			</div>
			...
			<div class="intentSetting" style="padding: 20px 0 0 20px;">
				<div class="itemDiv">
					...
					<select name="UNIT_IP_PROTO"  id="intelTdd_UNIT_IP_PROTO" class="border border-box item" >
						<option value="static">static</option>
						<option value="dhcp">dhcp</option>
				    </select>
				    <input type="hidden" value="" id="intelTdd_OLD_UNIT_IP_PROTO">
				</div>
				<div class="itemDiv staticDiv">
					<span><%=rb.getString("WangGuan")%></span>
					<input type="text" name="UNIT_IP_GATEWAY" id="intelTdd_UNIT_IP_GATEWAY"  class="border border-box staticItem"  oldValue="${GATE_WAY}"
					 		value="${GATE_WAY}" onblur="intelTdd_validateIPAddress1(event)" must="1"
					 		title="<%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%>"/>
					<div class="errorTitle" id="intelTdd_UNIT_IP_GATEWAY_err"><%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%></div>
				</div>
				<div class="itemDiv staticDiv">
					<span><%=rb.getString("IPDiZhi")%></span>
					<input type="text" name="UNIT_IP_ADDRESS_NEW" id="intelTdd_UNIT_IP_ADDRESS_NEW" class="border border-box staticItem"  oldValue="${CELL_IP_NEW}"
							value="${CELL_IP_NEW}" onblur="intelTdd_validateIPAddress1(event)" must="1"
					 		title="<%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%>"/>
					<div class="errorTitle" id="intelTdd_UNIT_IP_ADDRESS_NEW_err"><%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%></div>
				</div>
				<div class="itemDiv staticDiv">
					<span><%=rb.getString("ZiWangYanMa")%></span>
					<input type="text" name="UNIT_IP_NET_MASK"  id="intelTdd_UNIT_IP_NET_MASK" class="border border-box staticItem"  oldValue="${IP_NET_MASK}"
							value="${IP_NET_MASK}" onblur="intelTdd_validateIPAddress1(event)" must="1"
					 		title="<%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%>"/>
					<div class="errorTitle" id="intelTdd_UNIT_IP_NET_MASK_err"><%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%></div>
				</div>
				<div class="itemDiv staticDiv" style="display: none">
					<span><%=rb.getString("MoRenDnsDiZhi1")%></span>
					<input type="text" disabled="disabled" name="UNIT_IP_DNS1"  id="intelTdd_UNIT_IP_DNS1" class="border border-box staticItem"  value="114.114.114.114"/>
				</div>
				<div class="itemDiv staticDiv">
					<span><%=rb.getString("MoRenDNS")%></span>
					<input type="text" disabled="disabled" name="UNIT_IP_DNS2"  id="intelTdd_UNIT_IP_DNS2"class="border border-box staticItem"  value="8.8.8.8"/>
				</div>
				<div class="itemDiv staticDiv">
					<span><%=rb.getString("ZiDingYiDnsDiZhi")%></span>
					<input type="text" name="UNIT_IP_DNS"  id="intelTdd_UNIT_IP_DNS" class="border border-box staticItem" oldValue=""  
							onblur="intelTdd_validateIPAddress1(event)"
					 		title="<%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%>"/>
					<div class="errorTitle" id="intelTdd_UNIT_IP_DNS_err"><%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%></div>
				</div>
			</div>
			<!-- ICIC设置 -->
			...
			<div class="NTPSetting" style="padding: 20px 0 0 20px;">
				<div class="itemDiv" style="float:none;clear:both;">
					...
					<input type="text" name="LTE_X_BAICELLS_NTP_SYNC_INTERVAL" id="intelTdd_LTE_X_BAICELLS_NTP_SYNC_INTERVAL" class="border border-box item" oldValue="${X_BAICELLS_NTP_SYNC_INTERVAL}" value="${X_BAICELLS_NTP_SYNC_INTERVAL}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="10" max_value="65535"
							title="int, min value: 10, max value: 65535"/>	
					<div class="errorTitle" id="intelTdd_LTE_X_BAICELLS_NTP_SYNC_INTERVAL_err">int, min value: 10, max value: 65535</div>
				</div>
				<div class="itemDiv">
					<span><%=rb.getString("NTPFuWuQi1") %></span>
					<input type="text" name="LTE_NTP_SERVER_1"  id="intelTdd_LTE_NTP_SERVER_1" class="border border-box item" oldValue="${NTP_SERVER_1}" value="${NTP_SERVER_1}"
							onblur="intelTdd_validateMaxAndMinLength1(event)" min_length="0" max_length="256"
							title="String,min length 0,max length 256.Example:number.country.pool.ntp.org"/>
					<div class="errorTitle" id="intelTdd_LTE_NTP_SERVER_1_err">String,min length 0,max length 256</div>
				</div>
				<div class="itemDiv">
					<span><%=rb.getString("NTPDuanKou1") %></span>
					<input type="text" name="LTE_NTP_PORT_1"  id="intelTdd_LTE_NTP_PORT_1" class="border border-box item" oldValue="${NTP_PORT_1}" value="${NTP_PORT_1}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="1" max_value="20000"
							title="int, min value: 1, max value: 20000"/>
					<div class="errorTitle" id="intelTdd_LTE_NTP_PORT_1_err">int, min value: 1, max value: 20000</div>
				</div>
				<div class="itemDiv">
					<span><%=rb.getString("NTPFuWuQi2") %></span>
					<input type="text" name="LTE_NTP_SERVER_2" id="intelTdd_LTE_NTP_SERVER_2"  class="border border-box item" oldValue="${NTP_SERVER_2}" value="${NTP_SERVER_2}"
							onblur="intelTdd_validateMaxAndMinLength1(event)" min_length="0" max_length="256"
							title="String,min length 0,max length 256.Example:number.country.pool.ntp.org"/>
					<div class="errorTitle" id="intelTdd_LTE_NTP_SERVER_2_err">String,min length 0,max length 256</div>
				</div>
				<div class="itemDiv">
					<span><%=rb.getString("NTPDuanKou2") %></span>
					<input type="text" name="LTE_NTP_PORT_2"  id="intelTdd_LTE_NTP_PORT_2" class="border border-box item" oldValue="${NTP_PORT_2}" value="${NTP_PORT_2}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="1" max_value="20000"
							title="int, min value: 1, max value: 20000"/>
					<div class="errorTitle" id="intelTdd_LTE_NTP_PORT_2_err">int, min value: 1, max value: 20000</div>
				</div>
				<div class="itemDiv">
					<span><%=rb.getString("NTPFuWuQi3") %></span>
					<input type="text" name="LTE_NTP_SERVER_3" id = "intelTdd_LTE_NTP_SERVER_3" class="border border-box item" oldValue="${NTP_SERVER_3}" value="${NTP_SERVER_3}"
							onblur="intelTdd_validateMaxAndMinLength1(event)" min_length="0" max_length="256"
							title="String,min length 0,max length 256.Example:number.country.pool.ntp.org"/>
					<div class="errorTitle" id="intelTdd_LTE_NTP_SERVER_3_err">String,min length 0,max length 256</div>
				</div>
				<div class="itemDiv">
					<span><%=rb.getString("NTPDuanKou3") %></span>
					<input type="text" name="LTE_NTP_PORT_3"  id="intelTdd_LTE_NTP_PORT_3" class="border border-box item" oldValue="${NTP_PORT_3}" value="${NTP_PORT_3}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="1" max_value="20000"
							title="int, min value: 1, max value: 20000"/>
					<div class="errorTitle" id="intelTdd_LTE_NTP_PORT_3_err">int, min value: 1, max value: 20000</div>
				</div>
				<div class="itemDiv">
					<span><%=rb.getString("NTPFuWuQi4") %></span>
					<input type="text" name="LTE_NTP_SERVER_4"  id="LintelTdd_TE_NTP_SERVER_4" class="border border-box item" oldValue="${NTP_SERVER_4}" value="${NTP_SERVER_4}"
							onblur="intelTdd_validateMaxAndMinLength1(event)" min_length="0" max_length="256"
							title="String,min length 0,max length 256.Example:number.country.pool.ntp.org"/>
					<div class="errorTitle" id="LintelTdd_TE_NTP_SERVER_4_err">String,min length 0,max length 256</div>
				</div>
				<div class="itemDiv">
					<span><%=rb.getString("NTPDuanKou4") %></span>
					<input type="text" name="LTE_NTP_PORT_4" id="intelTdd_LTE_NTP_PORT_4"  class="border border-box item" oldValue="${NTP_PORT_4}" value="${NTP_PORT_4}"
							onblur="intelTdd_validateMaxAndMinVal1(event)" min_value="1" max_value="20000"
							title="int, min value: 1, max value: 20000"/>
					<div class="errorTitle" id="intelTdd_LTE_NTP_PORT_4_err">int, min value: 1, max value: 20000</div>
				</div>
			</div>
			<!-- IPSEC设置 -->
			<div class="IPSECSetting" style="padding: 20px 0 0 20px;display:none;" id="intelTdd_IPSECDIV">

			</div>
			
			<!-- LGW设置 -->
			<!-- <div class="LGWSetting">LGW设置</div> -->
			<c:if test="${lgw == 1}">
			<div class="LGWSetting" style="padding: 20px 0 0 20px;" id="lgw">
				...
				<div class="itemDiv staticDiv lgw" name="LGW" id="1" style="float:none;clear:both;">
					...
					<input type="text"  name="LGW_IMSI"  id="intelTdd_LGW_IMSI" class="border border-box itemStatisticInput" style="width:169px;height:26px;padding-left:10px" 
							oldValue="${LGW_IMSI}" onblur="imsionblur(event)" title="<%=rb.getString("LGWImsiTiShi")%>" />
					-
					<input type="text"  name="LGW_IMSI_IP"  id="intelTdd_LGW_IMSI_IP" class="border border-box itemStatisticInput" style="width:169px;height:26px;padding-left:10px" 
							onblur="validateipandrangeforbinding(event)" oldValue="${LGW_IMSI_IP}" title="<%=rb.getString("LGWIpTiShi")%>"/>
		...
		<div class="windowButtonGroup" style="position:absolute;bottom:37px;left:47px;">
			<a class="easyui-linkbutton linkbutton linkbutton_trend" onclick="intelTdd_cellSettingCommit()"><span><%=rb.getString("QueDing")%></span></a>
...
<%-- 窗口-右键设置进度条 --%>
<div id="intelTdd_winSettingPro" title="<%=rb.getString("CanShuPeiZhiJinDu")%>" class="easyui-window"
...
var earfcnoldValue = "${EARFCNULINUSE}";
$(function(){
	//如果SAS开关打开，则该参数不可配置
	if(SASEnble == "1"){
		$("#intelTdd_LTE_UL_DL_EARFCN").attr("disabled",true);
		$("#intelTdd_LTE_UL_BANDWIDTH").attr("disabled",true).css("background","#EAF1F4");
	}
	var earoldVal = translateToFre(earfcnoldValue);
	$("#intelTdd_LTE_UL_DL_EARFCN").val(earoldVal);
})
//宽带
$("select[name='LTE_UL_BANDWIDTH']").bind("change",function(){
	...
	var oldValue=$("#intelTdd_OLD_LTE_UL_BANDWIDTH").val();
	var oldValueText = "";
	...
	for(var i=0;i<opts.length;i++){
		if(opts[i].value == this.value){
			if(oldValue!=this.value){
				$("#intelTdd_LTE_UL_BANDWIDTH").css({"color":"blue"});
				$("#intelTdd_LTE_UL_BANDWIDTH option").css({"color":"black"});
				ele.siblings(".promptTitle").show();
		    	ele.siblings(".promptTitle").find(".unmodiValue").text(oldValueText);
			}else{
				$("#intelTdd_LTE_UL_BANDWIDTH").css({"color":"black"});	
...
function intelTdd_informICICPRBRang(e){
	var ele = $(e["target"]);
	bandWidth = ele.val();
...
$("select[name='LTE_TDD_SUBFRAME_ASSIGNMENT']").bind("change",function(){
	...
	var oldValue=$("#intelTdd_OLD_LTE_TDD_SUBFRAME_ASSIGNMENT").val();
	var oldValueText = "";
	for(var i=0;i<opts.length;i++){
		if(opts[i].value == oldValue){
			oldValueText = $(opts[i]).text();
	...
	for(var i=0;i<opts.length;i++){
		if(opts[i].value == this.value){
			...
			if(oldValue!=this.value){
				$("#intelTdd_LTE_TDD_SUBFRAME_ASSIGNMENT").css({"color":"blue"});
				$("#intelTdd_LTE_TDD_SUBFRAME_ASSIGNMENT option").css({"color":"black"});
				ele.siblings(".promptTitle").show();
		    	ele.siblings(".promptTitle").find(".unmodiValue").text(oldValueText);
			}else{
				$("#intelTdd_LTE_TDD_SUBFRAME_ASSIGNMENT").css({"color":"black"});	
				ele.siblings(".promptTitle").hide();
			}
			break;
		}
...
$("select[name='LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS']").bind("change",function(){
	...
	var oldValue=$("#intelTdd_OLD_LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS").val();
	var oldValueText = "";
	for(var i=0;i<opts.length;i++){
		if(opts[i].value == oldValue){
			oldValueText = $(opts[i]).text();
	...
	for(var i=0;i<opts.length;i++){
		if(opts[i].value == this.value){
			$(opts[i]).attr("selected",true);
			if(oldValue!=this.value){
				$("#intelTdd_LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS").css({"color":"blue"});
				$("#intelTdd_LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS option").css({"color":"black"});
				ele.siblings(".promptTitle").show();
		    	ele.siblings(".promptTitle").find(".unmodiValue").text(oldValueText);
			}else{
				$("#intelTdd_LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS").css({"color":"black"});	
				ele.siblings(".promptTitle").hide();
			}
			break;
		}
...
$("select[name='UNIT_IP_PROTO']").bind("change",function(){
	...
	var oldValue=$("#intelTdd_OLD_UNIT_IP_PROTO").val();
	var ele = $(this);
	var oldValueText = "";
	for(var i=0;i<opts.length;i++){
		if(opts[i].value == oldValue){
	...
	for(var i=0;i<opts.length;i++){
		if(opts[i].value == this.value){
			$(opts[i]).attr("selected",true);
			if(oldValue!=this.value){
				$("#intelTdd_UNIT_IP_PROTO").css({"color":"blue"});
				$("#intelTdd_UNIT_IP_PROTO option").css({"color":"black"});
				ele.siblings(".promptTitle").show();
		    	ele.siblings(".promptTitle").find(".unmodiValue").text("");
			}else{
				$("#intelTdd_UNIT_IP_PROTO").css({"color":"black"});
				ele.siblings(".promptTitle").hide();
			}
			break;
		}
...
function intelTdd_authbyChange(e){
	var ele = $(e["target"]);
	...
	intelTdd_changeColor(ele.attr("id"));
}

function intelTdd_bindInterface(e){
	var ele = $(e["target"]);
	intelTdd_changeColor(ele.attr("id"))
}
...
$(function() {
	...
	if ("${IP_PROTO}" == "") {
		...
		$("#intelTdd_OLD_UNIT_IP_PROTO").val("dhcp");
	} else {
		$("select[name='UNIT_IP_PROTO']").val("${IP_PROTO}");
		$("#intelTdd_OLD_UNIT_IP_PROTO").val("${IP_PROTO}");
	}
	
	$("#intelTdd_OLD_LTE_DL_BANDWIDTH").val("${DL_BANDWIDTH}");
	$("#intelTdd_OLD_LTE_UL_BANDWIDTH").val("${UL_BANDWIDTH}");
	$("#intelTdd_OLD_LTE_TDD_SUBFRAME_ASSIGNMENT").val("${SUBFRAME_ASSIGNMENT}");
	$("#intelTdd_OLD_LTE_TDD_SPECIAL_SUB_FRAME_PATTERNS").val("${SPECIAL_SUBFRAME_PATTERNS}");
	
	$("#intelTdd_LTE_PB").val("${LTE_PB}");
	$("#intelTdd_OLD_LTE_PB").val("${LTE_PB}");
	if ("${LTE_CELL_POWER_MODIFY_LIST}" == "" && false) {
		$("#intelTdd_intelTddPowerModifyDiv").css("display", "none");
	} else {
		//获取可以修改的参数信号功率列表
		...
		if (powerModifyList.indexOf("[") > -1) {
			...
			for (var count = minNum; count <= minMax; count++) {
				...
				$("#intelTdd_LTE_CELL_POWER_MODIFY").append(opt);
			}
		} else {
			var opt = "<option value='"+powerModifyList+"' selected='selected'>"+powerModifyList+"</option>";
			
			$("#intelTdd_LTE_CELL_POWER_MODIFY").append(opt);
			$("#intelTdd_LTE_CELL_POWER_MODIFY").attr("disable", "disable");
		} */
		setTimeout(function(){$("#intelTdd_LTE_CELL_POWER_MODIFY").next().find('input:visible').attr("oldvalue","${CELL_POWER_MODIFY}");},0);
		$("#intelTdd_OLD_LTE_CELL_POWER_MODIFY").val("${CELL_POWER_MODIFY}");
	}
	...
	if(ipsecBindInterface.length != 0 && ipsecBindInterface.BIND_INTERFACE_LIST != ""){
			...
			BangDingJieKou =intelTdd_createIpsecSelectDiv(jsonBandChoice,LEFT_INTERFACE,'<%=rb.getString("BangDingJieKou") %>',"LEFT_INTERFACE");
	}
	
	if(BangDingJieKou.length > 0){
		$("#intelTdd_IPSECDIV").append(BangDingJieKou);
	}
	
	
	if(cellIpsecInfos.length==0 && BangDingJieKou.length == 0){
		$("#intelTdd_IPSECDIV").append('<%=rb.getString("IpsecTiShi") %>');
	}else{
		for(var i=0;i<cellIpsecInfos.length;i++){
			var ID = cellIpsecInfos[i].ID;
			//创建ID文本框
			var IDDIV = intelTdd_createIpsecInputDiv('<%=rb.getString("SuoYin") %>',"ID_"+ID,ID,true);
			var TunnelMingChengDIV = intelTdd_createIpsecInputDiv('<%=rb.getString("TunnelMingCheng") %>',"TUNNEL_NAME_"+ID,cellIpsecInfos[i].TUNNEL_NAME);
			var TunnelWangGuanDIV = intelTdd_createIpsecInputDiv('<%=rb.getString("TunnelWangGuan") %>',"TUNNEL_GATEWAY_"+ID,cellIpsecInfos[i].TUNNEL_GATEWAY);
			var IpsecZiWangDIV = intelTdd_createIpsecInputDiv('<%=rb.getString("IpsecZiWang") %> ',"RIGHT_SUBNET_"+ID,cellIpsecInfos[i].RIGHT_SUBNET);
			var MuDiDuanBiaoJiFuDIV = intelTdd_createIpsecInputDiv('<%=rb.getString("MuDiDuanBiaoJiFu") %>',"RIGHT_IDENTIFIER_"+ID,cellIpsecInfos[i].RIGHT_IDENTIFIER);
			var RenZhengFangShiDIV =intelTdd_createIpsecSelectDiv({"psk":"psk","cert":"cert","aka_psk":"aka_psk","aka_cert":"aka_cert"},cellIpsecInfos[i].AUTHBY,'<%=rb.getString("RenZhengFangShi") %>',"AUTHBY_"+ID);
			var YuGongXiangMiYaoDIV = intelTdd_createIpsecInputPasswordDiv('<%=rb.getString("YuGongXiangMiYao") %>',"PRE_SHARED_KEY_"+ID,cellIpsecInfos[i].PRE_SHARED_KEY);
		
			if("${isSuperAdmin}" == "1"){//超级管理员可以查看预共享秘钥
				$("#intelTdd_IPSECDIV").append(IDDIV).append(TunnelMingChengDIV).append(TunnelWangGuanDIV).append(IpsecZiWangDIV).append(MuDiDuanBiaoJiFuDIV).append(RenZhengFangShiDIV).append(YuGongXiangMiYaoDIV);
			}else{
				$("#intelTdd_IPSECDIV").append(IDDIV).append(TunnelMingChengDIV).append(TunnelWangGuanDIV).append(IpsecZiWangDIV).append(MuDiDuanBiaoJiFuDIV).append(RenZhengFangShiDIV);
			}
		}
		...
		$("select[name^='AUTHBY']").change(intelTdd_authbyChange);
		$("select[name^='LEFT_INTERFACE']").change(intelTdd_bindInterface);
	}
		...
		if (mmeArr.length > 1) {
			...
			for(var count = 1; count < newMMEInputCount; count++) {
				...
				var $mmeInput = $("<input type='text' name='LTE_SIGLINK_SERVER_LIST' class='border border-box item' "
						+ "id='LTE_SIGLINK_SERVER_LIST_org_"+count+"' onblur='intelTdd_validateIPAddress1(event)' oldValue='" + mmeArr[count] + "' value='" + mmeArr[count] + "' style='margin-left:0px'></input>");
				var $mmeButtn = $("<a onclick='intelTdd_removeMMEIpInputText(this)' style='margin-left:3px'>" 
...
<%-- 关闭窗口 --%>
function intelTdd_closeSelfWin() {
	/* $("#winCellSettings").window("close"); */
	closeDefaultWindow();
}
<%-- 提交 --%>
function intelTdd_cellSettingCommit() {
	...
	$.post("${ctx}/cell/cpeinfos/getCellConnStatus.action", {"small_cell_code": cellCode}, function(data){
		// 基站未连接，不允许修改参数
		 if(data["connStatus"] == "false") {
			...
			if ($("#intelTdd_compactParamConfig .item.err_border").length > 0) {
				return;
			}
			if($("select[name='LGW_ENABLE']").val()=="1"){
				var lgwModeSelect = $("select[name='LGW_MODE']").val();
				if(lgwModeSelect=='NAT'){
					if ($("#intelFdd_compactParamConfig .lgwInput.err_border").length > 0) {
						return;
					}
					...
					if(selectStaticAddrInput == "1") {
						if ($("#intelFdd_compactParamConfig .itemStatisticInput.err_border").length > 0) {
							return;
						}
					}
				}
			...
			if(selectProto == "static") {
				if ($("#intelTdd_compactParamConfig .staticItem.err_border").length > 0) {
					return;
				}
			}
			...
			$("#intelTdd_compactParamConfig .item,.lgwInput,.itemStatisticInput").each(function() {
				...
				if(oldValue == undefined){
					...
					var selectId = "intelTdd_OLD_" + paramName;
					var reg = new RegExp("^LGW");
					if(reg.test(paramName)){
						oldValue = $("#intelTdd_OLD_" + paramName).val();
					} else {
				...
				if (isReboot(params) || lgwchange || icicreboot) {
					...
					if ("${noReboot}" == 1) {
						...
						$.messager.confirm("<%=rb.getString("QueRen")%>", promptInfo, function (r) {
				            if (r) {
				            	$("#intelTdd_winSettingPro").window("open");
				            	//请求修改参数
				    			$.post("${ctx}/cell/param/compactParamConfig.action", params, function(data) {
				    				$("#intelTdd_winSettingPro").window("close");
				    				if (data["success"]) {
				    					intelTdd_closeSelfWin();
...
var randomNum = 0;
//添加一个MME地址输入框
function intelTdd_addMMEIpInputText(e) {
	//添加新的输入框
	randomNum += 1;
	...
	var $mmeDiv = $("<div class='itemDiv'></div>");
	var $mmeInput = $("<input type='text' name='LTE_SIGLINK_SERVER_LIST' class='border border-box item' "
		 + "id='LTE_SIGLINK_SERVER_LIST_"+randomNum+"' onblur='intelTdd_validateIPAddress1(event)'  oldValue='' style='margin-left:0px'></input>");
	var $mmeButtn = $("<a onclick='intelTdd_removeMMEIpInputText(this)' style='margin-left:3px'>" 
			+ "<img src='${ctx}/skin/${manufacturer}/images/bi/setting_sub.png'/></a>");
	var $tip = $("<div class='promptTitle'><span><%=rb.getString("SheZhiCellId")%></span><span class='unmodiValue'></span></div>" + 
			"<div class='errorTitle' id='LTE_SIGLINK_SERVER_LIST_"+ randomNum +"_err'><%=rb.getString("QingShuRuYiGeYouXiaoDeIPDiZhi")%></div>");
	...
	$(e).attr("onclick", "intelTdd_addMMEIpInputText(this)");
}
//删除选中的MME地址输入框
function intelTdd_removeMMEIpInputText(e) {
	$(e).parent("div").remove();
...
function intelTdd_createIpsecInputDiv(name,filedName,value,disabled){
  /*   var mes = "\<\%=rb.getString(\'"+tip+"\')\%\>"; */ 
	if(disabled == true){
		...
		var div = "<div class='itemDiv'><span>"+name+"</span><input id='intelTdd_"+filedName+"' onblur='intelTdd_commonChangeTip(event)' type='text' name='"+filedName+"' class='border border-box item'  oldValue='"
	   +value+"' value='"+value+"'/></div>";
	}
	return $(div);
}

function intelTdd_createIpsecInputPasswordDiv(name,filedName,value,disabled){
	var tip = intelTdd_createTitleForIpSec(filedName);
	if(disabled == true){
		var div = "<div class='itemDiv'><span>"+name+"</span><input type='password' name='"+filedName+"' class='border border-box item' oldValue='"
		+value+"' value='"+value+"' disabled='disabled'/></div>";
	}else{
		var div = "<div class='itemDiv'><span>"+name+"</span><input type='password' id='intelTdd_" + filedName + "' onblur='intelTdd_commonChangeTip(event)' name='"+filedName+"' class='border border-box item'  oldValue='"
	   +value+"' value='"+value+"'/></div>";
	}
	return $(div);
}

function intelTdd_createIpsecSelectDiv(jsonOption,selectedValue,name,filedName){
	...
	var selectHtml="<select name='"+filedName+"' id='intelTdd_" + filedName +"' class='border border-box item'>"+opts+"</select>";
	if (!selectedValue) {
		selectedValue = "";
	}
	var div = "<div class='itemDiv'><span>"+name+"</span>"+selectHtml+"<input type='hidden' value='"+selectedValue+"' id='intelTdd_OLD_"+filedName+"'>";
	return $(div);
}


function intelTdd_validateMaxAndMinVal1(e) {
    var ele = $(e["target"]);
    ele.siblings(".promptTitle").hide();
    ...
    if(!$("#" + ele.attr("id") + "_err").is(':visible')){
	    ...
	    if(id=="LTE_FREQ_BAND_INDICATOR"){
	    	$("#intelTdd_LTE_BANDS_SUPPORTED").val(currVal);
	    }
    }
}
...
function intelTdd_validateMaxAndMinLength1(e){
  	var ele = $(e["target"]);
    var currVal = ele.val();
    var oldValue = ele.attr("oldValue");
    if(currVal!=oldValue){
...
function intelTdd_validateHostName(e){
	var ele = $(e["target"]);
    var currVal = ele.val();
...
function intelTdd_validateIPAddress1(e){
	var ele = $(e["target"]);
    ele.siblings(".promptTitle").hide();
    validateIPAddress(e);
    if(!$("#" + ele.attr("id") + "_err").is(':visible')){
...
function intelTdd_validateIPAddress11(e){
  	var ele = $(e["target"]);
    ele.siblings(".promptTitle").hide();
    validateIPAddress(e);
    if(!$("#" + ele.attr("id") + "_err").is(':visible')){
...
function intelTdd_commonChangeTip(e){
	var ele = $(e["target"]);
    ele.siblings(".promptTitle").hide();
	validateByRegex(e)
	if(!$("#" + ele.attr("id") + "_err").is(':visible')){
...
function intelTdd_changeColor(id){
	var opts = $("#"+id).find("option");
	//需要对OLD的命名规则为intelTdd_OLD_mibdn,此时发过来的id为intelTdd_mibdn，需要转换
	var i = id.indexOf("_");
...
function intelTdd_createTitleForIpSec(fieldName){
}
...
//LGW IP Pool Netmask
/* $("select[name='LGW_IP_POOL_NETMASK']").bind("change",function(){
	var ele = $(this);
	var currVal = $("#intelTdd_LGW_IP_POOL_NETMASK").val();
	var oldValueText = "255.255.255.0";
	if(currVal != oldValueText){
		ele.siblings(".promptTitle").show();
    	ele.siblings(".promptTitle").find(".unmodiValue").text(oldValueText);
	}else{
		ele.siblings(".promptTitle").hide();
...
function intelTdd_addISMIAndIPInputText(e) {
	...
	var $mmeInput = $("<input type='text' name='LGW_IMSI' class='border border-box itemStatisticInput' style='width:169px;height:26px;margin-left:0px;padding-left:10px' "  + " title='<%=rb.getString("LGWImsiTiShi")%>'"
		 + "id='intelTdd_LGW_IMSI_"+ imsiIP_tdd +"' onblur='imsionblur(event)'  oldValue='"+oldValue+"' ></input>"
		 +" - "
		 ...
		 + "id='intelTdd_LGW_IMSI_IP_"+ imsiIP_tdd +"' onblur='validateipandrangeforbinding(event)'  oldValue='"+oldValue+"' ></input>"
	);
	var $mmeButtn = $("<a onclick='intelTdd_removeIMSIAndIPInputText(this)' style='margin-left:3px' >" 
			+ "<img src='${ctx}/skin/${manufacturer}/images/bi/setting_sub.png'/></a>");
	var $tip = $("<div class='errorTitle' id='intelTdd_LGW_IMSI_"+ imsiIP_tdd +"_err'><%=rb.getString("LGWImsiTiShi")%></span></div>"+
		    "<div class='errorTitle' id='intelTdd_LGW_IMSI_IP_"+ imsiIP_tdd +"_err'><%=rb.getString("LGWIpTiShi")%></span></div>");
