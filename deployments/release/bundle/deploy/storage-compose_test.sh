...
not_contains "Docker 安装脚本不含固定默认网段" 'DOCKER_BIP_DEFAULT' "$DOCKER_INSTALL"
contains "Docker 安装脚本调用共享规划库" 'docker_network_plan' "$DOCKER_INSTALL"
contains "镜像源脚本调用共享规划库" 'docker_network_plan' "$SETUP_MIRRORS"
contains "镜像源脚本读取指定 daemon 配置" 'docker_network_read_daemon_bip "$DAEMON_JSON"' "$SETUP_MIRRORS"
