...
PM_ROOT_FIX_PLAN="$REPO_ROOT/docs/superpowers/plans/2026-07-30-pm-aggregation-resource-drift-root-fix.md"
...
contains "资源计划漂移检查 CPU 实际限额" 'container_spec_cpu_quota' "$HOST_ALERTS"
contains "资源计划漂移检查内存实际限额" 'container_spec_memory_limit_bytes' "$HOST_ALERTS"
contains "根治计划包含生产 CPU 基线" 'app=2, acs=5, worker=8, postgres=10, postgres-tsdb=16,' "$PM_ROOT_FIX_PLAN"
contains "根治计划包含生产内存基线" 'app=1536MiB, acs=4GiB, worker=2GiB,' "$PM_ROOT_FIX_PLAN"
contains "资源漂移基线 app" 'app 2/1536MiB' "$HOST_ALERTS"
contains "资源漂移基线 acs" 'acs 5/4096MiB' "$HOST_ALERTS"
contains "资源漂移基线 worker" 'worker 8/2048MiB' "$HOST_ALERTS"
contains "资源漂移基线 postgres" 'postgres 10/7168MiB' "$HOST_ALERTS"
contains "资源漂移基线 tsdb" 'postgres-tsdb 16/7168MiB' "$HOST_ALERTS"
contains "资源漂移基线 redis" 'redis 2/8192MiB' "$HOST_ALERTS"
contains "资源漂移基线 nats" 'nats 1/1024MiB' "$HOST_ALERTS"
contains "资源漂移基线 minio" 'minio 4/4096MiB' "$HOST_ALERTS"
contains "资源漂移基线 web" 'web 1/512MiB' "$HOST_ALERTS"
...
contains "overview 展示关闭队列" 'PM 窗口关闭与尾延迟' "$GRAFANA_OVERVIEW"
contains "overview 展示重算成本" 'PM 重算批量与快照扫描成本' "$GRAFANA_OVERVIEW"
contains "overview 展示结果替换成本" 'PM 结果替换与上卷快照耗时' "$GRAFANA_OVERVIEW"
