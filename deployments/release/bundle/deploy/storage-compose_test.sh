...
contains "安装健康检查默认动态等待 180 秒" 'OMC_HEALTHCHECK_TIMEOUT:-180' "$INSTALL"
contains "安装健康检查默认启用最终复核 120 秒" 'OMC_HEALTHCHECK_FINAL_GRACE:-120' "$INSTALL"
contains "安装健康检查默认单轮探针 90 秒" 'OMC_HEALTHCHECK_PROBE_TIMEOUT:-90' "$INSTALL"
contains "安装健康检查最终复核有独立探针上限" 'OMC_HEALTHCHECK_FINAL_PROBE_TIMEOUT:-120' "$INSTALL"
