...
DOCKER_INSTALL="$REPO_ROOT/deployments/release/bundle/docker/install-docker.sh"
DOCKER_NETWORK_LIB="$REPO_ROOT/deployments/docker/docker-network-lib.sh"
DOCKER_NETWORK_TEST="$REPO_ROOT/deployments/docker/docker-network-lib_test.sh"
SETUP_MIRRORS="$REPO_ROOT/deployments/release/bundle/setup-mirrors.sh"
DNS_FIX="$REPO_ROOT/deployments/docker/fix-docker-dns.sh"
DNS_DIAGNOSE="$REPO_ROOT/deployments/docker/diagnose-dns.sh"
...
RELEASE_CONF="$REPO_ROOT/deployments/release/release.conf"
MIGRATION_BASELINE_TEST="$REPO_ROOT/omcgo/cmd/migrate/migration_baseline_files_test.go"
...
echo "── Docker 网段统一由 DOCKER_BIP 派生 ──"
contains "开发 Compose 使用派生业务网段" 'subnet: "${DOCKER_COMPOSE_SUBNET:?DOCKER_BIP plan is required}"' "$DEV_COMPOSE"
contains "开发 Compose 使用新网络名" 'name: omcgo-dev-net' "$DEV_COMPOSE"
contains "测试 Compose 使用派生测试网段" 'subnet: "${DOCKER_TEST_SUBNET:?DOCKER_BIP plan is required}"' "$TEST_COMPOSE"
contains "测试 Compose 使用独立网络名" 'name: omcgo-test-net' "$TEST_COMPOSE"
contains "release infra 使用派生业务网段" 'subnet: "${DOCKER_COMPOSE_SUBNET:?DOCKER_BIP plan is required}"' "$RELEASE_COMPOSE"
contains "release app 独立启动使用派生业务网段" 'subnet: "${DOCKER_COMPOSE_SUBNET:?DOCKER_BIP plan is required}"' "$RELEASE_APP_COMPOSE"
contains "release web 独立启动使用派生业务网段" 'subnet: "${DOCKER_COMPOSE_SUBNET:?DOCKER_BIP plan is required}"' "$RELEASE_WEB_COMPOSE"
contains "release monitoring 独立启动使用派生业务网段" 'subnet: "${DOCKER_COMPOSE_SUBNET:?DOCKER_BIP plan is required}"' "$RELEASE_MONITORING_COMPOSE"
contains "web Dockerfile 使用固定 Nginx 镜像" 'FROM nginx:1.31.2-alpine' "$WEB_DOCKERFILE"
contains "Nginx HTTPS 门禁使用固定镜像" 'NGINX_TEST_IMAGE="${NGINX_TEST_IMAGE:-nginx:1.31.2-alpine}"' "$RELEASE_DEPLOY/nginx-https-file-entry_test.sh"
contains "Docker 安装脚本解析 DOCKER_BIP" 'docker_network_resolve_bip' "$DOCKER_INSTALL"
contains "Docker 安装脚本使用固定默认网段库" 'DOCKER_BIP_DEFAULT' "$DOCKER_NETWORK_LIB"
contains "发布包 .env 写入固定默认网段" 'DOCKER_BIP=${DOCKER_BIP_DEFAULT}' "$BUILD"
contains "发布配置声明固定默认网段" 'DOCKER_BIP_DEFAULT="10.240.0.1/16"' "$RELEASE_CONF"
contains "Docker 安装脚本调用共享规划库" 'docker_network_plan' "$DOCKER_INSTALL"
contains "Docker 安装脚本要求 python3" 'docker_network_require_python3' "$DOCKER_INSTALL"
contains "release 安装要求 python3" 'docker_network_require_python3' "$INSTALL"
contains "镜像源脚本调用共享规划库" 'docker_network_plan' "$SETUP_MIRRORS"
contains "镜像源脚本统一解析 DOCKER_BIP" 'docker_network_resolve_bip' "$SETUP_MIRRORS"
contains "镜像源脚本要求 python3" 'docker_network_require_python3' "$SETUP_MIRRORS"
contains "DNS 修复脚本调用共享规划库" 'docker_network_plan' "$DNS_FIX"
contains "DNS 修复脚本区分 Docker Desktop" 'DOCKER_OPERATING_SYSTEM' "$DNS_FIX"
contains "DNS 修复脚本限制 Linux 写入" 'HOST_OS" = "Linux"' "$DNS_FIX"
contains "共享规划库是唯一派生入口" 'docker_network_plan()' "$DOCKER_NETWORK_LIB"
contains "共享规划库派生 Compose 网段" 'DOCKER_COMPOSE_SUBNET' "$DOCKER_NETWORK_LIB"
contains "共享规划库校验允许网段" '100.64.0.0/10' "$DOCKER_NETWORK_LIB"
contains "共享规划库校验派生不越界" 'network.subnet_of(allowed)' "$DOCKER_NETWORK_LIB"
contains "共享规划库限制 OMC 网络清理" 'docker_network_is_omc_owned' "$DOCKER_NETWORK_LIB"
contains "release 安装清理规划外空网络" 'cleanup_empty_unplanned_docker_networks' "$INSTALL"
contains "Docker 安装清理规划外空网络" 'docker_network_cleanup_unplanned_networks' "$DOCKER_INSTALL"
contains "ACS 信任网关使用派生网段" 'trusted_proxy_cidrs: ["${DOCKER_COMPOSE_SUBNET}"]' "$ACS_PROD_CONFIG"
contains "release ACS 注入派生网段" 'DOCKER_COMPOSE_SUBNET: "${DOCKER_COMPOSE_SUBNET:?DOCKER_BIP plan is required}"' "$RELEASE_APP_COMPOSE"
contains "开发 ACS 注入派生网段" 'DOCKER_COMPOSE_SUBNET: "${DOCKER_COMPOSE_SUBNET:?DOCKER_BIP plan is required}"' "$DEV_COMPOSE"
contains "ACS 生产配置使用派生网段" 'trusted_proxy_cidrs: ["${DOCKER_COMPOSE_SUBNET}"]' "$ACS_PROD_CONFIG"
contains "healthcheck 不把 daemon 当规划输入" 'docker_network_resolve_bip_from_config' "$RELEASE_HEALTHCHECK"
contains "网络诊断不把 daemon 当规划输入" 'docker_network_resolve_bip_from_config' "$REPO_ROOT/deployments/docker/diagnose-network.sh"
contains "DNS 诊断不把 daemon 当规划输入" 'docker_network_resolve_bip_from_config' "$DNS_DIAGNOSE"
valid_bash "Docker 网段规划测试" "$DOCKER_NETWORK_TEST"
if bash "$DOCKER_NETWORK_TEST"; then
  ok
else
  bad "DOCKER_BIP 派生规划回归"
fi

echo "── 发布迁移基线语句边界 ──"
contains "发布构建调用迁移基线门禁" 'validate_release_migration_baseline' "$BUILD"
contains "迁移测试覆盖 UPS Goose 边界" 'TestUPSBaselinePLpgSQLStatementsHaveGooseBoundaries' "$MIGRATION_BASELINE_TEST"
...
contains "普通升级补齐 ACS 可信网关" 'upgrade_acs_trusted_proxy_cidrs' "$INSTALL"
...
contains "真实 NATS 验证支持平台网络模式" 'NATS_DOCKER_NETWORK' "$RELEASE_NATS_VERIFY"
contains "真实 NATS 验证识别 Docker Desktop" 'Docker\ Desktop' "$RELEASE_NATS_VERIFY"
contains "真实 NATS bridge 模式发布回环端口" '127.0.0.1:$PORT:$PORT' "$RELEASE_NATS_VERIFY"
contains "真实 NATS bridge 模式监听容器地址" 'nats_bind_address=0.0.0.0' "$RELEASE_NATS_VERIFY"
...
contains "启动检查一次读取 Docker service 标签快照" 'CONTAINER_SNAPSHOT="$(docker ps' "$RELEASE_HEALTHCHECK"
contains "启动检查按 YAML 角色解析" 'redis-routing-check-lib.sh' "$RELEASE_HEALTHCHECK"
contains "完整检查输出交付提交" 'package_metadata_value "$metadata_key"' "$RELEASE_HEALTHCHECK"
contains "完整检查输出容器镜像 ID" 'container_image_identity "$svc"' "$RELEASE_HEALTHCHECK"
...
contains "release ACS 使用可动态解析的正式 upstream 池" 'server acs-primary:7557 resolve;' "$NGINX_DEFAULT"
...
not_contains "release ACS 不再使用会轮询到 candidate 的共享别名" 'server acs:7557 resolve;' "$NGINX_DEFAULT"
not_contains "release ACS candidate 不进入正式 upstream" 'server acs-candidate:7557' "$NGINX_DEFAULT"
...
contains "healthcheck 从 collector 外部探测" 'http_ready_with_retry http://127.0.0.1:13133/' "$RELEASE_HEALTHCHECK"
contains "healthcheck readiness 探针重试连接拒绝" '--retry-connrefused' "$RELEASE_HEALTHCHECK"
contains "healthcheck readiness 探针单次超时覆盖 ACS 依赖检查" '--max-time 6' "$RELEASE_HEALTHCHECK"
contains "healthcheck readiness 探针有界重试" '--retry-max-time 20' "$RELEASE_HEALTHCHECK"
contains "healthcheck 候选 ACS readiness 使用重试探针" 'http_ready_with_retry "http://${ip}:7557/readyz"' "$RELEASE_HEALTHCHECK"
...
contains "healthcheck 复用一次容器快照" 'CONTAINER_SNAPSHOT="$(docker ps' "$RELEASE_HEALTHCHECK"
contains "healthcheck 复用容器 ID" 'container_id()' "$RELEASE_HEALTHCHECK"
contains "healthcheck 容器探针直连 docker exec" 'docker exec "$cid" "$@"' "$RELEASE_HEALTHCHECK"
contains "healthcheck 缓存 nginx 配置" 'WEB_NGINX_CONFIG_STATE=0' "$RELEASE_HEALTHCHECK"
contains "healthcheck 批量读取 Redis 参数" 'redis-cli --raw CONFIG GET maxmemory maxmemory-policy appendonly' "$RELEASE_HEALTHCHECK"
contains "healthcheck 批量读取 PostgreSQL 参数" 'SHOW shared_buffers; SHOW work_mem; SHOW max_connections' "$RELEASE_HEALTHCHECK"
not_contains "healthcheck 不为每个探针启动 timeout" 'timeout_probe()' "$RELEASE_HEALTHCHECK"
not_contains "healthcheck 不输出探针 SKIP" 'probe_skipped' "$RELEASE_HEALTHCHECK"
not_contains "healthcheck 不使用 compose exec 探针" 'compose_cli exec' "$RELEASE_HEALTHCHECK"
...
contains "ACS 网关忽略客户端伪造转发链" 'real_ip_header X-OMC-Direct-Transport-Peer' "$NGINX_DEFAULT"
contains "本地 ACS 网关忽略客户端伪造转发链" 'real_ip_header X-OMC-Direct-Transport-Peer' "$NGINX_LOCAL"
