...
echo "── Docker 仅使用 173.x 网段 ──"
contains "开发 Compose 固定 173.18 网段" 'subnet: 173.18.0.0/16' "$DEV_COMPOSE"
contains "开发 Compose 使用新网络名" 'name: omcgo-dev-net' "$DEV_COMPOSE"
contains "测试 Compose 固定 173.20 网段" 'subnet: 173.20.0.0/16' "$TEST_COMPOSE"
contains "测试 Compose 使用独立网络名" 'name: omcgo-test-net' "$TEST_COMPOSE"
contains "release infra 固定 173.18 网段" 'subnet: 173.18.0.0/16' "$RELEASE_COMPOSE"
contains "release app 独立启动固定 173.18 网段" 'subnet: 173.18.0.0/16' "$RELEASE_APP_COMPOSE"
contains "release web 独立启动固定 173.18 网段" 'subnet: 173.18.0.0/16' "$RELEASE_WEB_COMPOSE"
contains "release monitoring 独立启动固定 173.18 网段" 'subnet: 173.18.0.0/16' "$RELEASE_MONITORING_COMPOSE"
contains "Docker 安装脚本固定 bip" 'DOCKER_BIP="${DOCKER_BIP:-173.17.0.1/16}"' "$DOCKER_INSTALL"
contains "Docker 安装脚本固定自动地址池" 'DOCKER_ADDR_POOL_BASE="${DOCKER_ADDR_POOL_BASE:-173.19.0.0/16}"' "$DOCKER_INSTALL"
contains "镜像源脚本降级配置保留 bip" '"bip": "173.17.0.1/16"' "$SETUP_MIRRORS"
