...
### 3.2 按实际预算缩放的 floor-first 分配（评审关键修正）

朴素的「权重 × 预算」会在最低配主机上把 Postgres 压到 2g 却仍跑 `max_connections=200`——
**正好重演它要修的那个 OOM**。所以改为：

1. **先算预算**：从当前 `MemAvailable` 扣除系统保留、其它项目预留和监控固定成本，得到本机实际业务预算。
  32 GiB 只作为基线画像，不作为最终申请值。
2. **按预算缩放下限**：每个组件先按「实际业务预算 / 32 GiB 基线业务 floor 总额」缩放
  `floor` 和 `ceiling`，再对 ACS、两套数据库、Redis 等组件应用最低可运行比例约束。
  双 ACS 副本使用相同动态规格，绝不再把候选实例按固定 4 GiB 计账。
3. **门禁（带容忍度）**：动态 `Σfloor`（含监控）是最低门槛；`--floor-tolerance-pct`（默认 30）
   划出一个缓冲带——缺口在容忍度内（即 `空闲预算 ≥ Σfloor × (1 − 容忍度%)`）只降级为
   **WARN + 按下限分配**（不再向上伸缩），不阻断部署；缺口超过容忍度才 **die + 给建议最低配**。
  但即使提高容忍度，也禁止生成超过主机物理内存的计划。
4. **再分余量**：`剩余 = 空闲预算 − Σfloor`，按权重分给可伸缩组件，每个**封顶到动态 `ceiling`**。
5. **全量记账**：nats/minio/web/monitoring 也计入预算；最后校验 `Σ限额 ≤ 空闲预算`。

### 3.3 组件 floor / ceiling 表

单位 MiB。下表是 32 GiB 基线画像，实际 `floor/ceiling` 会按本机业务预算缩放；
再大仍以横向扩展为主。

| 组件 | 32 GiB 基线 floor | 32 GiB 基线 ceiling | 余量权重 | 依据 |
|------|------:|--------:|:-------:|------|
...
档位按**空闲内存**自动判定（`--tier` 可手动覆盖）；档位主要影响 CPU，内存始终按实际预算动态缩放：
...
**最低配置门禁（带 30% 容忍度）**：实际业务预算先按比例缩放 floor；ACS、双 PG、Redis
仍需满足最低可运行比例。预算不足时直接拒绝，不能通过提高容忍度生成超过物理内存的计划。
`--skip-monitoring` 只会移除监控固定成本，不会把业务组件恢复为固定 32 GiB 配置。
