...
REDIS_CORE_CPUS=2
REDIS_CORE_MEM=4g
REDIS_CORE_MAXMEMORY=3g
REDIS_PM_CPUS=2
REDIS_PM_MEM=8g
REDIS_PM_MAXMEMORY=6g
NATS_CPUS=1
NATS_MEM=1g
...
grep -Fx 'omc_resource_plan_memory_limit_bytes{compose_service="redis-core"} 4294967296' "$TMP/resource-plan.prom"
grep -Fx 'omc_resource_plan_memory_limit_bytes{compose_service="redis-pm"} 8589934592' "$TMP/resource-plan.prom"
...
[ "$(grep -c '^omc_resource_plan_cpu_cores{compose_service=' "$TMP/resource-plan.prom")" = 11 ]
[ "$(grep -c '^omc_resource_plan_memory_limit_bytes{compose_service=' "$TMP/resource-plan.prom")" = 11 ]
