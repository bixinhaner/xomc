...
ENV_PRESERVE_KEYS="POSTGRES_USER POSTGRES_PASSWORD POSTGRES_DB POSTGRES_TSDB_USER POSTGRES_TSDB_PASSWORD POSTGRES_TSDB_DB MINIO_ROOT_USER MINIO_ROOT_PASSWORD GRAFANA_ADMIN_USER GRAFANA_ADMIN_PASSWORD OMCGO_JWT_SECRET OMC_SHARED_SECRET OMC_PUBLIC_HOST OMCGO_NOTIFICATION_SMTP_ENABLED OMCGO_NOTIFICATION_SMTP_HOST OMCGO_NOTIFICATION_SMTP_PORT OMCGO_NOTIFICATION_SMTP_USERNAME OMCGO_NOTIFICATION_SMTP_PASSWORD OMCGO_NOTIFICATION_SMTP_FROM OMCGO_NOTIFICATION_SMTP_TLS_MODE OMCGO_NOTIFICATION_SMTP_TIMEOUT OMCGO_NOTIFICATION_SMTP_MAX_ATTACHMENT_BYTES POSTGRES_DATA_PATH TSDB_DATA_PATH REDIS_DATA_PATH REDIS_PM_DATA_PATH NATS_DATA_PATH MINIO_DATA_PATH OMCGO_DOCKER_ROOT_DIR OMCGO_DOCKER_VOLUME_PREFIX OMCGO_HOST_LOGS_PATH OMCGO_HOST_DATA_PATH DOCKER_LOG_MAX_SIZE DOCKER_LOG_MAX_FILE PM_AGGREGATION_FINALIZE_CONCURRENCY GPV_PROVISION_QUEUE GPV_PROVISION_CONCURRENCY GPV_PROVISION_QUEUE_DEPTH GPV_RPC_DURABLE GPV_RPC_SOURCE_CONSUMER GPV_RPC_START_SEQUENCE GPV_RPC_CONCURRENCY GPV_RPC_QUEUE_DEPTH GPV_ACK_WAIT GPV_MAX_DELIVER GPV_MAX_ACK_PENDING"
...
[ -f "$PKG_ROOT/license/keystore/omcPublicKey.store" ] || die "缺 license/keystore/omcPublicKey.store（发布包不完整）" "Missing license/keystore/omcPublicKey.store (incomplete release package)" 1
LICENSE_KEYSTORE_SHA256="1650aebbb63f320408ade0bc75128eec45d423d50a6ae61e4e368a30216f3331"
LICENSE_KEYSTORE_ACTUAL_SHA256="$(sha256sum "$PKG_ROOT/license/keystore/omcPublicKey.store" | awk '{print $1}')"
[ "$LICENSE_KEYSTORE_ACTUAL_SHA256" = "$LICENSE_KEYSTORE_SHA256" ] || die "License 公钥库校验失败：期望 $LICENSE_KEYSTORE_SHA256，实际 $LICENSE_KEYSTORE_ACTUAL_SHA256" "License keystore checksum mismatch: expected $LICENSE_KEYSTORE_SHA256, got $LICENSE_KEYSTORE_ACTUAL_SHA256" 1
...
# TrueLicense 公钥库外置到实例目录并只读挂载。升级时保留现场自定义 OEM keystore；
# 首次部署或旧版本缺失时，使用发布包内经过 SHA-256 门禁的默认公钥库播种。
install -d -m 0755 "$OMC_ROOT/etc/license"
if [ ! -f "$OMC_ROOT/etc/license/omcPublicKey.store" ]; then
  install -m 0644 "$RELEASE_DIR/license/keystore/omcPublicKey.store" \
    "$OMC_ROOT/etc/license/omcPublicKey.store" ||
    die "安装 License 公钥库失败：$OMC_ROOT/etc/license/omcPublicKey.store" "Failed to install the License keystore at $OMC_ROOT/etc/license/omcPublicKey.store" 1
  log "已安装 License 公钥库：$OMC_ROOT/etc/license/omcPublicKey.store" "Installed License keystore: $OMC_ROOT/etc/license/omcPublicKey.store"
else
  log "检测到现有 License 公钥库，升级时保留不覆盖：$OMC_ROOT/etc/license/omcPublicKey.store" "Existing License keystore detected and preserved during upgrade: $OMC_ROOT/etc/license/omcPublicKey.store"
fi
