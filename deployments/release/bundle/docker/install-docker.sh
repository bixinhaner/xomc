...
if command -v docker >/dev/null 2>&1; then
  ...
  log "如需配置加速镜像：bash setup-docker-mirror.sh --mirror <name>"
  exit 0
fi
...
# ── 加速镜像配置 ────────────────────────────────────────────────────────
if [ "$NO_MIRROR" = 1 ]; then
  log "--no-mirror：跳过加速镜像配置。后期可单独运行 bash setup-docker-mirror.sh"
elif [ -n "$MIRROR" ]; then
  log "按 --mirror=$MIRROR 配置加速镜像 ..."
  bash "$(dirname "$0")/setup-docker-mirror.sh" --mirror "$MIRROR"
else
  echo
  echo "─────────────────────────────────────────────────"
  echo " Docker 已装好。要不要配置【加速镜像】？"
  echo "  · 推荐配置（内网拉镜像 / 后续 docker pull 都会快很多）"
  echo "  · 跳过也行（后期可单独运行 setup-docker-mirror.sh）"
  echo "─────────────────────────────────────────────────"
  read -rp "现在配置加速镜像？[Y/n] " yn
  case "${yn:-Y}" in
    [Yy]*|"") bash "$(dirname "$0")/setup-docker-mirror.sh" ;;
    *)         log "跳过加速镜像配置" ;;
  esac
fi
...
log "  · 重新选择加速器：  sudo bash setup-docker-mirror.sh"
log "  · 看当前加速配置：  bash setup-docker-mirror.sh --show"
