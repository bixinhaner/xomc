...
UNINSTALL_SCRIPT="$SCRIPT_DIR/bundle/deploy/uninstall.sh"
SECRETS_LIB="$SCRIPT_DIR/bundle/deploy/secrets-lib.sh"
...
grep -Fq 'chmod 600 "$STAGE/deploy/.env"' "$BUILD_SCRIPT" || fail "release .env template must be private"
grep -Fq 'chmod 600 "$new"' "$INSTALL_SCRIPT" || fail "installed/upgraded .env must be private"
grep -Fq 'chmod 600 "$OMC_ROOT/etc/.env.saved"' "$INSTALL_SCRIPT" || fail "install snapshot must be private"
grep -Fq 'chmod 600 "$target_env"' "$SECRETS_LIB" || fail "credential-populated .env must be private"
grep -Fq 'chmod 600 "$OMC_ROOT/etc/.env.saved"' "$UNINSTALL_SCRIPT" || fail "uninstall snapshot must be private"
